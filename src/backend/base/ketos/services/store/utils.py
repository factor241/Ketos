...
if TYPE_CHECKING:
    from ketos.services.store.schema import ListComponentResponse
...
# Get the latest released version of ketos (https://pypi.org/project/ketos/)
async def get_lf_version_from_pypi():
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get("https://pypi.org/pypi/ketos/json")
        if response.status_code != httpx.codes.OK:
            return None
        return response.json()["info"]["version"]
    except Exception:  # noqa: BLE001
        logger.debug("Error getting the latest version of ketos from PyPI", exc_info=True)
        return None
