...
Compatibility callers invoke :func:`migrate_flow_payload` explicitly before
constructing a graph.  This module deliberately does not patch or register a
graph-deserializer hook.  The rewrite is intentionally tolerant:
...
visited so the caller can retain an auditable account of each compatibility
rewrite without coupling the migration package to an event service.
"""
...
from kfx.extension.migration.schema import (
    _NAMESPACED_ID_RE,
    MigrationEntry,
    MigrationTable,
)
...
class MigrationReport:
    ...
    nodes are no-ops. ``records`` retains every visited node (including
    no-ops) for auditing and diagnostics.
    """
...
def _component_type_aliases(components: Mapping[str, Any]) -> set[str]:
    ...
    from kfx.utils.component_aliases import get_component_type_aliases
...
def _known_current_types_from_index() -> frozenset[str]:
    ...
    """
    try:
        from kfx.graph.flow_builder.builder import load_local_registry
...
def _known_current_types_from_cache() -> set[str]:
    ...
    components_module = sys.modules.get("kfx.interface.components")
...
# Saved-flow node shape (current Ketos): the canonical legacy reference
    ...
    *,
    ...
    if table is None:
        ...
        if load_error is not None or loaded_table is None:
            ...
            from kfx.extension.migration.loader import empty_table
