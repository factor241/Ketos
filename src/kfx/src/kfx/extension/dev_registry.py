"""Local dev-extension registry for ``kfx extension dev``.

When an author types ``kfx extension dev <path>``, we don't ship the
Extension via pip; instead we record the absolute path in a small JSON
state file, then hand off to ``ketos run``.  At server startup, the
loader reads this state file and registers each surviving directory at
the ``official`` slot via :func:`kfx.extension.loader.load_extension`.
...
Scope notes:
    ...
    - Installed-package discovery keeps its own primitives and is
      ...
      Ketos startup merges in the same step.
"""
...
def _read_state(state_path: Path) -> list[DevExtensionEntry]:
    ...
    * **File present but unreadable** (permission error etc.) -- log a
      ...
      extension on the next ``ketos run`` with no diagnostic.  A
    ...
    * **File present but corrupt JSON / wrong shape** -- log a WARNING
    ...
    try:
        ...
        logger.warning(
            ...
            "treating as empty registry. Re-run `kfx extension dev <path>` "
...
def _write_state(state_path: Path, entries: Iterable[DevExtensionEntry]) -> None:
    ...
    ``dev_extensions.json`` gets code loaded at the next ``ketos run``;
        ...
        if not entry.path.is_dir():
            ...
            result.warnings.append(
                ExtensionError(
                    ...
                    hint=(
                        "Restore the directory or run `kfx extension dev <path>` "
...
def dev_extension_component_paths(*, state_dir: Path | None = None) -> tuple[list[Path], list[ExtensionError]]:
    ...
    paths, IDE integrations).  The Ketos startup path itself uses
    ...
    for result in load_dev_extensions(state_dir=state_dir):
        ...
        # caller (Ketos's lifespan hook) is the only thing that turns
        ...
        if result.source_path is None:
            ...
            errors.append(
                ExtensionError(
                    ...
                    location="<unknown>",
                    hint=("File a bug against kfx.extension.loader: LoadResult had components but no source_path."),
                )
            )
            continue
