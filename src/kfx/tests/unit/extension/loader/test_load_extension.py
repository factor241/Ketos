...
def test_single_bundle_registers_component(tmp_path: Path) -> None:
    ...
    assert result.extension_id == "kfx-pilot"
...
def test_locale_bundle_and_missing_marker_flow_to_loaded_components(tmp_path: Path) -> None:
    ...
    assert component.locale_bundle.namespace == "kfx-pilot"
...
def test_distribution_passed_through(tmp_path: Path) -> None:
    root = make_extension(tmp_path)
    result = load_extension(root, distribution="kfx-pilot")
    assert result.ok
    assert result.components[0].distribution == "kfx-pilot"
...
def test_runtime_multi_bundle_check(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from kfx.extension.loader import _orchestrator
...
def test_version_constraint_unsatisfied(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A manifest declaring kfx.compat without our BUNDLE_API_VERSION is rejected.
    ...
    so a bundle declaring only ``["2"]`` against a kfx that ships
    ...
    forged = ExtensionManifest.model_construct(
        ...
        version="1.2.3",
        name="Pilot",
        kfx=KfxCompat(compat=["999"]),  # well-formed but does not include BUNDLE_API_VERSION
...
def test_re_imported_class_is_skipped_via_module_filter(tmp_path: Path) -> None:
    ...
    coincidentally failed because the loader's synthetic ``_kfx_ext.*``
    ...
    :func:`kfx.extension.loader._detection.collect_component_classes`.
    """
    ...
    files = {
        ...
        "z_alias.py": (
            ...
            "    if name.startswith('_kfx_ext.') and name.endswith('primary')\n"
            ")\n"
