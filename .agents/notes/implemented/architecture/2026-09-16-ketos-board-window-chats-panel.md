...
**Geometry is a function of the window rectangle.** `window/panel-geometry.ts` derives the rectangle in world units from `BoardWindowState`: windowed it asks for 26% of the window width (floor 220px, cap 320px), takes 92% of its height centred, and tucks 30% of its own width under the frame's left edge, which lands the visible strip near a fifth of the window's area; fullscreen it docks to the board panel's left edge at full height. Because the panel lives inside the transformed canvas surface, dragging, resizing, panning, and zooming need no panel code at all.
