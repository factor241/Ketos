...
- Concrete ``*PayloadFields`` classes (for example deployment payload fields)
  define canonical slot names in kfx so both layers share one structure.
- Adapter integrations populate adapter-side registries in kfx
  (``*PayloadSchemas`` subclasses).
- Ketos integrations populate API-side registries in Ketos
  (for example ``DeploymentApiPayloads``), including Ketos-specific
