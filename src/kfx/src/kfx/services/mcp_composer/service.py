...
COMPOSER_BACKEND_AUTH_HEADER = "x-ketos-mcp-composer-token"
...
class MCPComposerService(Service):
    ...
    def __init__(self):
        ...
        self._backend_auth_tokens: dict[str, str] = {}  # project_id -> internal Composer-to-Ketos token
    ...
    def get_or_create_backend_auth_token(self, project_id: str) -> str:
        """Return the internal token used by MCP Composer to call Ketos's MCP endpoint."""
    ...
    def validate_backend_auth_token(self, project_id: str, token: str | None) -> bool:
        """Validate an internal Composer-to-Ketos token for a project."""
    ...
    async def _ensure_port_available(self, port: int, current_project_id: str) -> None:
        ...
        if not is_port_available:
            ...
            if port_owner_project == current_project_id:
                ...
                await logger.aerror(
                    f"Port {port} is in use by an unknown process (not owned by Ketos). "
    ...
    def _build_backend_auth_member_config(
        ...
        *,
        ...
        """Build mcp-composer member-server config with internal Ketos auth headers."""
    ...
    def _normalize_oauth_callback_aliases(cls, auth_config: dict[str, Any] | None) -> dict[str, Any]:
        ...
        `oauth_callback_url` is the canonical field name in Ketos. For compatibility,
    ...
    async def start_project_composer(
        ...
        *,
        ...
        Args:
            project_id: The project ID
            streamable_http_url: Streamable HTTP endpoint for the remote Ketos MCP server
