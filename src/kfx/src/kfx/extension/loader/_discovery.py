...
DEFAULT_MODULE_NAMESPACE: str = "_kfx_ext"
...
def module_name_for(
    ...
    With the default namespace this is ``_kfx_ext.<slot>.<bundle>.<dotted>``;
...
def import_bundle_module(module_name: str, file_path: Path) -> tuple[types.ModuleType | None, ExtensionError | None]:
    ...
    # entry under the same synthetic ``_kfx_ext.<slot>.<bundle>...`` name
    ...
    # ``_kfx_ext.<slot>.<bundle>.*`` sys.modules namespace) before calling
