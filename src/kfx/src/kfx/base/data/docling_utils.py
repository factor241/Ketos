...
def _get_docling_document_class() -> type[Any]:
    try:
        ...
        install_command = (
            "Install Docling with `uv pip install 'ketos[docling]'` or `uv pip install 'kfx-docling[local]'`."
        )
...
def docling_worker(
    *,
    ...
    try:
        ...
        msg = (
            "Docling is an optional dependency of Ketos. "
            "Install with `uv pip install 'ketos[docling]'` "
