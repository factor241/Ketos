"""Main Ketos to Stepflow converter implementation."""
...
class WorkflowAnalysis:
    """Typed analysis results from analyzing a Ketos workflow."""

    node_count: int  # Total number of nodes in the Ketos workflow
...
class KetosConverter:
    """Convert Ketos JSON workflows to Stepflow YAML workflows."""
    ...
    def convert_file(self, input_path: str | Path) -> str:
        """Convert a Ketos JSON file to Stepflow YAML.

        Args:
            input_path: Path to the Ketos JSON file
        ...
        workflow = self.convert(ketos_data)
        return self.to_yaml(workflow)

    def convert(self, ketos_data: dict[str, Any]) -> Flow:
        """Convert Ketos data structure to Stepflow workflow.

        Args:
            ketos_data: Parsed Ketos JSON data
        ...
        try:
            ...
            if "data" not in ketos_data:
                raise ConversionError("Invalid Ketos JSON: missing 'data' key")

            data = ketos_data["data"]
            nodes = data.get("nodes", [])
            edges = data.get("edges", [])

            if not nodes:
                raise ConversionError("No nodes found in Ketos workflow")
            ...
            # Create FlowBuilder
            builder = FlowBuilder(name=self._generate_workflow_name(ketos_data))
    ...
    def analyze(self, ketos_data: dict[str, Any]) -> WorkflowAnalysis:
        """Analyze Ketos workflow structure without conversion.
    ...
    def _generate_workflow_name(self, ketos_data: dict[str, Any]) -> str:
        """Generate a workflow name from Ketos data."""
        # Try to get name from various sources
        if "name" in ketos_data:
            name = ketos_data["name"]
            return str(name) if name is not None else "Converted Ketos Workflow"
        ...
        return "Converted Ketos Workflow"
