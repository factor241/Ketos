...
class TestParseDevMode:
    ...
    def test_dev_mode_not_set(self, monkeypatch):
        """Test default behavior when KFX_DEV is not set."""
        monkeypatch.delenv("KFX_DEV", raising=False)
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None

    def test_dev_mode_enabled_with_1(self, monkeypatch):
        """Test dev mode enabled with KFX_DEV=1."""
        monkeypatch.setenv("KFX_DEV", "1")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None  # Load all modules

    def test_dev_mode_enabled_with_true(self, monkeypatch):
        """Test dev mode enabled with KFX_DEV=true."""
        monkeypatch.setenv("KFX_DEV", "true")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None

    def test_dev_mode_enabled_with_yes(self, monkeypatch):
        """Test dev mode enabled with KFX_DEV=yes."""
        monkeypatch.setenv("KFX_DEV", "yes")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None

    def test_dev_mode_disabled_with_0(self, monkeypatch):
        """Test dev mode disabled with KFX_DEV=0."""
        monkeypatch.setenv("KFX_DEV", "0")
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None

    def test_dev_mode_disabled_with_false(self, monkeypatch):
        """Test dev mode disabled with KFX_DEV=false."""
        monkeypatch.setenv("KFX_DEV", "false")
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None
    ...
    def test_dev_mode_single_module(self, monkeypatch):
        ...
        monkeypatch.setenv("KFX_DEV", "mistral")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
    ...
    def test_dev_mode_multiple_modules(self, monkeypatch):
        ...
        monkeypatch.setenv("KFX_DEV", "mistral,openai,anthropic")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
    ...
    def test_dev_mode_modules_case_normalized(self, monkeypatch):
        ...
        monkeypatch.setenv("KFX_DEV", "Mistral,OpenAI")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
...
class TestReadComponentIndex:
    ...
    def test_read_index_file_not_found(self):
        ...
        with patch("kfx.interface.components.Path") as mock_path_class:
    ...
    def test_read_index_valid(self, tmp_path):
        ...
        with (
            patch("kfx.interface.components.inspect.getfile") as mock_getfile,
            patch("importlib.metadata.version") as mock_version,
        ):
            mock_getfile.return_value = str(tmp_path / "kfx" / "__init__.py")
            mock_version.return_value = "0.1.12"

            # Create the directory structure
            (tmp_path / "kfx" / "_assets").mkdir(parents=True)
            (tmp_path / "kfx" / "_assets" / "component_index.json").write_bytes(
    ...
    def test_read_index_invalid_sha256(self, tmp_path):
        ...
        with (
            ...
            (tmp_path / "kfx" / "_assets").mkdir(parents=True)
            (tmp_path / "kfx" / "_assets" / "component_index.json").write_bytes(orjson.dumps(index))
    ...
    def test_read_index_package_not_found(self, tmp_path):
        """Test reading index when kfx package metadata is unavailable (e.g. Docker workspace install)."""
        ...
        with (
            ...
            mock_getfile.return_value = str(tmp_path / "kfx" / "__init__.py")
            mock_version.side_effect = PackageNotFoundError("kfx")
...
class TestSaveGeneratedIndex:
    ...
    def test_save_generated_index(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kfx.interface.components._get_cache_path", lambda: cache_file)
...
@pytest.mark.asyncio
class TestImportKetosComponents:
    """Tests for import_ketos_components() async function."""

    async def test_import_with_dev_mode(self, monkeypatch):
        """Test import in dev mode (dynamic loading)."""
        monkeypatch.setenv("KFX_DEV", "1")

        with patch("kfx.interface.components._process_single_module") as mock_process:
            mock_process.return_value = ("category1", {"comp1": {"template": {}}})

            with (
                patch("kfx.interface.components.pkgutil.walk_packages") as mock_walk,
                patch("kfx.interface.components._save_generated_index") as mock_save,
            ):
                mock_walk.return_value = [
                    (None, "kfx.components.category1", False),
                ]

                result = await import_ketos_components()
    ...
    async def test_import_with_builtin_index(self, monkeypatch):
        ...
        with (
            patch("kfx.interface.components._read_component_index") as mock_read,
            patch("importlib.metadata.version", return_value="0.1.12"),
        ):
    ...
    async def test_import_with_missing_index_creates_cache(self, tmp_path, monkeypatch):
        ...
        with (
            patch("kfx.interface.components._read_component_index") as mock_read,
            patch("kfx.interface.components._process_single_module") as mock_process,
            patch("kfx.interface.components.pkgutil.walk_packages") as mock_walk,
            patch("importlib.metadata.version", return_value="0.1.12"),
        ):
            ...
            mock_process.return_value = ("category1", {"comp1": {"template": {}}})
            mock_walk.return_value = [(None, "kfx.components.category1", False)]
    ...
    async def test_import_with_custom_path_from_settings(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            result = await import_ketos_components(mock_settings)
    ...
    async def test_import_handles_import_errors(self, monkeypatch):
        ...
        with (
            ...
            mock_walk.return_value = [(None, "kfx.components.broken", False)]
