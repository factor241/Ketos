"""Table class for kfx package - pandas DataFrame subclass for Ketos data structures.

This module provides the Table class (formerly DataFrame) as the base type for tabular data in Ketos.
...
class Table(pandas_DataFrame):
    ...
    This is the base type for Ketos tabular data structures, replacing the legacy DataFrame class.
    DataFrame is maintained as an alias for backwards compatibility.

    This class extends pandas.DataFrame to provide seamless integration between
    Ketos's JSON objects and pandas' powerful data manipulation capabilities.
