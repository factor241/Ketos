"""Integration tests for kfx CLI with Simple Agent flow.

These tests verify that the kfx CLI can properly load and execute the
Simple Agent starter project, addressing the bug where kfx serve/run
...
- kfx requires langchain-core>=0.3.66,<1.0.0
...
import pytest
from kfx.__main__ import app as kfx_app
...
def get_starter_projects_path() -> Path:
    ...
    while current != current.parent:
        starter_path = current / "src" / "backend" / "base" / "ketos" / "initial_setup" / "starter_projects"
...
class TestSimpleAgentFlowLoading:
    ...
    def test_simple_agent_flow_loads_via_cli(self, simple_agent_flow_path: Path):
        """Test that kfx run can load the Simple Agent flow without critical errors."""
        ...
        critical_errors = [
            "No module named 'kfx.components",
            "No module named 'ketos",
    ...
    def test_simple_agent_flow_loads_directly(self, simple_agent_flow_path: Path):
        ...
        from kfx.load import load_flow_from_json
    ...
    def test_simple_agent_flow_json_output(self, simple_agent_flow_path: Path):
        """Test that kfx run produces valid JSON output."""
        result = runner.invoke(
            kfx_app,
...
class TestSimpleAgentServe:
    ...
    def test_serve_help(self):
        ...
        result = runner.invoke(kfx_app, ["serve", "--help"])
    ...
    def test_serve_requires_api_key(self, simple_agent_flow_path: Path, monkeypatch):
        """Test serve requires KETOS_API_KEY."""
        ...
        assert result.exit_code != 0 or "KETOS_API_KEY" in result.output
    ...
    def test_serve_starts_server_no_asyncio_error(self, simple_agent_flow_path: Path):
        """Regression test: kfx serve should not fail with asyncio error.

        This test verifies the fix for the issue where kfx serve failed with:
        ...
        proc = subprocess.Popen(  # noqa: S603
            ...
            text=True,
            env={**os.environ, "KETOS_API_KEY": "test-key-12345"},  # pragma: allowlist secret
        )
            ...
            while time.time() - start_time < timeout:
                ...
                if exit_code is not None:
                    ...
                    if "asyncio.run() cannot be called from a running event loop" in output:
                        pytest.fail(f"Regression: kfx serve failed with asyncio error.\nOutput:\n{output}")
...
class TestAllStarterProjectsLoad:
    """Test that all starter projects can load without kfx-specific module errors."""
    ...
    def test_all_projects_load(self, starter_projects_path: Path):
        """Test all starter project JSONs can load without kfx-specific errors.

        Note: This test only fails on kfx-specific module errors (kfx.components.*),
        ...
        for json_file in json_files:
            try:
                ...
                error_str = str(e)
                # Only track kfx-specific errors, not external dependency errors
                if "No module named 'kfx." in error_str:
                    kfx_module_errors.append((json_file.name, str(e)))
                elif "resolve_component_path" in error_str:
                    kfx_module_errors.append((json_file.name, f"Storage error: {e}"))
                # External dependency errors (langchain_anthropic, etc.) are acceptable
                # as kfx is designed to work with minimal dependencies

        if kfx_module_errors:
            error_details = "\n".join([f"  {name}: {error}" for name, error in kfx_module_errors])
            pytest.fail(f"KFX module errors in starter projects:\n{error_details}")
