...
Both ``kfx run`` and ``kfx serve`` accept ``--upgrade-flow`` with identical semantics:
...
from kfx.upgrade.checker import build_registry_lookup, check_flow_compatibility
...
def _load_bundled_registry() -> dict[str, Any]:
    ...
    This is the same source ``kfx upgrade`` reads, and the only registry that is reliably
    ...
    """
    from kfx.interface.components import _read_component_index
    ...
    if not blob or "entries" not in blob:
        msg = (
            ...
            "compatibility cannot be checked. Try reinstalling kfx "
            "(e.g. `pip install --force-reinstall kfx`)."
        )
...
def apply_upgrade_gate(
    ...
    Args:
        ...
        all_types_dict: The component registry to check against. When ``None`` (the default the
            ...
            source ``kfx upgrade`` reads. Tests pass an explicit registry to control classification.
