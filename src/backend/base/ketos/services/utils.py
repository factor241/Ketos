...
from ketos.services.database.utils import initialize_database
...
if TYPE_CHECKING:
    from kfx.services.settings.manager import SettingsService
...
async def setup_superuser(settings_service: SettingsService, session: AsyncSession) -> SetupSuperuserResult:
    if settings_service.auth_settings.AUTO_LOGIN:
        ...
        if configured_password == LEGACY_DEFAULT_SUPERUSER_PASSWORD.get_secret_value():
            await logger.awarning(
                "Ignoring legacy default KETOS_SUPERUSER_PASSWORD in AUTO_LOGIN mode; "
        ...
        try:
            with lock:
                ...
                if super_user:  # Only initialize if user was created
                    ...
                    # Recover MCP server config files orphaned when DB was reset but KETOS_CONFIG_DIR was kept.
    ...
    if password == LEGACY_DEFAULT_SUPERUSER_PASSWORD.get_secret_value():
        msg = "KETOS_SUPERUSER_PASSWORD cannot use the legacy default password"
        await logger.aerror(msg)
        raise ValueError(msg)
...
async def migrate_orphaned_mcp_servers_config(
    ...
    via a ``File`` row. When Ketos starts with a fresh database but the same
...
def register_all_service_factories() -> None:
    ...
    service_manager = get_service_manager()
    from kfx.services.mcp_composer import factory as mcp_composer_factory
    from kfx.services.settings import factory as settings_factory

    from ketos.services.auth import factory as auth_factory
    from ketos.services.auth.service import AuthService
    from ketos.services.authorization import factory as authorization_factory
    from ketos.services.authorization.service import KetosAuthorizationService
    from ketos.services.cache import factory as cache_factory
    from ketos.services.chat import factory as chat_factory
    from ketos.services.database import factory as database_factory
    from ketos.services.job_queue import factory as job_queue_factory
    from ketos.services.session import factory as session_factory
    from ketos.services.shared_component_cache import factory as shared_component_cache_factory
    from ketos.services.state import factory as state_factory
    from ketos.services.storage import factory as storage_factory
    from ketos.services.store import factory as store_factory
    from ketos.services.task import factory as task_factory
    from ketos.services.telemetry import factory as telemetry_factory
    from ketos.services.telemetry_writer import factory as telemetry_writer_factory
    from ketos.services.tracing import factory as tracing_factory
    from ketos.services.transaction import factory as transaction_factory
    from ketos.services.variable import factory as variable_factory
    ...
    # Override KFX's no-op auth service with Ketos's full JWT implementation
    ...
    # ``override=True`` so Ketos always has a default. A registered
    # authorization plugin replaces it by listing its class in
    # ``KETOS_CONFIG_DIR/kfx.toml`` (config files use ``override=True`` via
...
def register_builtin_adapters() -> None:
    ...
    try:
        import_module("ketos.services.adapters.deployment.watsonx_orchestrate.register")
...
def register_builtin_deployment_mappers() -> None:
    ...
    try:
        import_module("ketos.api.v1.mappers.deployments.watsonx_orchestrate")
    except ModuleNotFoundError as exc:
