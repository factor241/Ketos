...
from ketos.services.database.utils import (
...
class DeploymentProviderAccount(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # by a Ketos alembic migration.  The enum is Ketos-owned (see
    # ``DeploymentProviderKey`` above); KFX uses plain strings for adapter
    ...
    #   2. Register the corresponding adapter in KFX and mapper in Ketos.
    #   3. Create a Ketos alembic migration that runs:
    #        op.execute(
