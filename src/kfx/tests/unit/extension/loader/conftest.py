...
piece -- it strips the loader's ``_kfx_ext.<slot>...`` modules from
...
def component_source(class_name: str = "PilotThing", *, with_build: bool = True) -> str:
    ...
    subclass check fires without importing the real kfx Component (which
...
def make_installed_pyproject_extension(parent: Path, distribution_name: str) -> FakeDist:
    ...
    ``[tool.ketos.extension]`` discovery path. ``files`` points at a
...
def make_installed_pyproject_no_extension(parent: Path, distribution_name: str) -> FakeDist:
    """Fake distribution shipping pyproject.toml WITHOUT [tool.ketos.extension].
...
def make_installed_pyproject_malformed_extension(
    ...
    """Fake distribution shipping pyproject with [tool.ketos.extension] missing required fields.
...
[tool.ketos.extension]
# Intentionally missing id/version/kfx/bundles -- pydantic must reject.
...
def _scrub_synthetic_modules() -> Iterable[None]:
    ...
    The loader installs each bundle module under ``_kfx_ext.<slot>.<bundle>...``
    ...
    """
    yield
    for name in [m for m in sys.modules if m.startswith("_kfx_ext.")]:
