...
4. Правка `packages/client/ui-board/src/client/canvas/DashboardCanvas.tsx` также пересобирает `packages/client/ui-board/lib/client.js`; доска подключена к ростору, поэтому новая ревизия видна в браузере с открытой панелью «Доска» — без ручной пересборки, при необходимости после `reload`.
