...
def register(app: typer.Typer) -> None:
    ...
    def create_command_wrapper(
        ...
        *,
        ...
        """Scaffold a new Ketos flow JSON from a built-in template (lazy-loaded)."""
        from pathlib import Path

        from kfx.cli.create import create_command, print_templates
    ...
    def requirements_command_wrapper(
        flow_path: str = typer.Argument(help="Path to the Ketos flow JSON file"),
        ...
        kfx_package: str = typer.Option(
            "kfx",
            "--kfx-package",
            help="Name of the KFX package (default: kfx)",
        ),
        *,
        no_kfx: bool = typer.Option(
            False,
            "--no-kfx",
            help="Exclude the KFX package from output",
        ),
        ...
        """Generate requirements.txt from a Ketos flow JSON (lazy-loaded)."""
        import json
        from pathlib import Path

        from kfx.utils.flow_requirements import generate_requirements_txt
    ...
    def validate_command_wrapper(
        flow_paths: list[str] = typer.Argument(
            default=None,
            help="Path(s) to Ketos flow JSON file(s) or directories to validate. Defaults to flows/.",
        ),
        ...
        """Validate Ketos flow JSON files without executing them (lazy-loaded)."""
        from kfx.cli.validate import validate_command
