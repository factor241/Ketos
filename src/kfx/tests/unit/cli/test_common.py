"""Unit tests for KFX CLI common utilities."""
...
class TestApiKey:
    ...
    def test_get_api_key_not_set(self):
        ...
        with (
            ...
            pytest.raises(ValueError, match="KETOS_API_KEY environment variable is not set"),
        ):
            get_api_key()
    ...
    def test_get_api_key_empty_string(self):
        ...
        with (
            patch.dict(os.environ, {"KETOS_API_KEY": ""}),
            pytest.raises(ValueError, match="KETOS_API_KEY environment variable is not set"),
        ):
            get_api_key()
...
class TestLoadGraph:
    ...
    async def test_load_graph_from_path_success(self):
        ...
        with patch("kfx.cli.common.load_flow_from_json", return_value=mock_graph) as mock_load_flow:
            verbose_print = Mock()
    ...
    async def test_load_graph_from_path_failure(self):
        ...
        with patch("kfx.cli.common.load_flow_from_json", side_effect=Exception("Load error")) as mock_load_flow:
            verbose_print = Mock()
            path = Path("/test/flow.json")
...
class TestGraphExecution:
    ...
    async def test_execute_graph_with_capture_autogenerates_session_id(self):
        ...
        to keep streaming/persistence paths functional in kfx serve.
        """
    ...
    async def test_execute_graph_propagates_session_id_to_vertices(self):
        """Session_id must reach Memory/MessageHistory inputs on the kfx serve path.
    ...
    async def test_execute_graph_does_not_overwrite_hardcoded_session_id(self):
        ...
        Mirrors Ketos's playground precedence in ``build_graph_from_data``.
        """
    ...
    async def test_execute_graph_autogenerates_user_id_when_unset(self):
        """When the graph arrives without a user_id (typical for kfx serve), assign a UUID.
    ...
    async def test_execute_graph_passes_fallback_from_settings_default(self):
        ...
        has no DB row — matching ketos's API path behavior.
        """
        captured: dict = {}
    ...
    async def test_execute_graph_respects_disabled_fallback_setting(self):
        ...
        with patch("kfx.run._defaults.get_settings_service", return_value=mock_settings):
            await execute_graph_with_capture(mock_graph, "test input")
