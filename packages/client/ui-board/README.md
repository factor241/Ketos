...
- **The chats panel lists chats, it does not manage them** — picking a chat rebinds the window (which makes that session current, like focusing a window does) and the plus creates one in the shown project; renaming, archiving, forking, search, grouping and sorting menus, and workspace management stay in the app's own sidebar, and the client exposes no session delete at all.
- **A rebound window keeps its own title** — the header shows the name the window was opened with, not the chat's title, which the panel and the composer's directory chip carry.
