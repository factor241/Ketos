...
Tests that all starter project JSON files can be loaded by kfx run command
without import errors for ketos modules. We expect execution errors
...
def get_starter_projects_path() -> Path:
    ...
    # Navigate up to find the ketos project root
    current = test_file_path.parent
    while current != current.parent:
        if (current / "src" / "backend" / "base" / "ketos" / "initial_setup" / "starter_projects").exists():
            return current / "src" / "backend" / "base" / "ketos" / "initial_setup" / "starter_projects"
        current = current.parent

    # Fallback to a relative path from the test file
    # test_file is in: src/kfx/tests/unit/cli
    # starter projects are in: src/backend/base/ketos/initial_setup/starter_projects
    project_root = test_file_path.parent.parent.parent.parent.parent
    return project_root / "backend" / "base" / "ketos" / "initial_setup" / "starter_projects"
...
class TestRunStarterProjects:
    ...
    def test_run_starter_project_no_import_errors(self, template_file):
        """Test that starter project can be loaded without ketos or kfx import errors.

        We expect execution errors (missing API keys, missing inputs, etc.)
        but there should be NO errors about importing ketos or kfx modules.
        """
        ...
        # Check for import errors related to ketos or kfx
        if "ModuleNotFoundError" in all_output or "ImportError" in all_output or "Module" in all_output:
            # Check for ketos import errors
            if "No module named 'ketos'" in all_output or "Module ketos" in all_output:
                # Extract the specific error for better debugging
                error_line = ""
                for line in all_output.split("\n"):
                    if "ketos" in line and ("No module named" in line or "Module" in line):
                        error_line = line.strip()
                        break
                pytest.fail(f"Ketos import error found in {template_file.name}.\nError: {error_line}")

            # Check for kfx import errors (these indicate structural issues)
            if "No module named 'kfx." in all_output or "Module kfx." in all_output:
                # Extract the specific error for better debugging
                import re
                ...
                for line in clean_output.split("\n"):
                    if "kfx" in line and ("No module named" in line or "Module kfx." in line):
                        # Extract just the module name from various error formats
                        if "No module named" in line:
                            match = re.search(r"No module named ['\"]([^'\"]+)['\"]", line)
                            if match:
                                error_lines.append(f"  - Missing module: {match.group(1)}")
                        elif "Module kfx." in line and "not found" in line:
                            match = re.search(r"Module (kfx\.[^\s]+)", line)
                            if match:
                                error_lines.append(f"  - Missing module: {match.group(1)}")
                ...
                error_detail = "\n".join(unique_errors[:5])  # Show first 5 unique kfx errors
                pytest.fail(
                    f"KFX import error found in {template_file.name}.\n"
                    f"This indicates kfx internal structure issues.\n"
                    f"Missing modules:\n{error_detail}"
                )

            # Check for other critical import errors
            if "cannot import name" in all_output and ("ketos" in all_output or "kfx" in all_output):
    ...
    def test_run_basic_starter_projects_detailed(self):
        ...
        for template_name in basic_templates:
            ...
            assert "No module named 'ketos'" not in all_output, f"Ketos import error in {template_name}"

            # Check for module not found errors specifically related to ketos
            # (Settings service errors are runtime errors, not import errors)
            if "ModuleNotFoundError" in all_output and "ketos" in all_output and "kfx.services" not in all_output:
                # This is an actual ketos import error, not an internal kfx error
                pytest.fail(f"Module not found error for ketos in {template_name}")
    ...
    def test_run_starter_project_with_stdin(self, template_file):
        ...
        all_output = result.output
        assert "No module named 'ketos'" not in all_output
