"""Production-install discovery for the Ketos Extension System.
...
This module owns the two production install sources for Modes A, B, and C:
    ...
    2. **Seed directories** -- an optional filesystem source for Docker
       ...
       Defaults to ``/opt/ketos/bundles/`` when present; the
       ``KETOS_SEED_DIR`` environment variable overrides this with one
...
never touches the registry.  The :mod:`kfx.extension.registry` service is
...
Errors are surfaced as :class:`~kfx.extension.errors.ExtensionError`
...
DEFAULT_SEED_DIR: Path = Path("/opt/ketos/bundles")
"""Default filesystem seed directory used when ``KETOS_SEED_DIR`` is unset.
...
SEED_DIR_ENV_VAR: str = "KETOS_SEED_DIR"
...
``;`` on Windows), mirroring ``KETOS_COMPONENTS_PATH``."""
...
class DiscoveredExtension:
    ...
    def slot(self) -> Literal["official"]:
        ...
        ``KETOS_COMPONENTS_PATH`` folders; nothing in this module
        produces it.
        """
        return "official"
...
def _distribution_manifest_path_via_entry_points(
    ...
    ``ketos.extensions`` entry-point points at the package that ships the
    ...
    try:
        selected = list(eps.select(group="ketos.extensions"))
    except AttributeError:
        # Older importlib.metadata returns a plain tuple of EntryPoint with
        # no .select() helper; filter manually.
        selected = [ep for ep in eps if getattr(ep, "group", None) == "ketos.extensions"]
...
def _pyproject_declares_extension(pyproject_path: Path) -> bool:
    """Return ``True`` iff *pyproject_path* contains ``[tool.ketos.extension]``.
    ...
    try:
        ...
        # Unparseable TOML: not specifically a Ketos concern, treat as
...
def _build_installed_record(
    ...
    try:
        ...
        return None, ExtensionError(
            ...
            location=str(manifest_path),
            content=canonical,
            hint="Run `kfx extension validate` against the package source to see the per-field detail.",
        )
...
def discover_installed_extensions(
    ...
    Returns:
        ...
        :class:`~kfx.extension.errors.ExtensionError` per package whose
    ...
    :class:`~kfx.extension.registry.ExtensionRegistry`.
    """
    ...
    *,
    ...
    Behavior:
        ...
        * If ``seed_dir_env`` is unset/empty, fall back to ``default``.
          A missing default path is silently skipped: ``/opt/ketos/
            ...
            if not candidate.is_dir():
                errors.append(
                    ExtensionError(
                        ...
                        content=stripped,
                        hint=(
                            "Set KETOS_SEED_DIR to a directory that exists, "
                            "or unset it to fall back to /opt/ketos/bundles."
                        ),
                    )
                )
                continue
...
def _iter_seed_subdirectories(seed_root: Path) -> Iterator[Path]:
    ...
    (see kfx.extension._paths.is_within and loader._discovery.iter_bundle_python_files).
...
def _build_seed_record(seed_subdir: Path) -> tuple[DiscoveredExtension | None, ExtensionError | None]:
    ...
    try:
        ...
        return None, ExtensionError(
            ...
            content=seed_subdir.name,
            hint="Ship an extension.json or a pyproject.toml with [tool.ketos.extension].",
        )
    except (ValueError, TypeError, OSError) as exc:
        return None, ExtensionError(
            code="manifest-invalid",
            ...
            content=seed_subdir.name,
            hint="Run `kfx extension validate` against the seed subdirectory to see per-field detail.",
        )
...
def discover_seed_extensions(
    *,
    ...
    Args:
        seed_dir_env: Explicit override for ``$KETOS_SEED_DIR``.  Pass
        ...
        default: Path checked when ``seed_dir_env`` is empty.  Defaults to
            :data:`DEFAULT_SEED_DIR` (``/opt/ketos/bundles``).  Pass
