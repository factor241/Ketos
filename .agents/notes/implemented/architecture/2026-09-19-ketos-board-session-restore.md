...
**The bindings map is the second half of the stored `ui-board` section.** `BoardSettings` (schema version 1) carries `bindings: Record<windowId, sessionId>` beside the layout document. The layout capture still omits the map — `captureBoardLayout` captures the store and knows nothing about sessions — and `BoardLayoutPersistence` is the single writer that sends the complete section (captured layout plus the bridge's live map), so a layout gesture and a session change cannot race each other through two revision-CAS writers. That write is `remote.settings.replace`, not `update`: the settings service deep-merges an update patch, so only a wholesale section write can drop a pair whose window closed. `writeBindings` replaces the map and schedules the same debounced, min-interval, conflict-retrying write the layout uses, and the first-frame cache round-trips both halves.
