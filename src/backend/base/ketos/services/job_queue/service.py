...
_STREAM_PREFIX = "ketos:queue:"
_OWNER_PREFIX = "ketos:owner:"
...
_CANCEL_CHANNEL_PREFIX = "ketos:cancel:"
...
_ACTIVITY_PREFIX = "ketos:activity:"
...
_PUBLIC_JOB_PREFIX = "ketos:public_job:"
...
class RedisQueueWrapper:
    ...
    * End-of-stream →  ``XADD key * event_id __sentinel__ data __sentinel__ ts <float>``
    ...
    # value can be driven from settings (KETOS_REDIS_QUEUE_STARTUP_GRACE_S).
...
class RedisJobQueueService(JobQueueService):
    ...
    Configuration
    -------------
    Set ``KETOS_JOB_QUEUE_TYPE=redis`` and, optionally:

    * ``KETOS_REDIS_QUEUE_DB`` (default ``1``, separate from cache DB ``0``)
    * ``KETOS_REDIS_QUEUE_URL`` (full URL, overrides host/port/db)
    * ``KETOS_REDIS_QUEUE_HOST`` / ``KETOS_REDIS_QUEUE_PORT``
    ...
    ``ketos:cancel:*``. Any worker can publish a cancel signal via
    ...
    completion. See ``ketos.api.build.create_flow_response``.
    """
    ...
    async def _check_connection(self) -> None:
        ...
        try:
            ...
            await logger.aerror(
                ...
                "Set KETOS_JOB_QUEUE_TYPE=asyncio or start Redis before running Ketos."
            )
    ...
    _CANCEL_MARKER_PREFIX = "ketos:cancel-marker:"
    ...
    def get_queue_data(self, job_id: str) -> tuple[asyncio.Queue, EventManager, asyncio.Task | None, float | None]:  # type: ignore[override]
        ...
        * **Cross-worker path**: no local entry exists; a null ``EventManager`` and
          ...
          response's :func:`ketos.api.build.create_flow_response.on_disconnect`
