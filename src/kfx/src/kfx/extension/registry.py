...
Where :mod:`kfx.extension.discovery` *finds* manifest-shipping packages
and seed directories, this module *registers* them: it owns the in-memory
state that ``kfx extension list`` (and, downstream, the loader and the
...
milestone.  Two source kinds reach the registry:
    ...
    * **seed** -- an immediate subdirectory of a seed root (default
      ``/opt/ketos/bundles``, overridable via ``$KETOS_SEED_DIR``).
...
:class:`~kfx.extension.errors.ExtensionError` with code
...
if TYPE_CHECKING:
    ...
    from kfx.extension.discovery import DiscoveredExtension, SourceKind
...
class LoadStatus(str, Enum):
    """Lifecycle state surfaced through ``kfx extension list``.
...
class ExtensionImmutableError(RuntimeError):
    ...
    Wraps a typed :class:`~kfx.extension.errors.ExtensionError` so HTTP
...
class ExtensionRegistry:
    ...
    def list_extensions(self) -> list[Extension]:
        ...
        ``kfx extension list`` produces stable output even when the
    ...
    def _refuse_mutation(self, extension_id: str, *, verb: str) -> None:
        ...
        if extension.source_kind == "installed":
            ...
            hint = (
                ...
                "(remove the package from the image / lockfile) and restart Ketos. "
            ...
            hint = (
                ...
                "restart Ketos. Runtime mutation of seed Extensions is intentionally not supported."
            )
