"""FastMCP server exposing Ketos operations as MCP tools.

Connects to a running Ketos server via REST API. Flow data is never
...
from kfx.graph.flow_builder import (
...
from kfx.graph.flow_builder.spec import validate_spec_references
...
mcp = FastMCP(
    "ketos-mcp",
    instructions=(
        "Ketos MCP server -- build and run AI flows on a Ketos instance.\n"
        "\n"
        "Typical workflow:\n"
        "  1. login (or set KETOS_API_KEY env var)\n"
...
def _get_client() -> KetosClient:
    ...
    if _shared_client is None:
        _shared_client = KetosClient()
    return _shared_client


def _set_client(client: KetosClient) -> None:
...
async def login(username: str, password: str, server_url: str | None = None) -> dict[str, str]:
    """Authenticate with a Ketos server. Call this first.

    Credentials are stored and reused for all subsequent calls.

    Args:
        username: Ketos username.
        password: Ketos password.
        server_url: Server URL (defaults to KETOS_SERVER_URL env var or http://localhost:7860).
    """
    ...
    client = KetosClient(server_url=server_url)
    _set_client(client)
...
async def get_component_info(
    ...
    from kfx.mcp.redact import is_sensitive_field
...
async def export_flow(flow_id: str) -> dict[str, Any]:
    ...
    Args:
        flow_id: The flow UUID.
    """
    from kfx.mcp.redact import redact_node
