...
from ketos.services.cache.base import (
...
class RedisCache(ExternalAsyncBaseCacheService, Generic[LockType]):
    ...
    def _integrity_tag(self, namespaced_key: str, payload: bytes) -> bytes:
        ...
        letting anyone with write access to the ``ketos:cache:`` namespace
    ...
    async def get(self, key, lock=None):
        ...
        # port, or anyone able to write under the ketos:cache: namespace could
