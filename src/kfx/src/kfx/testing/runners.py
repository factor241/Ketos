...
from kfx.testing.result import FlowResult, _build_result, _build_result_from_sdk_response
...
async def _run_async(
    *,
    ...
    from kfx.run.base import RunError, run_flow
...
def _import_remote_run_request():
    ...
    try:
        from ketos_sdk.models import RunRequest  # type: ignore[import-untyped]
    except ImportError as exc:
        msg = "ketos-sdk is required for remote flow testing. Install: pip install ketos-sdk"
...
class LocalFlowRunner(_BaseLocalFlowRunner):
    ...
    * A ``dict`` (already-parsed flow JSON).
    ...
    Relative paths are resolved against ``--kfx-flow-dir`` (default: ``cwd``).
    """
...
# ---------------------------------------------------------------------------
# Remote runners (requires ketos-sdk)
# ---------------------------------------------------------------------------


class RemoteFlowRunner(_BaseRemoteFlowRunner):
    """Sync callable that runs flows against a live Ketos instance.

    Returned by :func:`flow_runner` when ``--ketos-env`` or
    ``--ketos-url`` is passed to pytest.  Call it like a function::
...
class AsyncRemoteFlowRunner(_BaseRemoteFlowRunner):
    """Async callable that runs flows against a live Ketos instance.

    Returned by :func:`async_flow_runner` when ``--ketos-env`` or
    ``--ketos-url`` is passed to pytest.  Use with ``await``::
