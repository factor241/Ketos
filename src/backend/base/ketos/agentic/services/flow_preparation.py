...
from kfx.base.models.model_metadata import MODEL_PROVIDER_METADATA, get_provider_param_mapping

import kfx
from ketos.agentic.helpers.assistant_workspace import resolve_assistant_fs_root

# Resolves only from the monorepo root; inject_kfx_components_path rewrites it to
# an absolute path at runtime so packaged installs (Desktop, pip, Docker) work.
KFX_COMPONENTS_PATH_SENTINEL = "./src/kfx/src/kfx/components/"
...
def inject_kfx_components_path(flow_data: dict) -> dict:
    """Rewrite Directory nodes targeting bundled kfx components to an absolute path.

    The bundled KetosAssistant flow hardcodes a relative path that only
    ...
    be a directory.", causing the Ketos Assistant to fail with
    ...
    `path` value equals KFX_COMPONENTS_PATH_SENTINEL, replaces it with the
    absolute path derived from the installed kfx package.
    """
    absolute_path = str(Path(kfx.__file__).parent / "components")
    ...
    for node in flow_data.get("data", {}).get("nodes", []):
        ...
        if path_field and path_field.get("value") == KFX_COMPONENTS_PATH_SENTINEL:
...
def load_and_prepare_flow(
    ...
    flow_data = inject_kfx_components_path(flow_data)
