"""Flow persistence backends for kfx serve.
...
- ``FilesystemFlowStore(directory)``: writes each flow as ``{id}.json``.
  Point at ``/tmp/kfx-flows`` for single-pod sharing across uvicorn workers,
...
class FilesystemFlowStore:
    ...
    Use ``/tmp/kfx-flows`` for single-pod worker sharing or a PVC mount path
