...
`tests/apply.client.spec.tsx` pins the registration on the production `SlotTestRuntime`: both occupants with their metadata (`order` 15, declared `locale`, a store handle, no `children`), the panel-sized canvas box and the icon at the owner-supplied size, the row label resolving `Board`/`看板` through the package dictionary, disposal removing the entries and their DOM while the frame-owned declarations survive, and the deferred path — the board applies before the slots are declared and registers when they arrive.
