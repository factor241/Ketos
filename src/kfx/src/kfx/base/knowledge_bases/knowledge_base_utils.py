...
async def get_knowledge_bases(kb_root: Path, user_id: UUID | str) -> list[str]:
    ...
    # Lazy imports: ketos's DB models aren't part of the kfx
    # standalone install, and kfx's validate-rewrite layer can't
    # substitute ``kfx.services.database.models.user.crud`` (no such
    # module). Deferring the imports to call time keeps this module
    # importable under ``kfx run <starter>.json``, which is exercised
    ...
    # sentinel check: kfx must stay importable without the ketos API
    # package, so we cannot reach for KBStorageHelper. The string literal
    # is kept in lockstep with KB_DELETED_SENTINEL in
    # src/backend/base/ketos/api/utils/kb_helpers.py via a unit test
