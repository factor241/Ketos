...
execution traces directly in Ketos's database, enabling the Trace View
...
from ketos.serialization.serialization import serialize
...
from ketos.services.tracing.span_sorting import (
    KETOS_SPAN_NAMESPACE,
    resolve_span_uuids,
...
class NativeTracer(BaseTracer):
    """Tracer that stores execution traces in Ketos's database.

    This tracer captures:
    - Component-level traces (via add_trace/end_trace)
    - LangChain-level traces (via get_langchain_callback)

    Enabled by default. Disable with KETOS_NATIVE_TRACING=false if needed.
    """
    ...
    async def _flush_to_database(self, error: Exception | None = None) -> None:
        ...
        try:
            ...
            try:
                ...
                flow_uuid = uuid5(KETOS_SPAN_NAMESPACE, f"invalid-flow-id:{self.flow_id}")
                logger.error(
            ...
            from ketos.services.tracing.formatting import safe_int_tokens
    ...
    def get_langchain_callback(self) -> BaseCallbackHandler | None:
        ...
        from ketos.services.tracing.service import component_context_var
        ...
        if component_context:
            ...
            parent_span_id = uuid5(KETOS_SPAN_NAMESPACE, f"{self.trace_id}-{component_id}")
        elif self._current_component_id:
            # Fallback for edge cases where component context might not be set
            parent_span_id = uuid5(KETOS_SPAN_NAMESPACE, f"{self.trace_id}-{self._current_component_id}")
    ...
    def end_langchain_span(
        ...
        # Default to chat since most LLM usage in Ketos is conversational
    ...
    def _map_trace_type(trace_type: str) -> SpanType:
        """Normalise Ketos's string trace types to the SpanType enum, defaulting to CHAIN for unknown values."""
