...
``pip install`` three local bundles and verify ``kfx extension list``
...
def test_pip_install_three_bundles_visible_at_official(tmp_path: Path) -> None:
    ...
    project_specs = [
        ("kfx-test-openai", "kfx_test_openai", "kfx-test-openai", "openai", "1.0.0"),
        ("kfx-test-anthropic", "kfx_test_anthropic", "kfx-test-anthropic", "anthropic", "0.4.1"),
        ("kfx-test-qdrant", "kfx_test_qdrant", "kfx-test-qdrant", "qdrant", "2.5.0"),
    ]
    ...
    assert ids == ["kfx-test-anthropic", "kfx-test-openai", "kfx-test-qdrant"]
...
def test_pip_install_seed_directory_three_bundles(tmp_path: Path) -> None:
    ...
    for bundle in ("openai", "anthropic", "qdrant"):
        sub = seed / f"kfx_test_{bundle}"
        sub.mkdir()
        (sub / bundle).mkdir()
        (sub / "extension.json").write_text(
            _EXTENSION_JSON_TEMPLATE.format(
                extension_id=f"kfx-test-{bundle}",
                version="1.0.0",
                name=f"kfx-test-{bundle}",
                bundle_name=bundle,
            ),
            encoding="utf-8",
        )

    from kfx.extension import discover_seed_extensions
