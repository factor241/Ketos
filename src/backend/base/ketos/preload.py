"""Gunicorn master preload hook for Ketos.

When ``KETOS_GUNICORN_PRELOAD=true`` (which flips gunicorn's
``preload_app`` option), this module runs the fork-safe parts of
Ketos's startup in the master process so that worker processes
inherit the result via copy-on-write. The dominant memory wins are:

- Python modules imported from custom-component bundles.
- The component types dict (``kfx.interface.components.component_cache``),
...
async def _run_master_preload() -> None:
    ...
    from ketos.main import load_bundles_with_error_handling
    ...
    try:
        ...
        if settings_service.settings.agentic_experience:
            from ketos.api.utils.mcp.agentic_mcp import (
...
def preload_master() -> None:
    """Run one-time Ketos initialization in the gunicorn master before workers are forked.
