"""Headless Ketos Assistant runner for MCP clients.
...
from kfx.mcp.flow_builder_tools import get_working_flow

from ketos.agentic.api.router import _resolve_assistant_context
...
async def _ensure_flow(session: AsyncSession, user_id: UUID, flow_id: str | None) -> tuple[Flow, bool]:
    ...
    new_flow = FlowCreate(
        ...
        description="Created by the Ketos Assistant via MCP",
        data={"nodes": [], "edges": []},
        folder_id=folder.id,
        user_id=user_id,
    )
