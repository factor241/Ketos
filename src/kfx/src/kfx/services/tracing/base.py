...
class BaseTracingService(Service, ABC):
    ...
    must provide, whether minimal (KFX) or full-featured (Ketos).
    """
    ...
    async def start_tracers(
        ...
        Args:
            ...
            user_id: Authenticated Ketos user identifier (optional)
            ...
            tracing_user_id: Optional caller-supplied label forwarded to tracing
                ...
                LangFuseTracer stamps it under ``ketos.tracing_user_id`` in
                trace metadata).
        """
