...
from kfx.utils.flow_requirements import generate_requirements_from_flow
...
from ketos.services.adapters.deployment.watsonx_orchestrate.utils import (
    dedupe_list,
...
def extract_ketos_artifact_from_zip(artifact_zip_bytes: bytes, *, snapshot_id: str) -> dict[str, Any]:
    """Read and parse the Ketos flow JSON from a wxO snapshot artifact zip."""
    try:
...
def build_ketos_artifact_bytes(
    *,
    ...
    kfx_requirement = _resolve_kfx_requirement()
    requirements = generate_requirements_from_flow(
        flow_definition,
        include_kfx=False,
        pin_versions=True,
    )
    requirements = [kfx_requirement, *requirements]
    requirements = dedupe_list(requirements)
    requirements_content = "\n".join(requirements) + "\n"
    logger.debug("build_ketos_artifact_bytes: filename='%s', requirements=%s", filename, requirements)
    ...
    *,
    ...
    if not flow_definition.get("last_tested_version"):
        ...
        if not detected_version:
            msg = "Unable to determine running Ketos version for snapshot creation."
            raise InvalidContentError(message=msg)
...
async def process_raw_flows_with_app_id(
    ...
    """Create ketos tools in wxO and connect them to the given app_id."""
...
def _resolve_kfx_requirement() -> str:
    """Pin kfx to the installed version, falling back to a minimum spec.

    If the ``WXO_KFX_REQUIREMENT_OVERRIDE`` environment variable is set, its
    value is used verbatim as the kfx requirement line (e.g.
    ``kfx-nightly==0.4.0.dev32``) instead of resolving from the installed
    package metadata.
    """
    override = os.environ.get("WXO_KFX_REQUIREMENT_OVERRIDE", "").strip()
    if override:
        logger.debug("Using wxO kfx requirement override: %s", override)
        return override
    try:
        return _pin_requirement_name("kfx")
        ...
        message = "Could not determine installed kfx version. Failing deployment."
...
async def verify_tools_by_ids(
    ...
    from kfx.services.adapters.deployment.schema import SnapshotItem, SnapshotListResult
    ...
    for tool in tools or []:
        if not isinstance(tool, dict) or not tool.get("id"):
            continue
        connections = extract_ketos_connections_binding(tool)
