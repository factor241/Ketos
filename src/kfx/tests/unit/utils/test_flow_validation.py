"""Unit tests for KFX flow validation helpers."""
...
async def test_ensure_component_hash_lookups_loaded_surfaces_loader_failures(monkeypatch):
    ...
    monkeypatch.setattr("kfx.services.deps.get_settings_service", lambda: settings_service)
    monkeypatch.setattr(component_cache, "type_to_current_hash", None)

    with (
        patch(
            "kfx.interface.components.get_and_cache_all_types_dict",
...
def test_substitute_trusted_node_code_replaces_known_and_blocks_unknown():
    from kfx.utils import flow_validation as fv
...
async def test_prepare_public_flow_build_substitutes_trusted_code(monkeypatch):
    ...
    monkeypatch.setattr("kfx.services.deps.get_settings_service", lambda: _public_settings())
...
def test_public_flow_blocks_structured_data_analysis_starter_template():
    ...
    starter_path = (
        repo_root / "src/backend/base/ketos/initial_setup/starter_projects/Structured Data Analysis Agent.json"
    )
