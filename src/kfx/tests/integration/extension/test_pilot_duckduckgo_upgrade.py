...
``DuckDuckGoSearchComponent`` from the pre-extraction Ketos:

    1. A saved flow uses the legacy bare class name ``DuckDuckGoSearchComponent``
       (the form Ketos serialized for years before the bundle move).
    ...
    4. The kfx-duckduckgo distribution is importable and ships the manifest
       in a location ``importlib.metadata.files`` can discover.

This covers the *deserialize-side* half of the M1 proof gate plus the
build-pipeline runtime contract: a saved flow from pre-migration Ketos
...
swap: standing up a pre-migration Ketos release, saving a flow,
...
import pytest
from kfx.extension.migration.loader import load_migration_table

REPO_ROOT = Path(__file__).resolve().parents[5]
TABLE_PATH = REPO_ROOT / "src" / "kfx" / "src" / "kfx" / "extension" / "migration" / "migration_table.json"
...
def _saved_flow(*nodes: dict) -> dict:
    """Wrap nodes in the canonical Ketos flow envelope."""
...
def test_legacy_bare_name_flow_upgrades(migration_table) -> None:
    ...
    Ketos used for years before the bundle move) must rewrite to
    ``ext:duckduckgo:DuckDuckGoSearchComponent@official``.
    """
    from kfx.extension.migration.rewrite import migrate_flow_payload
...
def test_legacy_import_path_flow_upgrades(migration_table) -> None:
    ...
    Some Ketos versions serialized the full module path instead of the
...
def test_short_import_path_flow_upgrades(migration_table) -> None:
    ...
    Catches flows that referenced ``kfx.components.duckduckgo.DuckDuckGoSearchComponent``
    via the package-level re-export rather than the file-level dotted path.
    """
    from kfx.extension.migration.rewrite import migrate_flow_payload

    flow = _saved_flow(_saved_flow_node("ddg-3", "kfx.components.duckduckgo.DuckDuckGoSearchComponent"))
...
@pytest.mark.integration
def test_kfx_duckduckgo_distribution_is_importable() -> None:
    ...
    ``ketos.extensions`` references in the entry-point.

    Skipped when the bundle is not installed in the test environment
    (kfx's own venv does not list kfx-duckduckgo as a dep); the ketos
    workspace venv pulls it in transitively from ketos's pyproject.
    """
    try:
        from kfx_duckduckgo import DuckDuckGoSearchComponent
    except ImportError:
        pytest.skip("kfx-duckduckgo not installed in this test environment")
...
def test_pilot_loads_and_resolves_to_runtime_class() -> None:
    ...
    resolves to a Component class that:
        ...
        2. Comes from the bundle's own source file (not a shadow copy
           ...
           Object identity vs. ``from kfx_duckduckgo import ...`` does
           NOT hold -- the loader stages bundle modules under
           ``_kfx_ext.<slot>.<bundle>`` so the registered class is a
    ...
    """
    try:
        import kfx_duckduckgo
    except ImportError:
        pytest.skip("kfx-duckduckgo not installed in this test environment")

    from kfx.extension import SLOT_OFFICIAL, load_extension
    from kfx_duckduckgo import DuckDuckGoSearchComponent as ExpectedClass
    ...
    package_dir = Path(kfx_duckduckgo.__file__).parent
    bundle_root = package_dir

    result = load_extension(bundle_root, slot=SLOT_OFFICIAL, distribution="kfx-duckduckgo")
    ...
    # modules under its own ``_kfx_ext.<slot>.<bundle>`` namespace, so the
    # class object the registry holds is a different ``type`` instance
    # than ``from kfx_duckduckgo import ...`` resolves to -- even though
...
def test_pilot_build_pipeline_runs_against_stub_wrapper() -> None:
    ...
    package_dir = Path(kfx_duckduckgo.__file__).parent
    result = load_extension(package_dir, slot=SLOT_OFFICIAL, distribution="kfx-duckduckgo")
    assert result.ok, [e.code for e in result.errors]
    ...
    # because the loaded class (built under ``_kfx_ext.<slot>.<bundle>``)
...
@pytest.mark.integration
def test_kfx_duckduckgo_ships_manifest() -> None:
    ...
    never registers and ``pip install ketos`` silently fails to pull
    in the pilot bundle.
    ...
    """
    try:
        dist = importlib_metadata.distribution("kfx-duckduckgo")
    ...
    if _is_editable_install(dist):
        ...
        assert manifest_path.is_file(), (
            f"kfx-duckduckgo source tree at {package_dir} does not ship "
        ...
        assert manifests, (
            "kfx-duckduckgo distribution does not ship extension.json in its "
            "wheel; the loader will skip it.  Check pyproject's "
            "[tool.hatch.build.targets.wheel] include rules."
        )
        manifest_path = Path(dist.locate_file(manifests[0]))

    # Round-trip the manifest: it must parse, declare kfx.compat=['1'],
    # and point at a bundle named 'duckduckgo'.
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert manifest["id"] == "kfx-duckduckgo"
