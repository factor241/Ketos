...
from kfx.components.files_and_knowledge._filesystem_isolation import (
    IsolationConfig,
    load_isolation_config,
)
from kfx.components.files_and_knowledge._filesystem_namespace import (
...
def _default_config_dir() -> Path:
    ...
    Operators set ``KETOS_FS_TOOL_BASE_DIR`` / ``KETOS_FS_TOOL_PEPPER_PATH``
...
class FileSystemToolComponent(Component):
    ...
    inputs = [
        StrInput(
            ...
            info=(
                ...
                "BASE_DIR is operator-controlled via KETOS_FS_TOOL_BASE_DIR."
            ),
        ),
        BoolInput(
            name="read_only",
        ...
        # and the backend gate in `src/kfx/src/kfx/template/utils.py` (toggle
    ...
    def _resolve_user_id(self) -> str | None:
        ...
        Why we filter ``"none"`` / ``"null"``: Ketos's ``PlaceholderGraph``
    ...
    def _shared_root(self, *, config: IsolationConfig) -> Path:
        ...
        try:
            ...
            msg = (
                ...
                f"Check that KETOS_FS_TOOL_BASE_DIR ({config.base_dir}) is writable "
                f"by the Ketos process user."
            )
    ...
    def _isolated_user_root(self, *, config: IsolationConfig, user_id: str) -> Path:
        ...
        try:
            ...
            msg = (
                ...
                f"{exc.strerror or exc}. "
                f"Check that KETOS_FS_TOOL_PEPPER_PATH points to a writable location."
            )
            raise PermissionError(msg) from exc
