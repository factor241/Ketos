...
from kfx.services.deps import injectable_session_scope, injectable_session_scope_readonly
from kfx.utils.validate_cloud import raise_error_if_astra_cloud_disable_component
from sqlmodel.ext.asyncio.session import AsyncSession

from ketos.services.auth.utils import get_current_active_user, get_current_active_user_mcp
from ketos.services.database.models.flow.model import Flow
from ketos.services.database.models.user.model import User
from ketos.services.store.utils import get_lf_version_from_pypi
from ketos.utils.constants import KETOS_GLOBAL_VAR_HEADER_PREFIX

if TYPE_CHECKING:
    from ketos.services.store.schema import StoreComponentCreate
...
async def check_ketos_version(component: StoreComponentCreate) -> None:
    ...
    ketos_version = await get_lf_version_from_pypi()
    if ketos_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Ketos")
    if ketos_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of Ketos ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({ketos_version}) and try again."
        )
...
def extract_global_variables_from_headers(headers, *, include_auth_headers: bool = False) -> dict[str, str]:
    ...
    By default, only headers with the ``X-KETOS-GLOBAL-VAR-*`` prefix are
    ...
    ``/workflow``, ``x-api-key`` is Ketos's own authentication key — exposing
    ...
    Example:
        headers = {"X-KETOS-GLOBAL-VAR-API-KEY": "secret", "x-api-key": "mykey"}
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(KETOS_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(KETOS_GLOBAL_VAR_HEADER_PREFIX) :].upper()
