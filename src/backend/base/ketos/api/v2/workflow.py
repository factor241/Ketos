...
from kfx.services.deps import get_settings_service, injectable_session_scope_readonly
from pydantic_core import ValidationError as PydanticValidationError
from sqlalchemy.exc import OperationalError

from ketos.api.utils import extract_global_variables_from_headers
from ketos.api.v1.schemas import RunResponse
...
from ketos.exceptions.api import (
...
from ketos.services.auth.utils import api_key_security
...
def _resolve_request_variables(
    ...
    values can be transported safely. The legacy ``X-KETOS-GLOBAL-VAR-*``
    ...
    if legacy_globals:
        logger.warning(
            "X-KETOS-GLOBAL-VAR-* headers are deprecated on /api/v2/workflows; "
            "send request-level globals in the JSON body instead. Header-based "
            "globals will be removed in a future Ketos release."
        )
...
async def execute_workflow(
    ...
    **background** and **stream** can't be true at the same time.
    ...
    Args:
        ...
        http_request: The HTTP request, used to honor deprecated ``X-KETOS-GLOBAL-VAR-*`` headers
...
async def execute_sync_workflow(
    ...
    # Unicode strings transport safely. The legacy X-KETOS-GLOBAL-VAR-*
