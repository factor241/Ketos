"""Integration tests for migrating in-tree Docling components to kfx-docling."""
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_name: str) -> None:
    ...
    target = f"ext:docling:{class_name}@official"
    flow = _saved_flow(f"kfx.components.docling.{module_name}.{class_name}")
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, _module_name: str) -> None:
    ...
    target = f"ext:docling:{class_name}@official"
    flow = _saved_flow(f"kfx.components.docling.{class_name}")
...
@pytest.mark.integration
def test_kfx_docling_distribution_is_importable() -> None:
    try:
        import kfx_docling
    except ImportError:
        pytest.skip("kfx-docling not installed in this test environment")

    for class_name, _module_name in COMPONENTS:
        assert getattr(kfx_docling, class_name).__name__ == class_name
...
def test_bundle_loads_and_resolves_runtime_classes() -> None:
    ...
    result = load_extension(Path(kfx_docling.__file__).parent, slot=SLOT_OFFICIAL, distribution="kfx-docling")
