...
Examples::
    ...
    kfx status                          # scans flows/ in cwd, uses [defaults] env
    kfx status --env staging            # compare against staging
    kfx status --dir ./my-flows/        # specify a custom flows directory
    kfx status --env prod --remote-only # also show server flows not tracked locally
"""
...
def _load_sdk() -> tuple[object, object, object, type]:
    """Return (normalize_flow, flow_to_json, Client, KetosNotFoundError) from ketos_sdk."""
    sdk = load_sdk("status")
    from ketos_sdk.exceptions import KetosNotFoundError
    from ketos_sdk.serialization import flow_to_json, normalize_flow

    return normalize_flow, flow_to_json, sdk.Client, KetosNotFoundError
            ...
            if not raw_id:
                statuses.append(
                    FlowStatus(
                        ...
                        path=path,
                        detail="run kfx export --env <env> first to assign a stable id",
                    )
                )
                continue
