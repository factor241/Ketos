"""Ketos Extension System (foundation).
...
Public surface for this milestone:
    ...
    - ``validate_extension`` -- the offline, non-executing manifest + AST checker
      that backs ``kfx extension validate``.
...
and ``from kfx.extension import X`` keeps working unchanged -- only the wall
...
if TYPE_CHECKING:
    ...
    from kfx.extension.dev_registry import (
        DevExtensionEntry,
    ...
    from kfx.extension.validate import (
        ValidateReport,
        validate_extension,
    )
...
def __getattr__(name: str) -> Any:
    ...
    Importing ``kfx.extension`` no longer eagerly pulls every submodule.
    The first access of e.g. ``kfx.extension.validate_extension`` imports
    ``kfx.extension.validate`` and caches the resolved attribute in the
...
def __dir__() -> list[str]:
    """Make every lazy export show up in ``dir(kfx.extension)`` for tab completion."""
