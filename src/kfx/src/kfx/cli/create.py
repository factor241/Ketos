"""kfx create -- scaffold a new flow JSON from a built-in template.
...
Examples::
    ...
    kfx create my-chatbot
    kfx create my-rag --template hello-world
    kfx create my-flow --output-dir ./flows --overwrite
    kfx create --list
"""
...
# Descriptions shown in ``kfx create --list``
...
def create_command(
    ...
    *,
    ...
    available = list_templates()
    if not available:
        console.print("[red]Error:[/red] No flow templates found in the kfx package.")
        raise typer.Exit(1)
    ...
    console.print(f"  [bold]kfx validate {dest}[/bold]")
    console.print(f"  [bold]kfx serve {dest}[/bold]")
