...
@pytest.mark.parametrize("namespace", ["components", "kfx-other"])
...
def test_locale_bundle_rejects_non_flat_or_empty_locale_maps(locales: dict) -> None:
    payload = _with(
        _VALID,
        locale_bundle={"namespace": "kfx-openai", "locales": locales},
    )
...
def test_multi_bundle_rejected_at_model_level() -> None:
    ...
    surfaced by the validator pipeline in ``kfx.extension.validate`` (covered by
...
def test_load_manifest_extension_json(tmp_path: Path) -> None:
    ...
    assert source.manifest.id == "kfx-openai"
...
[tool.ketos.extension.kfx]
compat = ["1"]

[[tool.ketos.extension.bundles]]
name = "openai"
path = "openai"
"""
...
def test_load_manifest_extension_json_wins_over_pyproject(tmp_path: Path) -> None:
    (tmp_path / "extension.json").write_text(json.dumps(_VALID), encoding="utf-8")
    (tmp_path / "pyproject.toml").write_text("[tool.ketos.extension]\nid='wrong'", encoding="utf-8")
    source = load_manifest(tmp_path)
    assert source.kind == "extension.json"
...
def test_kfx_compat_rejects_invalid_versions() -> None:
    KfxCompat(compat=["1"])
    KfxCompat(compat=["1", "2"])
    with pytest.raises(ValidationError):
        KfxCompat(compat=[])
    with pytest.raises(ValidationError):
        KfxCompat(compat=["0"])  # version 0 is not valid
    with pytest.raises(ValidationError):
        KfxCompat(compat=["01"])  # leading zero rejected
    with pytest.raises(ValidationError):
        KfxCompat(compat=["v1"])  # non-numeric prefix rejected
    with pytest.raises(ValidationError):
        KfxCompat(compat=[""])  # empty string rejected
    with pytest.raises(ValidationError):
        KfxCompat(compat=[1])  # type: ignore[list-item] # int rejected by StrictStr
    with pytest.raises(ValidationError):
        KfxCompat(compat=["1", "1"])  # duplicate
