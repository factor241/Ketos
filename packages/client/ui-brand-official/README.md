---
description: "Official Ketos brand occupants for the sidebar, active only in official builds; for users and maintainers choosing or replacing brand presentation."
...
This package gives an `official` client build the Ketos mark and name in the sidebar. Other build profiles keep the shell's fish mark and local-build label, while the conversation hero always uses the animated fish. Choose it for deployments branded as Ketos; deployments with another identity should provide a replacement brand package. It has no runtime state and does not affect model requests.
...
- **Shared primitive artwork stays put** — `ui-primitives` keeps its `FishLogo` and hero-fish exports unchanged; the sidebar mark and name artwork is local to this package.
