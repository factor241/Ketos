...
def test_importing_ketos_module_in_kfx():
    code = dedent("""from ketos.custom import   Component
...
def test_importing_ketos_logging_in_kfx():
    """Test that ketos.logging can be imported in kfx context without errors."""
    code = dedent("""
...
class TestGetModuleFallbacks:
    ...
    def test_ketos_falls_back_to_kfx(self):
        result = _get_module_fallbacks("ketos.custom")
        assert result == ["ketos.custom", "kfx.custom"]

    def test_ketos_deep_path(self):
        result = _get_module_fallbacks("ketos.custom.validate")
        assert result == ["ketos.custom.validate", "kfx.custom.validate"]
    ...
    def test_bare_ketos_no_fallback(self):
        assert _get_module_fallbacks("ketos") == ["ketos"]
    ...
    def test_original_always_first(self):
        ...
        for name in ["ketos.custom", "langchain.agents", "requests"]:
