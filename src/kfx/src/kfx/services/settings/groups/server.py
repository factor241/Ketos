...
class ServerSettings(BaseModel):
    ...
    host: str = "localhost"
    """The host on which Ketos will run."""
    port: int = 7860
    """The port on which Ketos will run."""
    ...
    """The log level for Ketos."""
    log_file: str | None = "logs/ketos.log"
    """The path to log file for Ketos."""
    ...
    """If set to True, Ketos will open the browser on startup."""
    backend_only: bool = False
    """If set to True, Ketos will not serve the frontend."""
    ...
    prefix (e.g. '/ketos').  When set, the MCP SSE transport includes this
    prefix in the POST-back URL so clients can reach the correct endpoint.
    Can also be set via the KETOS_ROOT_PATH environment variable."""
    user_agent: str = "ketos"
    ...
    def validate_runtime_port(cls, value):
        ...
        Kubernetes auto-creates env vars like KETOS_RUNTIME_PORT=tcp://<ip>:<port>
        for services, which collides with the KETOS_ env prefix. Extract the port
