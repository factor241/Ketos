...
import pytest
from kfx.custom.utils import _generate_code_hash, build_component_metadata, build_custom_component_template_from_inputs
...
class TestMetadataInTemplateBuilders:
    ...
    @patch("kfx.custom.utils.ComponentFrontendNode")
    def test_build_from_inputs_adds_metadata_with_module(self, mock_frontend_class):
        ...
        from kfx.custom.utils import build_custom_component_template_from_inputs
        ...
        with patch("kfx.custom.utils.get_component_instance") as mock_get_instance:
            mock_instance = Mock()
            ...
            with (
                patch("kfx.custom.utils.add_code_field", return_value=mock_frontend),
                patch("kfx.custom.utils.reorder_fields"),
            ):
                # Call the function
    ...
    @patch("kfx.custom.utils.CustomComponentFrontendNode")
    def test_build_template_adds_metadata_with_module(self, mock_frontend_class):
        ...
        with patch("kfx.custom.utils.run_build_config") as mock_run_build:
            mock_instance = Mock()
            mock_instance._get_field_order = Mock(return_value=[])
            mock_run_build.return_value = ({}, mock_instance)

            with (
                patch("kfx.custom.utils.add_extra_fields"),
                patch("kfx.custom.utils.add_code_field", return_value=mock_frontend),
                patch("kfx.custom.utils.add_base_classes"),
                patch("kfx.custom.utils.add_output_types"),
                patch("kfx.custom.utils.reorder_fields"),
            ):
                # Call the function
...
class TestDependencyAnalyzer:
    ...
    def test_top_level_basic(self):
        ...
        from kfx.custom.dependency_analyzer import _top_level
    ...
    def test_is_relative(self):
        ...
        from kfx.custom.dependency_analyzer import _is_relative
    ...
    def test_analyze_dependencies_basic(self):
        ...
        from kfx.custom.dependency_analyzer import analyze_dependencies
...
"""
    ...
    def test_analyze_component_dependencies(self):
        ...
        from kfx.custom.dependency_analyzer import analyze_component_dependencies
...
"""
    ...
    def test_dependency_info_dataclass(self):
        ...
        from kfx.custom.dependency_analyzer import DependencyInfo
    ...
    def test_get_distribution_version_deterministic(self):
        ...
        """
        from kfx.custom.dependency_analyzer import _get_distribution_version
        ...
        for fake_map in (fake_map_asc, fake_map_desc):
            ...
            with (
                patch("kfx.custom.dependency_analyzer._get_packages_distributions", return_value=fake_map),
                patch("kfx.custom.dependency_analyzer.md.distribution", return_value=mock_dist) as mock_md,
            ):
...
"""
        ...
        # Verify dependency analysis results
        dep_info = mock_frontend.metadata["dependencies"]
        assert dep_info["total_dependencies"] == 1  # Only ketos (os is stdlib, filtered out)

        # Check for dependencies
        package_names = [pkg["name"] for pkg in dep_info["dependencies"]]
        assert "ketos" in package_names  # ketos should be detected as external
...
"""
    ...
    def test_build_component_metadata_with_real_component(self):
        ...
        from kfx.custom.custom_component.component import Component
        from kfx.custom.utils import build_component_metadata
...
from ketos.field_typing import LanguageModel
...
"""
        ...
        build_component_metadata(
            mock_frontend, test_component, "ketos.components.lmstudio", "LMStudioModelComponent"
        )
        ...
        assert "ketos" in package_names  # project dependency
