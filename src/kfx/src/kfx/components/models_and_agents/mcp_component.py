...
from kfx.base.agents.utils import maybe_unflatten_dict, safe_cache_get, safe_cache_set
from kfx.base.mcp.util import (
    MCPStdioClient,
    MCPStreamableHttpClient,
    update_tools,
)
from kfx.base.tools.constants import TOOL_OUTPUT_DISPLAY_NAME, TOOL_OUTPUT_NAME
from kfx.custom.custom_component.component_with_cache import ComponentWithCache
from kfx.inputs.inputs import InputTypes  # noqa: TC001
from kfx.io import BoolInput, DictInput, DropdownInput, FloatInput, McpInput, MessageTextInput, Output
from kfx.io.schema import schema_to_ketos_inputs
from kfx.log.logger import logger
from kfx.schema.dataframe import DataFrame
from kfx.schema.message import Message
from kfx.services.deps import get_storage_service, session_scope
...
class MCPToolsComponent(ComponentWithCache):
    ...
    Behaviour notes:
    - Stale agent tools vs server were caused by Ketos caching the Toolset output
    ...
    def _build_tool_output(self) -> Output:
        ...
        # toggle: Ketos's Output.cache defaults to True and was memoizing the first to_toolkit()
    ...
    documentation: str = "https://docs.ketos.test/mcp-tools"
    icon = "Mcp"
    name = "MCPTools"
    ...
    async def _validate_schema_inputs(self, tool_obj) -> list[InputTypes]:
        ...
        try:
            ...
            schema_inputs = schema_to_ketos_inputs(input_schema)
    ...
    async def update_tool_list(self, mcp_server_value=None):
        ...
        async with self._update_tool_list_lock:
            ...
            try:
                ...
                # When running in KFX standalone mode the full Ketos package and
                ...
                try:
                    from ketos.api.v2.mcp import get_server
                    ...
                    # from an installed Ketos — a real API break that should NOT be
                    # swallowed as "standalone mode". ModuleNotFoundError alone covers the
                    # "Ketos absent" case.
                    #
                    # Even within ModuleNotFoundError, only treat this as KFX standalone mode
                    # when one of the target Ketos modules is itself missing. Transitive
                    # ModuleNotFoundError (e.g. a dependency like sqlmodel failing to import
                    # inside ketos.*) indicates a real bug in the full Ketos stack and
                    ...
                    is_ketos_standalone = missing_module == "ketos" or missing_module.startswith("ketos.")
                    if not is_ketos_standalone:
                        raise
                    await logger.ainfo(
                        "Ketos package not available; using MCP server config from flow value (KFX standalone mode)."
                    )
                else:
                ...
                if not request_variables and has_headers:
                    try:
                        from kfx.services.deps import get_variable_service
                ...
                msg = (
                    ...
                    "Raise ``KETOS_MCP_SERVER_TIMEOUT`` for the deployment if the MCP "
    ...
    def get_inputs_for_all_tools(self, tools: list) -> dict:
        ...
        for tool in tools:
            if not tool or not hasattr(tool, "name"):
                continue
            try:
                ketos_inputs = schema_to_ketos_inputs(tool.args_schema)
                inputs[tool.name] = ketos_inputs
    ...
    async def build_output(self) -> DataFrame:
        ...
        try:
            ...
            if self.tool != "":
                # Set session context for persistent MCP sessions using Ketos session ID
    ...
    def _get_session_context(self) -> str | None:
        """Get the Ketos session ID for MCP session caching."""
