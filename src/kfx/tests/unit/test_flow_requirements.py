...
import pytest
from kfx.utils.flow_requirements import (
    MODULE_EXTRA_DEPS,
    _detect_providers_from_template,
    _extract_component_requirements,
    _extract_imports,
    _get_import_to_dist_map,
    _get_kfx_provided_imports,
    _get_kfx_transitive_dists,
    _import_to_package,
    _pin_version,
...
def _find_starter_projects_dir() -> Path:
    ...
    for parent in current.parents:
        candidate = parent / "src" / "backend" / "base" / "ketos" / "initial_setup" / "starter_projects"
        if candidate.is_dir():
            return candidate
...
"""
    ...
    def test_kfx_imports(self):
        code = "from kfx.schema.message import Message"
        result = _extract_imports(code)
        assert "kfx" in result
...
class TestDynamicResolution:
    ...
    def test_kfx_transitive_dists_includes_kfx(self):
        dists = _get_kfx_transitive_dists()
        assert "kfx" in dists

    def test_kfx_transitive_dists_includes_langchain(self):
        dists = _get_kfx_transitive_dists()
        assert "langchain" in dists
        assert "langchain-core" in dists

    def test_kfx_transitive_dists_includes_pydantic(self):
        dists = _get_kfx_transitive_dists()
        assert "pydantic" in dists

    def test_kfx_provided_imports_includes_expected(self):
        provided = _get_kfx_provided_imports()
        ...
        for imp in resolvable:
            assert imp in provided, f"{imp} should be provided by kfx"

    def test_kfx_provided_imports_excludes_optional(self):
        """Packages not in kfx's dep tree should NOT be in provided."""
        provided = _get_kfx_provided_imports()
        # langchain-openai is an optional provider, not a core kfx dep
...
class TestExtractComponentRequirements:
    def test_kfx_only_component(self):
        code = """
...
"""
    ...
    def test_kfx_provided_filtered(self):
        """Imports that are transitively provided by kfx should not appear as requirements.
        ...
        if not resolvable:
            pytest.skip("None of the test imports are kfx-provided in this environment")
...
"""
    ...
    def test_provider_detected(self):
        node = _make_node(
            "LanguageModel",
            "from kfx.base.models.model import LCModelComponent",
            template_extra={
                "model": {
    ...
    def test_module_extra_deps(self):
        ...
        whether it's transitively provided by kfx, but the extra runtime deps
...
"""
    ...
    def test_ketos_imports_filtered(self):
        """Components with ketos imports should NOT list ketos as a dep.

        kfx provides the ketos interfaces at runtime, so ketos/ketos_base
        should be filtered out just like kfx itself.
        """
        code = """
from ketos.custom import Component
from ketos.io import MessageTextInput
"""
        node = _make_node("LegacyComponent", code)
        packages, _ = _extract_component_requirements(node)
        assert "ketos" not in packages
        assert "ketos-base" not in packages
...
class TestGenerateRequirementsFromFlow:
    ...
    def test_include_kfx_false(self):
        flow = _make_flow()
        result = generate_requirements_from_flow(flow, include_kfx=False, pin_versions=False)
        assert "kfx" not in result

    def test_custom_kfx_package_name(self):
        flow = _make_flow()
        result = generate_requirements_from_flow(flow, kfx_package="kfx-nightly", pin_versions=False)
        assert "kfx-nightly" in result
        assert "kfx" not in result
    ...
    def test_results_sorted(self):
        ...
        result = generate_requirements_from_flow(flow, pin_versions=False)
        # kfx should be first, then sorted extras
        assert result[0] == "kfx"
        extras = result[1:]
...
class TestVersionPinning:
    def test_pin_version_installed_package(self):
        ...
        result = _pin_version("kfx")
        assert result.startswith("kfx==")
    ...
    def test_pin_versions_true_by_default(self):
        ...
        assert result[0].startswith("kfx==")
    ...
    def test_pinned_output_includes_versions_for_installed_deps(self):
        ...
        # kfx is installed, so it should be pinned
        assert result[0].startswith("kfx==")
...
class TestStarterProjects:
    ...
    def test_basic_prompting_includes_kfx(self, basic_prompting_flow):
        """Basic Prompting (ships with no pre-selected model) should still need kfx.
    ...
    def test_simple_agent_requirements(self, simple_agent_flow):
        """Simple Agent requires kfx and no longer drags in langchain-community.
        ...
        include ``kfx`` but never ``langchain-community`` (regardless of whether
        kfx happens to provide it transitively in a given environment).
        """
    ...
    def test_basic_prompting_from_file(self):
        ...
        generated requirements should include ``kfx`` but none of the
    ...
    def test_kfx_nightly_package_name(self, basic_prompting_flow):
        """Test specifying kfx-nightly as the package name."""
        result = generate_requirements_from_flow(
            basic_prompting_flow,
            kfx_package="kfx-nightly",
            pin_versions=False,
        )
        assert result[0] == "kfx-nightly"
        assert "kfx" not in result
...
class TestResolveProviderPackages:
    ...
    def _skip_if_provider_not_loaded(provider_name: str):
        ...
        """
        try:
            from kfx.base.models.model_input_constants import MODEL_PROVIDERS_DICT
        except ImportError:
...
class TestResolveEmbeddingProviderPackages:
    ...
    def test_all_embedding_providers_resolve(self):
        ...
        from kfx.base.models.unified_models import EMBEDDING_PROVIDER_CLASS_MAPPING
...
class TestErrorHandling:
    ...
    def test_generate_requirements_from_file_wrong_structure(self, tmp_path):
        """A valid JSON file that isn't a flow should still produce a result (just kfx)."""
...
# ===================================================================
# Typer CLI tests: kfx requirements
# ===================================================================


class TestTyperRequirementsCommand:
    """Tests for the typer-based ``kfx requirements`` CLI command."""
    ...
    def flow_file(self, tmp_path):
        flow = _make_flow(_make_node("Simple", "import kfx"))
    ...
    def test_output_flag_writes_file(self, runner, app, flow_file, tmp_path):
        ...
        assert result.exit_code == 0
        assert out.exists()
        assert "kfx" in out.read_text(encoding="utf-8")
        assert "Requirements written to" in result.output

    def test_no_kfx_flag(self, runner, app, flow_file):
        result = runner.invoke(app, ["requirements", str(flow_file), "--no-kfx", "--no-pin"])
        assert result.exit_code == 0
        # With --no-kfx and only kfx imports, no packages should appear after header
        lines = [line for line in result.output.strip().split("\n") if line and not line.startswith("#")]
        assert "kfx" not in lines
    ...
    def test_kfx_package_flag(self, runner, app, flow_file):
        result = runner.invoke(app, ["requirements", str(flow_file), "--kfx-package", "kfx-nightly", "--no-pin"])
        assert result.exit_code == 0
        assert "kfx-nightly" in result.output
        # Should not contain bare "kfx" as a separate line
        lines = [line for line in result.output.strip().split("\n") if line and not line.startswith("#")]
        assert "kfx" not in lines
