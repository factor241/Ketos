"""kfx environment configuration — resolve Ketos instance URL and API key.
...
TOML format is also accepted (``ketos-environments.toml`` or any ``.toml``
...
class ConfigError(Exception):
    ...
    will simply be ``None`` on the returned :class:`KetosEnvironment`.
...
@dataclass
class KetosEnvironment:
    """A fully-resolved Ketos target instance.

    Attributes:
        name:    Human-readable label (environment name or ``"__inline__"``).
        url:     Base URL of the Ketos instance.
...
def _parse_env_block(name: str, block: Any, config_path: Path) -> KetosEnvironment:
    ...
    return KetosEnvironment(name=name, url=url, api_key=api_key)


def _load_config(path: Path) -> tuple[dict[str, KetosEnvironment], str | None]:
    ...
    envs: dict[str, KetosEnvironment] = {}
...
def resolve_environment(
    ...
    *,
    ...
    """Resolve an environment name (or inline flags) to a :class:`KetosEnvironment`.
    ...
    if target is not None:
        ...
        return KetosEnvironment(name=name, url=target, api_key=api_key)
    ...
    if api_key is not None:
        resolved = KetosEnvironment(name=resolved.name, url=resolved.url, api_key=api_key)
