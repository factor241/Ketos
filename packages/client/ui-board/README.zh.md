...
- **面板的文件夹浏览器需要 browse 目录选择器** —— 组合了平台原生选择器的部署无法列出目录，此时文件夹浏览器会报告失败，注册目录仍走应用自身的「添加文件夹」流程（系统选择器）。
- **聊天面板打开时小地图让位** —— 面板是管理界面，否则小地图的角落会压住它。
