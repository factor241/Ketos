...
# late — see ketos_launcher.py for the same pattern).
#
# The `ketos` console script routes through ketos_launcher.py which
# handles this. This guard catches the bypass paths (`python -m ketos`,
# `uv run python -m ketos`, etc.) so they're not silent footguns. Only
# fires for direct CLI invocation; ordinary `import ketos.__main__` is
# unaffected.
...
from ketos.cli.progress import create_ketos_progress
from ketos.initial_setup.setup import get_or_create_default_folder
from ketos.main import setup_app
from ketos.services.auth.utils import get_current_user_from_access_token
from ketos.services.database.models.api_key.crud import check_key
from ketos.services.database.service import UnsupportedPostgreSQLVersionError, check_postgresql_version_sync
from ketos.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
from ketos.services.utils import get_auto_login_superuser_password, initialize_services
from ketos.utils.version import fetch_latest_version, get_version_info
from ketos.utils.version import is_pre_release as ketos_is_pre_release
...
# Add KFX commands as a sub-app
try:
    from kfx.cli.commands import serve_command
    from kfx.cli.run import run as kfx_run

    kfx_app = typer.Typer(name="kfx", help="Ketos Executor commands")
    kfx_app.command(name="serve", help="Serve a flow as an API", no_args_is_help=True)(serve_command)
    kfx_app.command(name="run", help="Run a flow directly", no_args_is_help=True)(kfx_run)

    app.add_typer(kfx_app, name="kfx")
except ImportError:
    # KFX not available, skip adding the sub-app
    pass
...
# Platforms where `ketos run` bypasses Gunicorn and runs uvicorn directly
...
def ensure_multi_worker_safe(num_workers: int) -> None:
    ...
    Skipped entirely when ``KETOS_JOB_QUEUE_TYPE=redis`` is configured —
    ...
    msg = (
        ...
        "  * Configure a shared job queue: KETOS_JOB_QUEUE_TYPE=redis. Works "
...
def run(
    *,
    ...
    progress = create_ketos_progress(verbose=verbose)

    # Step 0: Initializing Ketos
    ...
    # Step 6: Launching Ketos
    if use_direct_uvicorn():
        # KETOS_GUNICORN_PRELOAD is a Gunicorn-only knob: it triggers fork-safe
        ...
        with progress.step(6):
            # Use Gunicorn with KetosUvicornWorker for non-Windows systems
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "ketos")
        'A new version of ketos is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=ketos_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if ketos_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use ketos.utils.version to get the version info
    version_info = get_version_info()
    ketos_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= ketos_is_pre_release(ketos_version)  # Update pre-release status

    notice = build_version_notice(ketos_version, package_name)
    ...
    access_link = f"[bold]{status_icon} Open Ketos {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set KETOS_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from ketos.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set KETOS_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run ketos api_key`")
            raise typer.Exit(1)
...
# command to copy the ketos database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'ketos.db' and 'ketos-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "ketos.db"
    pre_db_path = cache_dir / "ketos-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from ketos.services.database.models.api_key.crud import create_api_key, delete_api_key
            from ketos.services.database.models.api_key.model import ApiKey, ApiKeyCreate
