"""Unit tests for the shared --upgrade-flow gate (kfx.upgrade.cli_gate)."""
...
# that `kfx upgrade` reads. An empty registry classifies EVERY node as `blocked`, so every flow
...
def test_missing_bundled_registry_raises_upgrade_flow_error():
    ...
    """
    with (
        patch("kfx.interface.components._read_component_index", return_value=None),
