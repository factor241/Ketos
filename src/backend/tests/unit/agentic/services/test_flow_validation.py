...
import pytest
from ketos.agentic.services.flow_agent_model import AgentModelOutcome
from ketos.agentic.services.flow_graph_build_check import BuildCheckResult
from ketos.agentic.services.flow_static_validation import FlowValidationReport
from ketos.agentic.services.flow_validation import (
    FlowVerifyStatus,
    verify_flow,
)
...
class TestVerifyFlow:
    ...
    async def test_should_not_leak_a_secret_in_the_caveat(self):
        async def build_fn(**_kw):
            return BuildCheckResult(ok=False, error="auth failed api key REDACTED_OPENAI_API_KEY")

        result = await _verify(build_fn=build_fn)
        assert "REDACTED_OPENAI_API_KEY" not in (result.caveat or "")
