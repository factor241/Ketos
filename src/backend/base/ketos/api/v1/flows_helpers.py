...
from ketos.services.database.models.flow.utils import get_webhook_component_in_flow
from ketos.services.database.models.folder.model import Folder
from ketos.services.database.models.folder.utils import get_default_folder_id
from ketos.services.deps import get_settings_service
from ketos.services.storage.service import StorageService
...
async def _new_flow_uncommitted(
    *,
    session: AsyncSession,
    flow: FlowCreate,
    user_id: UUID,
    ...
    """Create a Flow with flush-only database semantics and no external persistence."""
    normalized_flow = flow.model_copy(deep=True)
    if validate_folder and normalized_flow.folder_id is not None:
        folder = (
            await session.exec(
                select(Folder).where(Folder.id == normalized_flow.folder_id, Folder.user_id == user_id)
            )
        ).first()
        if not folder:
            raise HTTPException(status_code=400, detail="Folder not found")

    normalized_flow.user_id = user_id
    normalized_flow.name = await _deduplicate_flow_name(session, normalized_flow.name, user_id)

    if normalized_flow.endpoint_name:
        normalized_flow.endpoint_name = await _deduplicate_endpoint_name(
            session,
            normalized_flow.endpoint_name,
            user_id,
            fail_on_conflict=fail_on_endpoint_conflict,
        )

    db_flow = Flow.model_validate(normalized_flow.model_dump(exclude={"id"}))
    effective_id = flow_id if flow_id is not None else normalized_flow.id
...
async def _new_flow(
    *,
    ...
    """Create or upsert a flow and persist its optional filesystem projection."""
    try:
        await _verify_fs_path(flow.fs_path, user_id, storage_service)
        db_flow = await _new_flow_uncommitted(
            session=session,
            flow=flow,
            user_id=user_id,
            flow_id=flow_id,
            fail_on_endpoint_conflict=fail_on_endpoint_conflict,
...
async def _read_flow(
    ...
    """
    from ketos.services.authorization.fetch import authorized_or_owner_scoped
...
def _build_flows_download_response(
    ...
    if len(normalised_flows) > 1:
        ...
        filename = f"{current_time}_ketos_flows.zip"
