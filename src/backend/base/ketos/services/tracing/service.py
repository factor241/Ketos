...
def _get_langsmith_tracer():
    from ketos.services.tracing.langsmith import LangSmithTracer
...
def _get_opik_tracer():
    from ketos.services.tracing.opik import OpikTracer

    return OpikTracer


def _get_traceloop_tracer():
    from ketos.services.tracing.traceloop import TraceloopTracer
...
def _get_openlayer_tracer():
    from ketos.services.tracing.openlayer import OpenlayerTracer
...
class TraceContext:
    def __init__(
        ...
        # ``user_id`` is the authenticated Ketos user (e.g. API-key owner)
...
class TracingService(Service):
    ...
    def _initialize_langfuse_tracer(self, trace_context: TraceContext) -> None:
        ...
        # ``user_id`` carries the authenticated Ketos user and drives
    ...
    async def start_tracers(
        ...
        as ``ketos.tracing_user_id``).
        """
        if self.deactivated:
            return
        try:
            project_name = project_name or os.getenv("LANGCHAIN_PROJECT", "Ketos")
    ...
    def project_name(self):
        if self.deactivated:
            return os.getenv("LANGCHAIN_PROJECT", "Ketos")
        trace_context = trace_context_var.get()
        if trace_context is None:
