...
when the same class is re-exported from different modules (e.g., kfx.Message vs
ketos.schema.Message) but Python's isinstance() checks fail due to different
module paths.
"""
...
class CrossModuleModel(BaseModel, metaclass=CrossModuleMeta):
    ...
    Example:
        ...
        >>> from kfx.schema.message import Message as KfxMessage
        >>> from ketos.schema import Message as KetosMessage
        >>> msg = KfxMessage(text="hello")
        >>> isinstance(msg, KetosMessage)  # True (with cross-module support)
    """
