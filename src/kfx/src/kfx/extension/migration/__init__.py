...
references in three shapes:
    ...
    * old import path (``"ketos.components.openai.OpenAIEmbeddings"``)
...
``ext:<bundle>:<Class>@<slot>`` identifier.  Compatibility callers opt in by
passing a saved-flow payload to :func:`migrate_flow_payload`; importing this
package does not install a graph-deserializer hook.
...
Public surface:
    ...
    - :class:`MigrationReport` -- per-node rewrite outcomes for callers that
      need to record or display the compatibility action.
"""

from kfx.extension.migration.loader import (
    MIGRATION_TABLE_PATH,
    empty_table,
    invalidate_cache,
    load_migration_table,
)
from kfx.extension.migration.rewrite import (
    MigrationReport,
    NodeRewriteRecord,
    RewriteOutcome,
