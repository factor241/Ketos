...
from ketos.services.database import models
from ketos.services.database.constants import (
...
from ketos.services.database.session import NoopSession
from ketos.services.database.utils import Result, TableResults, is_unrecoverable_alembic_revision_error
from ketos.services.deps import get_settings_service
from ketos.services.utils import teardown_superuser
from ketos.utils.migration_lock import MIGRATION_ADVISORY_LOCK_ID
...
POSTGRESQL_SCHEMA_MIGRATION_LOCK_ID = MIGRATION_ADVISORY_LOCK_ID
...
def check_sqlite_database_path(database_url: str) -> None:
    ...
    ``KETOS_DATABASE_URL`` are resolved by SQLAlchemy against the current
    ...
    is opaque, so surface where Ketos actually tried to open the database and
    ...
    if db_path.is_absolute():
        msg += "Create the directory before starting Ketos, or point KETOS_DATABASE_URL at an existing path."
    else:
        msg += (
            f"The relative path '{db_path}' from KETOS_DATABASE_URL was resolved against the current working "
            f"directory ('{Path.cwd()}'). Set KETOS_DATABASE_URL to an absolute path "
            f"(e.g. 'sqlite:///{resolved}'), or create the directory before starting Ketos."
        )
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        # This file is in ketos.services.database.manager.py
        # the ini is in ketos
        ketos_dir = Path(__file__).parent.parent.parent
        self.script_location = ketos_dir / "alembic"
        self.alembic_cfg_path = ketos_dir / "alembic.ini"
        ...
        if self.alembic_log_to_stdout:
            ...
            base_dir = Path(config_dir) if config_dir else ketos_dir
    ...
    async def initialize_alembic_log_file(self):
        ...
        try:
            ...
            await logger.awarning(
                ...
                "Migration output falls back to stdout. Set KETOS_ALEMBIC_LOG_FILE to a writable path "
                "or KETOS_ALEMBIC_LOG_TO_STDOUT=true to silence this warning."
            )
    ...
    async def ensure_postgresql_version(self) -> None:
        ...
        Ketos's schema uses UNIQUE NULLS DISTINCT, which is only supported in PostgreSQL 15+.
    ...
    def _open_alembic_log_buffer(self):
        ...
        try:
            ...
            logger.warning(
                ...
                "Falling back to stdout. Set KETOS_ALEMBIC_LOG_FILE to a writable path "
                "or KETOS_ALEMBIC_LOG_TO_STDOUT=true to silence this warning."
            )
            return nullcontext(sys.stdout)

    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        del fix  # retained in the public call shape; destructive fix mode is disabled
        ...
        with _postgres_migration_lock(self.database_url), buffer_context as buffer:
            ...
            try:
                ...
                if isinstance(exc, util.exc.CommandError) and is_unrecoverable_alembic_revision_error(exc):
                    raise
    ...
    async def run_migrations(self, *, fix=False) -> None:
        ...
        async with session_scope() as session:
            ...
            try:
                revision_result = await session.exec(text("SELECT version_num FROM alembic_version"))
            except Exception as exc:
                message = str(exc).casefold()
                missing_table = "alembic_version" in message and any(
                ...
                revisions = revision_result.all()
                if len(revisions) != 1:
                    msg = "Alembic must contain exactly one current revision; refusing to alter persisted history"
                    raise util.exc.CommandError(msg)
                ...
                if not isinstance(current_revision, str) or not current_revision.strip():
                    msg = (
                        "Alembic current revision is missing; refusing to recreate or reset the alembic_version history"
                    )
                    raise util.exc.CommandError(msg)
    ...
    def try_downgrade_upgrade_until_success(alembic_cfg, retries=5) -> None:
        del alembic_cfg, retries
        msg = "Destructive Alembic downgrade/upgrade recovery is disabled; restore a version-matched backup"
