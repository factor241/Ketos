...
* ``ketos.services.authorization.audit``   — batched audit pipeline
* ``ketos.services.authorization.guards``  — ``ensure_*_permission`` family
* ``ketos.services.authorization.listing`` — ``filter_visible_resources``

This module is preserved so existing imports
(``from ketos.services.authorization.utils import ensure_flow_permission``)
...
from ketos.services.authorization.listing import (
    _default_resource_id_getter,
    filter_visible_resources,
)
from ketos.services.deps import get_authorization_service, get_settings_service
