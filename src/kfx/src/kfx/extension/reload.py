...
:class:`~kfx.extension.registry.BundleRegistry`.
...
1. **Stage 1 -- parallel load** into a per-reload staging namespace
   ...
   ``sys.modules`` without colliding with the live ``_kfx_ext.*`` entries.
...
3. **Stage 3 -- atomic swap** under the registry's write lock.  This is
   ...
   ``_kfx_ext.*`` namespace; the new :class:`BundleRecord` is installed.
...
from kfx.extension.bundle_registry import (
    BundleRecord,
    BundleRegistry,
    ReloadInProgressError,
)
from kfx.extension.errors import ExtensionError
from kfx.extension.loader import (
...
from kfx.extension.reload_swap import (
...
# ---------------------------------------------------------------------------
#
# Layers above kfx (the ketos component cache; eventually the events
# service) need to react to a successful reload.  ``kfx`` cannot import
# ``ketos``, so we expose a tiny registration API: ``ketos`` calls
...
def register_post_swap_hook(hook: Callable[[BundleRecord], None]) -> None:
    ...
    canonical use case is the ketos component cache, which needs to
            ...
            errors=[
                ExtensionError(
                    ...
                    hint=(
                        ...
                        "`kfx extension dev <path>`), then reload via "
...
# _drop_staging_modules) live in :mod:`kfx.extension.reload_swap`.  They
