...
def main():
    """Launches ketos with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from ketos.__main__ import main as ketos_main

        ketos_main()


def _launch_with_exec():
    """Launch ketos by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "ketos.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec ketos: {e}", file=sys.stderr)
        sys.exit(1)
