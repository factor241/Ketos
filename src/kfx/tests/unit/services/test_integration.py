...
from kfx.services.manager import ServiceManager
from kfx.services.schema import ServiceType

from .conftest import MockSessionService


class TestStandaloneKFX:
    """Test KFX running standalone without ketos."""
...
class TestKFXWithKetosConfig:
    """Test KFX with ketos configuration."""

    @pytest.fixture
    def ketos_config_dir(self, tmp_path):
        """Create a temporary ketos-style config directory."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()

        # Create kfx.toml with ketos services
        config_file = config_dir / "kfx.toml"
        config_file.write_text(
            """
[services]
storage_service = "kfx.services.storage.local:LocalStorageService"
cache_service = "kfx.services.cache.service:ThreadingInMemoryCache"
"""
        )
    ...
    def test_config_overrides_defaults(self, clean_manager, ketos_config_dir):
        """Test that config file overrides default services."""
        # Discover plugins from config
        clean_manager.discover_plugins(ketos_config_dir)
    ...
    def test_multiple_services_from_config(self, clean_manager, ketos_config_dir):
...
"""
    ...
    def test_kfx_toml_takes_precedence(self, clean_manager, tmp_path):
        """Test that kfx.toml takes precedence over pyproject.toml."""
        config_dir = tmp_path / "config"
        config_dir.mkdir()

        # Create both files with different services
        (config_dir / "kfx.toml").write_text(
            """
[services]
storage_service = "kfx.services.storage.local:LocalStorageService"
"""
...
"""
        ...
        # Should only have storage from kfx.toml
        assert ServiceType.STORAGE_SERVICE in clean_manager.service_classes
