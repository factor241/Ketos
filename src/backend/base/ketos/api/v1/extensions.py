...
from kfx.extension.bundle_registry import get_default_registry
from kfx.extension.errors import ExtensionError
from kfx.extension.reload import ReloadInProgressError, reload_bundle
from kfx.log.logger import logger
from kfx.services.deps import get_extension_events_service, get_settings_service
from pydantic import BaseModel

from ketos.api.error_codes import ApiErrorCode, coded_http_error
from ketos.api.utils.core import CurrentActiveUser
        ...
        raise _typed_http_exception(
            ...
            error=ExtensionError(
                ...
                message=(
                    ...
                    "Set KETOS_ENABLE_EXTENSION_RELOAD=true to enable it on "
                    "a local-development install (Mode A)."
                ),
                hint=(
                    "Local development: ``kfx extension dev`` sets the flag "
                    "for you.  Self-hosted: export KETOS_ENABLE_EXTENSION_RELOAD=true "
...
async def reload_extension_bundle(
    ...
    Returns the typed :class:`~kfx.extension.reload.ReloadResult` body on
        ...
        raise _typed_http_exception(
            ...
            error=ExtensionError(
                ...
                hint=(
                    ...
                    "distribution name); run ``kfx extension list`` to see the "
    ...
    if not result.ok:
        ...
        primary = (
            ...
            if result.errors
                ...
                location=f"{extension_id}/{bundle_name}",
                hint="Run `kfx extension validate` against the bundle source for details.",
            )
        )
        logger.warning(
