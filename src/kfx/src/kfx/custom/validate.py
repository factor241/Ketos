...
from kfx.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
from kfx.log.logger import logger

_KETOS_IS_INSTALLED = False

with contextlib.suppress(ImportError):
    import ketos  # noqa: F401

    _KETOS_IS_INSTALLED = True
...
def create_class(code, class_name):
    ...
    code = code.replace("from ketos import CustomComponent", "from ketos.custom import CustomComponent")
    code = code.replace(
...
def _get_module_fallbacks(module_name: str) -> list[str]:
    ...
    Handles ketos -> kfx and langchain -> langchain_classic remapping at the
    ...
    """
    names = [module_name]
    if module_name.startswith("ketos."):
        names.append(module_name.replace("ketos.", "kfx.", 1))
    if module_name.startswith("langchain."):
...
def get_default_imports(code_string):
    ...
    ketos_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, ketos_imports)
    ketos_module = importlib.import_module("kfx.field_typing")
    default_imports.update({name: getattr(ketos_module, name) for name in necessary_imports})
