...
class RuntimeSettings(BaseModel):
    ...
    dev: bool = False
    """If True, Ketos will run in development mode."""
    ...
    def set_event_delivery(cls, value, info):
        ...
        # (KETOS_JOB_QUEUE_TYPE=redis) to share state across workers, or
        ...
        if info.data.get("workers", 1) > 1 and info.data.get("job_queue_type", "asyncio") != "redis":
            ...
            if requested != "direct":
                logger.warning(
                    ...
                    "Set KETOS_JOB_QUEUE_TYPE=redis to keep '%s' delivery in multi-worker setups.",
                    requested,
                    requested,
                )
            return "direct"
