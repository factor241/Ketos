...
**Статус: отложено (post-MVP).** Desktop-приложение не входит в scope MVP (Часть I, §II.1: единственная поддерживаемая поверхность — web-профиль), поэтому литералы ниже в этапе 0 не менялись. Таблица — зафиксированные целевые замены для будущей итерации ребрендинга desktop; при её выполнении тесты `apps/desktop/tests/expected/*` обновляются тем же изменением.
