"""Level 1 structural validation checks for Ketos flow JSON.
...
if TYPE_CHECKING:
    from kfx.cli.validation.core import ValidationResult
...
def _make_issue(
    ...
    from kfx.cli.validation.core import ValidationIssue
...
def _node_display_name(node: dict[str, Any]) -> str | None:
    from kfx.cli.validation.core import _node_display_name as _ndn
...
def _check_version_mismatch(flow: dict[str, Any], result: ValidationResult) -> None:
    """Warn when nodes were built with a different Ketos version.

    Each unique ``lf_version`` embedded in the node metadata that differs from
    the currently installed Ketos version triggers a single warning covering
    all affected nodes.  If Ketos is not installed the check is skipped
    silently (kfx can run standalone).
    """
    from kfx.cli.validation.core import _get_lf_version

    installed = _get_lf_version()
    if installed is None:
        return  # Ketos not installed; skip silently
    ...
    for built_version, node_names in sorted(version_to_nodes.items()):
        ...
        result.issues.append(
            _make_issue(
                ...
                node_name=None,
                message=(
                    f"{count} component(s) built with Ketos {built_version} "
