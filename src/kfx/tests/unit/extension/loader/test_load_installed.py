...
def test_loads_each_installed_extension_once(tmp_path: Path) -> None:
    ...
    dist_a = make_installed_extension(tmp_path, "kfx-alpha")
    dist_b = make_installed_extension(tmp_path, "kfx-bravo")
    _populate_bundle(tmp_path, "kfx-alpha")
    _populate_bundle(tmp_path, "kfx-bravo")

    results = load_installed_extensions(distributions=[dist_a, dist_b])
    by_dist = {r.distribution: r for r in results}
    assert set(by_dist) == {"kfx-alpha", "kfx-bravo"}
...
def test_duplicate_distribution_emits_warning(tmp_path: Path) -> None:
    ...
    result = results[0]
    assert result.distribution == "kfx-pilot"
...
def test_editable_install_discovered_via_direct_url(tmp_path: Path) -> None:
    ...
    the source path.  Without this fallback, an ``kfx-duckduckgo`` linked
    ...
    dist = _EditableDist("kfx-pilot", project_root)
...
def test_results_are_lexicographically_ordered(tmp_path: Path) -> None:
    ...
    for name in ("kfx-zeta", "kfx-alpha", "kfx-mike"):
        make_installed_extension(tmp_path, name)
        _populate_bundle(tmp_path, name)
    dists = [
        FakeDist(name="kfx-zeta", root=tmp_path, files=[Path("kfx-zeta") / "extension.json"]),
        FakeDist(name="kfx-alpha", root=tmp_path, files=[Path("kfx-alpha") / "extension.json"]),
        FakeDist(name="kfx-mike", root=tmp_path, files=[Path("kfx-mike") / "extension.json"]),
    ]
    results = load_installed_extensions(distributions=dists)
    assert [r.distribution for r in results] == ["kfx-alpha", "kfx-mike", "kfx-zeta"]
