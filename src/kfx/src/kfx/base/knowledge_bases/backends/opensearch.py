...
Ketos Knowledge Bases can target a self-hosted or managed
OpenSearch cluster (AWS OpenSearch Service, Aiven, OpenSearch
Project, etc.) alongside the other native DB connectors. URL and
auth credentials are resolved through Ketos's ``variable_service``
...
* ``url_variable`` — name of the Ketos variable holding the
...
Ketos installs without OpenSearch deps keep working.
"""
...
class OpenSearchBackend(BaseVectorStoreBackend):
    """OpenSearch k-NN as a Ketos KB backend."""
    ...
    async def _resolve_secrets(self) -> None:
        ...
        if not url:
            msg = (
                f"OpenSearchBackend needs the {url_variable!r} Ketos variable "
    ...
    async def similarity_search(
        ...
        *,
        ...
        key, which is what every Ketos callsite that doesn't pass a
