...
from kfx.base.agents.utils import data_to_messages, get_chat_output_sender_name, resolve_agent_verbose
...
def normalize_message_content(message: BaseMessage) -> str:
    ...
    Note:
        This addresses the inconsistency in kfx.schema.data.Data.to_lc_message() where:
...
class ALTKBaseTool(BaseTool):
    ...
    def _get_altk_llm_object(self, *, use_output_val: bool = True) -> Any:
        ...
        if isinstance(llm_object, ChatAnthropic):
            ...
            logger.info("ALTK currently only supports OpenAI and Anthropic models through Ketos.")
            llm_client_obj = None
...
class ALTKBaseAgentComponent(AgentComponent):
    ...
    async def run_agent(
        ...
        if hasattr(self, "chat_history") and self.chat_history:
            ...
            # Handle both kfx.schema.message.Message and ketos.schema.message.Message types
