...
class AdapterRegistry(Generic[T]):
    ...
    def discover(self, config_dir: Path) -> None:
        ...
        Args:
            config_dir: Directory to search for ``kfx.toml`` / ``pyproject.toml``.
        """
        with self._lock:
            if self._discovered:
                logger.debug(
    ...
    def _discover_from_config(self, *, config_dir: Path) -> None:
        source = get_preferred_config_source(
            config_dir,
            kfx_root_path=self._config_section_path,
            pyproject_root_path=("tool", "kfx", *self._config_section_path),
        )
        if source is None:
            return
    ...
    def _load_config_file(self, *, config_path: Path, root_path: tuple[str, ...]) -> None:
        ...
        if config_path.name == "kfx.toml" or section:
...
def get_adapter_registry(
    ...
    * ``entry_point_group`` → ``"kfx.<adapter_type>.adapters"``
