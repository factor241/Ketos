...
from kfx.custom.utils import abuild_custom_components, create_component_template
...
from kfx.extension.bundle_registry import BundleRecord, get_default_registry
from kfx.extension.reload import register_post_swap_hook
from kfx.log.logger import logger
from kfx.utils.flow_validation import collect_code_by_hash, collect_component_hash_lookups
from kfx.utils.validate_cloud import (
...
MIN_MODULE_PARTS = 2
MIN_MODULE_PARTS_WITH_FILENAME = 4  # Minimum parts needed to have a module filename (kfx.components.type.filename)
...
def _post_reload_refresh_cache(record: BundleRecord) -> None:
    ...
    needing to thread it through ``kfx.extension.reload``.  Concrete logic
...
def _parse_dev_mode() -> tuple[bool, list[str] | None]:
    """Parse KFX_DEV to determine dev mode and which modules to load.

    Development mode must be explicitly enabled via the KFX_DEV environment variable.
    ...
    Supports two modes:
    - Boolean mode: KFX_DEV=1/true/yes loads all modules dynamically
    - List mode: KFX_DEV=mistral,openai,anthropic loads only specified modules
...
def _read_component_index(custom_path: str | None = None) -> dict | None:
    ...
    try:
        ...
        if custom_path:
            ...
            # Use built-in index
            pkg_dir = Path(inspect.getfile(kfx)).parent
        ...
        # Version check: ensure index matches installed kfx version
        from importlib.metadata import PackageNotFoundError, version

        try:
            installed_version = version("kfx")
        except PackageNotFoundError:
            # In some deployment environments (e.g. Docker with workspace installs),
            # kfx may be importable but lack dist-info metadata. Skip version check.
            logger.debug("Could not determine installed kfx version (no package metadata); skipping version check")
...
def _save_generated_index(modules_dict: dict) -> None:
    ...
    try:
        ...
        ketos_version = version("ketos")

        # Build index structure
        index = {
            "version": ketos_version,
            "metadata": {
...
async def _send_telemetry(
    ...
    try:
        ...
        from ketos.services.telemetry.schema import ComponentIndexPayload
...
def _warm_circular_imports() -> None:
    ...
    cleanly when first imported from a single thread, but the kfx policy modules reach
...
async def _load_components_dynamically(
    ...
    try:
        ...
        await logger.aerror(f"Failed to import ketos.components package: {e}", exc_info=True)
        return modules_dict
...
async def _load_full_dev_mode() -> tuple[dict[str, Any], str]:
    ...
    """
    await logger.adebug("KFX_DEV full mode: loading all modules dynamically")
...
async def _load_selective_dev_mode(
    ...
    Returns:
        Tuple of (modules_dict, index_source)
    """
    await logger.adebug(f"KFX_DEV selective mode: reloading {target_modules}")
...
async def import_ketos_components(
    settings_service: Optional["SettingsService"] = None,
    telemetry_service: Any | None = None,
) -> dict[str, dict[str, Any]]:
    """Asynchronously discovers and loads all built-in Ketos components.
...
def _process_single_module(modname: str) -> tuple[str, dict] | None:
    ...
    try:
        ...
        return None
    # Extract the top-level subpackage name after "kfx.components."
    # e.g., "kfx.components.Notion.add_content_to_page" -> "Notion"
...
def _components_path_extension_paths(settings_service: "SettingsService") -> list[Path]:
    ...
    Ketos components path is excluded (it is not an inline-bundle root).
...
# distribution = explicit, packaged install) to least (KETOS_COMPONENTS_PATH
...
async def import_extension_components(
    ...
    Two sources feed this:
        ...
        - Subfolders of every ``KETOS_COMPONENTS_PATH`` entry (parsed
    ...
    # ``$KETOS_SEED_DIR`` (or the default ``/opt/ketos/bundles``)
    ...
    # When neither $KETOS_SEED_DIR is set nor /opt/ketos/bundles
    # exists this is a no-op, so it costs nothing in Mode A.
    seed_results = load_seed_extensions()
    # Dev extensions registered via ``kfx extension dev`` ship the same v0
    ...
    # approach that appended their bundle directories to KETOS_COMPONENTS_PATH,
    ...
    # dev (`kfx extension dev`) > inline (KETOS_COMPONENTS_PATH) -- and the
    ...
    for result in all_results:
        ...
        for loaded in result.components:
            try:
                ...
                # doesn't actually inherit from the kfx Component base will
...
def refresh_bundle_cache_from_record(record: "BundleRecord") -> None:
    ...
    The reload pipeline updates :class:`~kfx.extension.bundle_registry.BundleRegistry`
    ...
    server restart.  Called by :func:`kfx.extension.reload.reload_bundle`
    in Stage 5.
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Ketos
    ...
    if component_cache.all_types_dict is None:
        ...
        ketos_components = await import_ketos_components(settings_service, telemetry_service)
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from kfx.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from kfx.custom.utils import build_custom_components
