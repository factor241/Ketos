 ...
 *
 * The `ketos/` group is excepted like `experimental/`: it is the soft fork's
 * MVP group, its packages stay local, and the final non-release branch
 * requires their manifests to be `private: true`.
 */
const standardReleaseMemberDirectory =
  /^(?:packages\/(?!ketos\/)(?!experimental\/)[^/]+\/[^/]+|apps\/(?!desktop(?:-host)?$)[^/]+|vendor\/[^/]+)$/
