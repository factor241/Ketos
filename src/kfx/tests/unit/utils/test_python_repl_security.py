...
import pytest
from kfx.utils.python_repl_security import safe_builtins, validate_code_safety
...
class TestFormatterSinkBypassesAreBlocked:
    ...
    def test_formatter_vformat_bypass(self, monkeypatch):
        ...
        monkeypatch.setenv("KFX_REPL_CANARY", "SHOULD_NOT_LEAK")
        leaked = string.Formatter().vformat("{0.__globals__[os].environ[KFX_REPL_CANARY]}", (self._func_with_os(),), {})
        assert leaked == "SHOULD_NOT_LEAK"  # gadget is real when unguarded

        code = 'string.Formatter().vformat("{0.__globals__[os].environ[KFX_REPL_CANARY]}", (f,), {})'
        with pytest.raises(ValueError, match="not allowed"):
            validate_code_safety(code)
    ...
    def test_formatter_get_field_bypass(self, monkeypatch):
        ...
        monkeypatch.setenv("KFX_REPL_CANARY", "SHOULD_NOT_LEAK")
        obj, _ = string.Formatter().get_field("0.__globals__[os].environ[KFX_REPL_CANARY]", (self._func_with_os(),), {})
        assert obj == "SHOULD_NOT_LEAK"  # gadget is real when unguarded

        code = 'string.Formatter().get_field("0.__globals__[os].environ[KFX_REPL_CANARY]", (f,), {})'
        with pytest.raises(ValueError, match="not allowed"):
            validate_code_safety(code)
    ...
    def test_operator_attrgetter_bypass(self, monkeypatch):
        ...
        monkeypatch.setenv("KFX_REPL_CANARY", "SHOULD_NOT_LEAK")
        leaked = operator.attrgetter("__globals__")(self._func_with_os())["os"].environ["KFX_REPL_CANARY"]
        assert leaked == "SHOULD_NOT_LEAK"  # gadget is real when unguarded
    ...
    def test_operator_methodcaller_format_bypass(self, monkeypatch):
        ...
        tmpl = "{0." + "__globals__" + "[os].environ[KFX_REPL_CANARY]}"
...
class TestEnsureCodeExecutionEnabled:
    ...
    def test_blocks_when_custom_components_disabled(self, monkeypatch):
        ...
        from kfx.utils.python_repl_security import CodeExecutionDisabledError, ensure_code_execution_enabled
    ...
    def test_allows_when_custom_components_enabled(self, monkeypatch):
        ...
        from kfx.utils.python_repl_security import ensure_code_execution_enabled
    ...
    def test_allows_when_services_layer_absent(self, monkeypatch):
        ...
        # Deleting the imported name makes ``from kfx.services.deps import
        # get_settings_service`` raise ImportError, simulating an absent services layer.
        monkeypatch.delattr("kfx.services.deps.get_settings_service")
        ensure_code_execution_enabled()  # must not raise
    ...
    def test_blocks_when_settings_service_is_none(self, monkeypatch):
        ...
        monkeypatch.setattr("kfx.services.deps.get_settings_service", lambda: None)
        with pytest.raises(CodeExecutionDisabledError):
            ensure_code_execution_enabled()
    ...
    def test_non_import_error_propagates(self, monkeypatch):
        ...
        monkeypatch.setattr("kfx.services.deps.get_settings_service", _boom)
