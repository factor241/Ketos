...
the existing ``kfx validate`` (which validates flow JSON, not extensions).
...
Commands shipped here:
    ...
    - ``dev``       -- register a local extension and launch Ketos with
                       it loaded.

Each command is implemented as a thin shell over the helpers in
``kfx.extension.*`` so the same code paths are reachable from tests
...
extension_app = typer.Typer(
    name="extension",
    help="Author and inspect Ketos Extensions.",
...
def validate_command(
    ...
    *,
    ...
    subprocess with a temporary Ketos state directory.
    """
    from kfx.extension import format_extension_error, validate_extension
...
def list_command(
    ...
    seed_dir: str | None = typer.Option(
        ...
        "--seed-dir",
        help=(
            "Override $KETOS_SEED_DIR for this invocation. "
    ...
    Output (text mode) is one row per Extension::
        ...
        kfx-openai          1.2.0     openai              @official  installed  discovered
        kfx-anthropic       0.4.1     anthropic           @official  seed       discovered
    ...
    :func:`~kfx.extension.format_extension_error`.
    ...
    # ``kfx extension list`` should still exit 0 in those cases so CI scripts
    # like ``kfx extension list && ...`` keep flowing -- the warning stays on
    ...
    # listing can immediately spot a wrong-venv mismatch between ``kfx``
    # and ``ketos run`` without a separate "did my bundle install?"
    # debug cycle.
...
def reload_command(
    extension_id: str | None = typer.Argument(
        None,
        help=("ID of the extension whose Bundle should be reloaded (e.g. kfx-arxiv). Omit when passing --all."),
    ),
    bundle: str | None = typer.Option(
        ...
        help=(
            ...
            "same source ``kfx extension list`` uses)."
        ),
    ),
    target: str | None = typer.Option(
        None,
        "--target",
        help=("Ketos server URL (default: $KETOS_HOST / http://localhost:7860)."),
    ),
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="API key for the Ketos server (default: $KETOS_API_KEY).",
    ),
    output_format: str = typer.Option(
        "text",
        "--format",
    ...
    *,
    ...
    if extension_id is None:
        typer.echo(
            "extension reload requires an extension id (e.g. `kfx extension reload kfx-arxiv`), "
...
def _resolve_bundle_from_discovery(extension_id: str) -> str | None:
    ...
    """
    from kfx.extension import discover_all_extensions
    ...
    if not matches:
        typer.echo(
            ...
            "  - Run `kfx extension list` to see what is installed locally.\n"
...
def _post_reload(
    *,
    ...
    from kfx.cli._extension_reload_client import reload_via_http
...
def _render_reload_text(response, *, extension_id: str, bundle_name: str) -> None:
    ...
    from kfx.extension.errors import ExtensionError, format_extension_error
    ...
    for raw in raw_errors:
        ...
        try:
            err = ExtensionError(
                ...
                hint=str(raw.get("hint") or "Run `kfx extension validate` for details."),
...
def _reload_all(*, target: str | None, api_key: str | None, output_format: str) -> None:
    ...
    if not extensions:
        typer.echo(
            ...
            "  - Run `kfx extension list` to confirm what is installed.",
        )
        return
...
def schema_command(
    ...
    ``schemas.ketos.test/extension/v1.json``; the release pipeline uploads
    this same artifact.
    """
    from kfx.extension.schema import build_schema_json
...
def init_command(
    ...
    *,
    ...
    name: str | None = typer.Option(
        None,
        "--name",
        help=("Human-readable display name shown in Ketos.  Defaults to the title-cased extension id."),
    ),
) -> None:
    ...
    """
    from kfx.extension import format_extension_error
    from kfx.extension.init_template import (
        BASIC_TEMPLATE,
        InitOptions,
        derive_bundle_name,
        derive_display_name,
    ...
    typer.echo(f"  kfx extension validate {target_path}")
    typer.echo(f"  kfx extension dev {target_path}")
...
def _resolve_ketos_executable() -> str | None:
    """Find the ``ketos`` binary on PATH, if any.

    Returns ``None`` when ketos isn't installed; the dev command then
    falls back to ``python -m ketos.__main__`` so an author with the
    ``kfx`` package alone still gets a usable error message.
    """
    return shutil.which("ketos")


@extension_app.command(
    name="dev",
    help="Register a local extension and launch Ketos with it loaded.",
)
def dev_command(
    target: str = typer.Argument(
        ".",
        help=(
            ...
            "[tool.ketos.extension] in pyproject.toml."
        ),
    ),
    *,
    ...
    skip_launch: bool = typer.Option(
        ...
        help=(
            ...
            "``ketos run``.  Useful for tests and for embedding in "
            "external dev-server scripts."
        ),
    ),
    extra_args: list[str] | None = typer.Argument(
        None,
        help="Extra arguments forwarded to ``ketos run`` (after a ``--`` separator).",
    ),
) -> None:
    """Register the extension and launch a Ketos dev server.
    ...
    Flow:
        ...
        5. ``exec``-style hand-off to ``ketos run`` (or
           ``python -m ketos``) with the env var
           ``KETOS_LAZY_LOAD_COMPONENTS=false`` so dev-extensions are
           visible in the palette immediately.

    AC #4 ("boots Ketos with the new Extension visible in the palette
    within 5s") is delivered jointly by this command and the startup
    hook in ``ketos.main`` that consults the dev registry.
    """
    from kfx.extension import format_extension_error, register_dev_extension, validate_extension
    ...
    typer.echo("  - Click 'Reload' on the bundle header in the Ketos palette.")

    if skip_launch:
        return

    extras = list(extra_args or [])
    cmd = _build_ketos_run_argv(extras)
    typer.echo("")
    typer.echo("Launching: " + " ".join(shlex.quote(part) for part in cmd))

    env = _build_dev_launch_env(os.environ)

    # Replace the current process so Ctrl-C in the launched ketos
    # propagates without an extra pty/job-control hop.  When ``ketos``
...
def _build_dev_launch_env(base_env: os._Environ[str] | dict[str, str]) -> dict[str, str]:
    """Build the env dict handed to the launched ``ketos run`` process.
    ...
    Flags set:
        - ``KETOS_LAZY_LOAD_COMPONENTS=false`` (always, overriding the
          author's shell): dev components must appear in the palette
          eagerly so the AC's 5-second budget holds.
        - ``KETOS_ENABLE_EXTENSION_RELOAD=true`` (setdefault): turns on
    ...
    """
    env = dict(base_env)
    env["KETOS_LAZY_LOAD_COMPONENTS"] = "false"
    env.setdefault("KETOS_ENABLE_EXTENSION_RELOAD", "true")
    return env


def _build_ketos_run_argv(extra_args: list[str]) -> list[str]:
    """Build the argv list that exec's ``ketos run``.

    Falls back to ``python -m ketos`` when the ``ketos`` binary is
    not on PATH so the dev loop still works inside an ``kfx``-only
    install (author hits a clear failure mode rather than a confusing
    one).
    """
    binary = _resolve_ketos_executable()
    if binary is not None:
        return [binary, "run", *extra_args]
    return [sys.executable, "-m", "ketos", "run", *extra_args]
