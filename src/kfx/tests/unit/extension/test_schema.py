...
import jsonschema
import pytest
from kfx.extension.manifest import DEFERRED_FIELDS, EXTENSION_SCHEMA_URL, ExtensionManifest
from kfx.extension.schema import build_schema, build_schema_json
...
@pytest.mark.parametrize(
    "locale_bundle",
    [
        {"namespace": "kfx-openai", "locales": {}},
        {"namespace": "kfx-openai", "locales": {"ru": {}}},
        {
            "namespace": "kfx-openai",
...
_MALFORMED_CASES: list[tuple[str, dict[str, Any]]] = [
    ...
    # 5. name empty
    ("name", {**_VALID, "name": ""}),
    # 6. kfx.compat missing
    ("kfx", {**_VALID, "kfx": {}}),
    # 7. kfx.compat empty
    ("kfx", {**_VALID, "kfx": {"compat": []}}),
    # 8. kfx.compat wrong element type (must be a string, not int)
    ("kfx", {**_VALID, "kfx": {"compat": [1]}}),
    # 9. bundles empty
