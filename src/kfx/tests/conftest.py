...
def allow_custom_components_by_default(monkeypatch):
    """Keep KFX tests aligned with the documented default unless a test opts out."""
...
def pytest_configure(config):  # noqa: ARG001
    ...
    # Check if ketos is installed first - fail fast
    import os

    if not os.getenv("KFX_TEST_ALLOW_KETOS"):
        try:
            import ketos  # noqa: F401

            pytest.exit(
                "\n"
                "=" * 80 + "\n"
                "ERROR: ketos is installed. These tests require ketos to NOT be installed.\n"
                "\n"
                ...
                "    uv run pytest ...\n"
                "\n"
                "The kfx tests are designed to run in isolation from ketos to ensure proper\n"
                "packaging and dependency management.\n"
                "=" * 80 + "\n",
                returncode=1,
            )
        except ImportError:
            # Good, ketos is not installed
            pass
...
def use_noop_database():
    """Ensure all KFX tests use NoopDatabaseService.
    ...
    with patch("kfx.services.deps.get_db_service", return_value=NoopDatabaseService()):
        yield
...
def use_noop_session():
    ...
    with patch("kfx.services.deps.session_scope") as mock_session_scope:
...
# Simple client fixture for basic HTTP testing (without full ketos app dependencies)
@pytest.fixture(name="client")
async def simple_client_fixture():
    """Simple HTTP client for basic testing."""
    # For kfx-specific tests, we might not need the full ketos app
