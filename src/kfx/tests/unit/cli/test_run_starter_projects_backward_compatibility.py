...
Tests that all starter project JSON files from tag 1.6.0 can be loaded by kfx run command
...
class TestRunStarterProjectsBackwardCompatibility:
    ...
    def test_run_1_6_0_starter_project_no_import_errors(self, template_file):
        """Test that 1.6.0 starter project can be loaded without ketos or kfx import errors.
        ...
        known_failing_projects = {
            "News Aggregator.json": "Contains SaveToFile component with ketos.api import bug "
        ...
        if "ModuleNotFoundError" in all_output or "ImportError" in all_output or "Module" in all_output:
            ...
            if "No module named 'ketos'" in all_output or "Module ketos" in all_output:
                ...
                for line in all_output.split("\n"):
                    if "ketos" in line and ("No module named" in line or "Module" in line):
                        ...
                        break
                pytest.fail(f"Ketos import error found in 1.6.0 template {template_file.name}.\nError: {error_line}")
            ...
            if "No module named 'kfx." in all_output or "Module kfx." in all_output:
                ...
                pytest.fail(
                    f"KFX import error found in 1.6.0 template {template_file.name}.\n"
    ...
    @pytest.mark.xfail(
        reason="1.6.0 basic templates have ketos import issues - components expect ketos package to be available"
    )
    def test_run_basic_1_6_0_starter_projects_detailed(self):
        ...
        for template_name in basic_templates:
            ...
            if "No module named 'ketos'" in all_output:
                pytest.fail(f"Ketos import error in 1.6.0 template {template_name}")
            ...
            if "ModuleNotFoundError" in all_output and "ketos" in all_output and "kfx.services" not in all_output:
                ...
                pytest.fail(f"Module not found error for ketos in 1.6.0 template {template_name}")
    ...
    def test_run_1_6_0_starter_project_with_stdin(self, template_file):
        ...
        all_output = result.output
        if "No module named 'ketos'" in all_output:
            pytest.fail("Ketos import error in 1.6.0 stdin test")
    ...
    def test_run_1_6_0_starter_project_inline_json(self, template_file):
        ...
        all_output = result.output
        if "No module named 'ketos'" in all_output:
            pytest.fail("Ketos import error in 1.6.0 inline JSON test")
