...
in the real ``kfx.custom.custom_component.component.Component`` -- that
...
def is_component_subclass(obj: object) -> bool:
    ...
    here (the loader is part of kfx; importing the rich Component class
    pulls in graph/, vertex/, etc., which are not needed for registration).

    The Ketos base classes themselves (``Component``, ``CustomComponent``,
