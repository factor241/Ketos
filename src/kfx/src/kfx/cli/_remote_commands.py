...
def register(app: typer.Typer) -> None:
    ...
    @app.command(
        name="status", help="Compare local flow files against a remote Ketos instance", rich_help_panel="Remote"
    )
    def status_command_wrapper(
        ...
        target: str | None = typer.Option(
            None,
            "--target",
            help="Ketos instance URL (inline override — skips config file lookup).",
        ),
        api_key: str | None = typer.Option(
            None,
            "--api-key",
            help="API key for the Ketos instance (used with --target or to override config).",
        ),
    ...
    @app.command(
        name="push",
        help="Push flow JSON to a remote Ketos instance (upsert by stable ID)",
    ...
    def push_command_wrapper(
        ...
        """Push Ketos flows to a remote instance using stable IDs for upsert (lazy-loaded)."""
    ...
    def pull_command_wrapper(
        ...
        """Pull and normalize flows from a remote Ketos instance (lazy-loaded)."""
    ...
    def export_command_wrapper(
        ...
        """Export and normalize Ketos flow JSON for version control (lazy-loaded)."""
