...
from ketos.api import health_check_router, log_router
from ketos.api.error_codes import register_api_error_handlers
from ketos.api.router import router
from ketos.api.v1.mcp_projects import init_mcp_servers
...
from ketos.middleware import ContentSizeLimitMiddleware
from ketos.plugin_routes import load_plugin_routes
from ketos.services.database.models.deployment.exceptions import DeploymentGuardError
from ketos.services.database.service import UnsupportedPostgreSQLVersionError
...
from ketos.services.tracing.otel_fastapi_patch import patch_otel_fastapi_route_details
from ketos.services.utils import initialize_services, initialize_settings_service, teardown_services
from ketos.utils.mcp_cleanup import cleanup_mcp_sessions
...
def cors_origins_contain_wildcard(origins) -> bool:
    ...
    `KETOS_CORS_ORIGINS="*"` is parsed as the raw string on some Python versions
...
def warn_about_future_cors_changes(settings):
    ...
    if using_permissive:
        logger.warning(
            ...
            "Set KETOS_CORS_ORIGINS for production. Stricter defaults in v2.0."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        from kfx.interface.components import component_cache, get_and_cache_all_types_dict

        from ketos.preload import PreloadStep, get_owned_temp_dirs, is_step_complete

        configure()

        # Startup message
        if version:
            await logger.adebug(f"Starting Ketos v{version}...")
        else:
            await logger.adebug("Starting Ketos...")
        ...
        # early failure (e.g. an unresolvable KETOS_DATABASE_URL) is masked by a
        ...
        try:
            ...
            if get_settings_service().settings.sentry_dsn:
                try:
                    ...
                    await logger.awarning(
                        "KETOS_SENTRY_DSN is set but sentry-sdk is not installed; "
                    ...
                    try:
                        ...
                        await logger.awarning(f"Failed to initialize Sentry SDK (check KETOS_SENTRY_DSN): {e}")
            ...
            # Locally-registered dev extensions (``kfx extension dev``) are
            ...
            if get_settings_service().settings.agentic_experience:
                if is_step_complete(PreloadStep.AGENTIC_GLOBALS):
                    ...
                    from ketos.api.utils.mcp.agentic_mcp import initialize_agentic_global_variables
            ...
            if get_settings_service().settings.agentic_experience:
                if is_step_complete(PreloadStep.AGENTIC_MCP):
                    ...
                    from ketos.api.utils.mcp.agentic_mcp import auto_configure_agentic_mcp_server
            ...
            async def refresh_models_dev_periodically() -> None:
                ...
                availability.
                """
                from kfx.base.models.models_dev_catalog import (
            ...
            # KETOS_MODELS_DEV_REFRESH=false disables the live models.dev
            ...
            from ketos.api.v1.mcp import start_streamable_http_manager
            from ketos.api.v1.mcp_projects import start_project_task_group
            ...
            if "ketos migration --fix" not in str(exc):
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from ketos.api.v1.mcp import stop_streamable_http_manager
                    from ketos.api.v1.mcp_projects import stop_project_task_group
                ...
                with shutdown_progress.step(2):
                    ...
                    try:
                        from ketos.services.authorization.utils import drain_pending_audit_writes
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Ketos shutdown complete")
...
def create_app():
    ...
    configure()
    from ketos.agentic.api.router import compose_stage09_lifespan

    lifespan = compose_stage09_lifespan(get_lifespan(version=__version__))
    ...
    if cors_origins_contain_wildcard(origins):
        if allow_credentials:
            ...
            logger.warning(
                ...
                "KETOS_CORS_ALLOW_CREDENTIALS=true; disabling credentials because a "
                ...
                "Set KETOS_CORS_ORIGINS to explicit origins to keep credentials enabled."
            )
    ...
    async def set_locale(request: Request, call_next):
        ...
        from ketos.utils.i18n import resolve_accept_language
    ...
    if settings.mcp_server_enabled:
        from ketos.api.v1 import mcp_router
    ...
    # Discover and register additional routers from plugins (ketos.plugins entry-point)
    ...
    async def rate_limit_exception_handler(request: Request, _exc: RateLimitExceeded):
        ...
        from ketos.services.rate_limit.service import get_limiter_key
...
def get_static_files_dir():
    """Get the static files directory relative to Ketos's main.py file."""
