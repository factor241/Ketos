...
import pytest
from kfx.custom.validate import (
    add_type_ignores,
...
"""
    ...
    @patch("kfx.custom.validate.logger")
...
"""
    ...
    def test_replaces_legacy_imports(self):
        ...
        code = """
from ketos import CustomComponent
...
"""
        ...
        with patch("kfx.custom.validate.prepare_global_scope") as mock_prepare:
            mock_prepare.return_value = {"CustomComponent": type("CustomComponent", (), {})}
            with patch("kfx.custom.validate.extract_class_code") as mock_extract:
                mock_extract.return_value = Mock()
                with patch("kfx.custom.validate.compile_class_code") as mock_compile:
                    mock_compile.return_value = compile("pass", "<string>", "exec")
                    with patch("kfx.custom.validate.build_class_constructor") as mock_build:
...
"""
        ...
        with (
            patch("kfx.custom.validate.prepare_global_scope", side_effect=validation_error),
...
class TestGetDefaultImports:
    ...
    @patch("kfx.field_typing.constants.CUSTOM_COMPONENT_SUPPORTED_TYPES", {"TestType": Mock()})
    ...
    def test_includes_ketos_imports(self):
        """Test that ketos imports are included when found in code."""
        # Use an actual type from CUSTOM_COMPONENT_SUPPORTED_TYPES
        code = "Chain is used here"

        with patch("kfx.custom.validate.importlib") as mock_importlib:
            mock_module = Mock()
