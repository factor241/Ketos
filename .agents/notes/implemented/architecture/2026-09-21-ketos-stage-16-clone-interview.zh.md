...
**一个协调器把该模式组合进 `agent.ctx`，随附路由在存储数据变动时通知它。** `src/session.ts` 在 `agent/created`、`agent/session-start`、`agent/disposed`，以及路由在一次获准写入后新发出的变更通知上做对账。正是这条通知让随附的顺序成立：`sessions.create()` 在宿主上创建 agent，因此绑定写在 agent 存在之后，没有任何生命周期事件能观察到它。每次对账都读取存储的绑定与克隆，并驱动一个 `agent.ctx.inject(['tools', 'systemPrompt'], …)` scope；对访谈中的会话，该 scope 注册 `clone_draft_save` 与 `clone:interview` 段落，因此这两项贡献与那个 agent 同生共死，绝不进入全局注册表：对普通会话而言 `ctx.tools.get('clone_draft_save')` 是 undefined，未绑定 agent 组装出的提示词中不含任何访谈文本。同一 scope 也为每个已绑定会话携带克隆的档案与记忆——该决策由[克隆记忆阶段](2026-09-22-ketos-stage-17-clone-memory.zh.md)拥有。对账按 agent 串接，因此两个重叠的触发无法把该 scope 安装两次；在途的安装会在其 await 之后重新检查释放，因此没有 scope 会活得比插件 fiber 更久。
...
- [Clone memory is one FTS5 store in clones.db, composed into the clone session scope and edited in the clone window](2026-09-22-ketos-stage-17-clone-memory.zh.md) —— 本阶段把此处的逐 agent scope 泛化为克隆会话 scope，并在访谈之旁加入档案段落、记忆快照与记忆工具。
