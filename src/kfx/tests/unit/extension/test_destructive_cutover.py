...
def test_migration_runtime_and_graph_hook_are_absent() -> None:
    ...
    graph_source = (
        REPO_ROOT / "src" / "kfx" / "src" / "kfx" / "graph" / "graph" / "base.py"
...
def test_removed_flow_migration_event_is_absent_from_source_and_tests() -> None:
    ...
    offenders = [
        ...
        and removed_event in path.read_text(encoding="utf-8")
    ]
...
def test_bundle_manifest_version_matches_distribution(bundle: str) -> None:
    ...
    manifest = json.loads(
        (project_root / "src" / package_name / "extension.json").read_text(encoding="utf-8")
    )
