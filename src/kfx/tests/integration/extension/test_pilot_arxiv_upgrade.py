...
``ArXivComponent`` from the pre-extraction Ketos:
    ...
    3. A second flow uses the legacy import path
       ``kfx.components.arxiv.arxiv.ArXivComponent``; same outcome.
    4. The kfx-arxiv distribution is importable AND ships the manifest in
...
def test_legacy_import_path_flow_upgrades(migration_table) -> None:
    ...
    flow = _saved_flow(
        _saved_flow_node(
            "arxiv-2",
            "kfx.components.arxiv.arxiv.ArXivComponent",
        )
    )
...
def test_short_import_path_flow_upgrades(migration_table) -> None:
    ...
    flow = _saved_flow(_saved_flow_node("arxiv-3", "kfx.components.arxiv.ArXivComponent"))
...
@pytest.mark.integration
def test_kfx_arxiv_distribution_is_importable() -> None:
    ...
    (kfx's own venv does not list kfx-arxiv as a dep); the ketos
    workspace venv pulls it in transitively from ketos's pyproject.
    """
    try:
        from kfx_arxiv import ArXivComponent
    except ImportError:
        pytest.skip("kfx-arxiv not installed in this test environment")
...
@pytest.mark.integration
def test_kfx_arxiv_ships_manifest() -> None:
    ...
    """
    try:
        dist = importlib_metadata.distribution("kfx-arxiv")
    ...
    if _is_editable_install(dist):
        ...
        package_dir = Path(kfx_arxiv.__file__).parent
        manifest_path = package_dir / "extension.json"
        assert manifest_path.is_file(), (
            f"kfx-arxiv source tree at {package_dir} does not ship "
        ...
        assert manifests, (
            "kfx-arxiv distribution does not ship extension.json in its "
