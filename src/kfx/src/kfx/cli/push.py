"""kfx push -- push normalized flow JSON to a remote Ketos instance.
...
Push a single flow to staging::
    ...
    kfx push my_flow.json --env staging

Push several flows at once::

    kfx push flows/*.json --env staging

Push all flows in a directory and place them in a named project::

    kfx push --dir ./flows/ --env staging --project "My RAG Pipeline"

Dry-run to see what would happen::

    kfx push ./flows/ --env production --dry-run
"""
...
from kfx.cli.common import load_sdk
...
def _extract_flow_id(flow: dict[str, Any], path: Path) -> UUID:
    ...
    raw_id = flow.get("id")
    if not raw_id:
        console.print(f"[red]Error:[/red] {path} has no 'id' field. Run [bold]kfx export[/bold] first.")
        raise typer.Exit(1)
    try:
    ...
    *,
    ...
    if local_file_content is not None:
        try:
            ...
            from ketos_sdk.exceptions import KetosNotFoundError
    ...
    except sdk.KetosHTTPError as exc:
        return PushResult(
            path=path,
            flow_id=flow_id,
            flow_name=flow_name,
...
def _find_project_root() -> Path | None:
    """Return the kfx project root (directory containing .kfx/), or None.

    Only the ``.kfx`` marker is used — ``.git`` is intentionally excluded so
    that the containment check doesn't reject paths when running inside a
    larger monorepo or outside any kfx project.
    """
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        if (directory / ".kfx").is_dir():
            return directory
...
def _collect_flow_files(sources: list[str], dir_path: str | None) -> list[Path]:
    ...
    ``flows/`` — mirroring the behaviour of ``kfx pull``.
    """
    paths: list[Path] = []
    root = _find_project_root()

    # Default to flows/ when nothing is specified, just like kfx pull does.
...
def push_command(
    ...
    *,
    ...
    from kfx.config import ConfigError, resolve_environment
    ...
    if not paths:
        console.print(
            ...
            "Run [bold]kfx pull[/bold] first, or pass explicit file paths."
        )
        raise typer.Exit(1)
