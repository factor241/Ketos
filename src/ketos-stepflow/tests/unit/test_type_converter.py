...
from ketos_stepflow.worker.base_executor import BaseExecutor
from ketos_stepflow.worker.handlers import (
    BaseModelInputHandler,
    BaseModelOutputHandler,
)
from ketos_stepflow.worker.handlers.base_model import (
    _is_secret_str_type,
)
...
class TestOutputTreeWalker:
    ...
    @pytest.mark.asyncio
    async def test_ketos_types_still_work(self):
        """Test that existing Ketos type serialization still works."""
        ...
        handlers = [KetosTypeOutputHandler(), BaseModelOutputHandler()]
        serialized = await self.executor._apply_output_handlers(message, handlers)

        # Should have Ketos type metadata, not BaseModel metadata
        assert "__ketos_type__" in serialized
        assert serialized["__ketos_type__"] == "Message"
