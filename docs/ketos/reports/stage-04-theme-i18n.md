...
- **Проверки:** `pnpm exec vitest run packages/client/ui-board/tests` — 8 файлов, 49 passed; `pnpm run test:gui` — 386 файлов, 5475 passed, 1 skipped; `pnpm run lint`, `typecheck`, `verify-client-packages`, `verify-module-graph`, `verify-client-ui-i18n` — зелёные; живьём проверены чипы, `+`-меню, `/`-меню, смена разрешения (команда доходит до хоста), вложение-картинка и активация отправки.
