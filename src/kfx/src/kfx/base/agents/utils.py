...
from kfx.services.cache.base import CacheService
...
def resolve_agent_verbose() -> bool:
    ...
    **stdout** via ``print()`` -- bypassing Ketos's logging system entirely (no log
    ...
    surfaced through Ketos's own event stream (the "Agent Steps" content blocks).
