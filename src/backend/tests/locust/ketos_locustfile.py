"""Ketos API Locust Load Testing File.

Comprehensive load testing for Ketos API with multiple user behaviors and performance analysis.
...
Environment Variables:
    - KETOS_HOST: Base URL for the Ketos server (default: http://localhost:7860)
...
TEST_MESSAGES = {
    ...
    "realistic": "Hey, Could you check https://docs.ketos.org for me? Later, could you calculate 1390 / 192 ?",
}
...
def setup_error_logging():
    ...
    global ERROR_LOG_FILE, KETOS_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"ketos_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("ketos_load_test_errors")
    ...
    # Try to capture Ketos logs
    ketos_log_paths = ["ketos.log", "logs/ketos.log", "../../../ketos.log", "../../../../ketos.log"]

    for log_path in ketos_log_paths:
        if Path(log_path).exists():
            KETOS_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if KETOS_LOG_FILE:
        print(f"   • Ketos logs: {KETOS_LOG_FILE}")
    else:
        print("   • Ketos logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"ketos_load_test_error_summary_{timestamp}.json"
...
def capture_ketos_logs():
    """Capture recent Ketos logs if available."""
    if not KETOS_LOG_FILE or not Path(KETOS_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of Ketos log
        with open(KETOS_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"ketos_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# Ketos server logs captured during load test\n")
            f.write(f"# Original log file: {KETOS_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 Ketos logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture Ketos logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    print(f"\n{'=' * 60}")
    print(f"KETOS API LOAD TEST RESULTS - GRADE: {grade}")
    print(f"{'=' * 60}")
...
class BaseKetosUser(FastHttpUser):
    """Base class for all Ketos API load testing user types."""

    abstract = True
    REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "30.0"))

    # Use the host provided by environment variable or default
    host = os.getenv("KETOS_HOST", "http://localhost:7860")
    ...
    def on_start(self):
        ...
        if not self.api_key:
            raise ValueError("API_KEY environment variable is required. Run setup_ketos_test.py first.")

        if not self.flow_id:
            raise ValueError("FLOW_ID environment variable is required. Run setup_ketos_test.py first.")
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # Ketos API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BaseKetosUser):
...
class AggressiveUser(BaseKetosUser):
...
class SustainedLoadUser(BaseKetosUser):
...
class TailLatencyHunter(BaseKetosUser):
...
class ScalabilityTestUser(BaseKetosUser):
