...
from kfx.interface.utils import extract_input_variables_from_prompt
from kfx.log.logger import logger
from kfx.schema.data import Data
from kfx.schema.message import Message

# Database imports removed - kfx should be lightweight
...
async def emit_vertex_build_event(
    *,
    ...
    try:
        ...
        from ketos.services.event_manager import webhook_event_manager
        ...
        pass  # ketos not available (standalone kfx usage)
...
async def log_transaction(
    ...
    Uses the pluggable TransactionService to log transactions. When running within ketos,
    the concrete TransactionService implementation persists to the database.
    When running standalone (kfx only), transactions are not persisted.
...
def _try_enqueue_via_telemetry_writer(vertex_build) -> bool:
    """Best-effort handoff to the ketos telemetry writer.

    Returns ``True`` when the row was accepted by the writer (caller should
    skip the direct DB write). Returns ``False`` when ketos isn't installed,
...
async def log_vertex_build(
    *,
    ...
    When running within ketos, it will use ketos's database service to persist the build.
    When running standalone (kfx only), it will only log debug messages.
    """
    try:
        # Try to use ketos's services if available (when running within ketos)
        try:
            from ketos.services.deps import get_db_service as ketos_get_db_service
            from ketos.services.deps import get_settings_service as ketos_get_settings_service

            settings_service = ketos_get_settings_service()
            ...
            from ketos.services.database.models.vertex_builds.crud import (
            ...
            db_service = ketos_get_db_service()
            ...
            # Fallback for standalone kfx usage (without ketos)
            ...
            # Log basic vertex build info - concrete implementation is in ketos
...
def has_chat_output(vertices: dict[Vertex, int]):
    from kfx.graph.schema import InterfaceComponentTypes
