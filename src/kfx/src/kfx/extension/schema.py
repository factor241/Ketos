...
The published schema lives at :data:`~kfx.extension.manifest.EXTENSION_SCHEMA_URL`
(``schemas.ketos.test/extension/v1.json``).  This module is the source of
truth for what gets uploaded there.

Two consumers:

    1. ``kfx extension schema`` (CLI, this PR) -- writes the schema to disk so
...
def _public_deferred_names() -> list[str]:
    ...
    Resolving here keeps :mod:`kfx.extension.manifest` the single source of
...
def build_schema() -> dict[str, Any]:
    ...
    schema["title"] = "Ketos Extension Manifest (v1)"
    ...
    schema["description"] = (
        ...
        "See https://docs.ketos.test/extensions-manifest for the field-by-field reference "
        "and https://docs.ketos.test/extensions-author-guide for the author's guide."
    )
    return schema
