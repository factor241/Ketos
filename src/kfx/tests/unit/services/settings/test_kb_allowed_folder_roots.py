...
HTTP 500), and ``KETOS_KB_ALLOWED_FOLDER_ROOTS`` bound to nothing because
...
def test_kb_allowed_folder_roots_binds_single_value_from_env(monkeypatch):
    """A single directory binds from the KETOS_-prefixed env var."""
    monkeypatch.setenv("KETOS_KB_ALLOWED_FOLDER_ROOTS", "/srv/docs")
    settings = Settings()
...
def test_kb_allowed_folder_roots_parses_comma_separated_env(monkeypatch):
    ...
    monkeypatch.setenv("KETOS_KB_ALLOWED_FOLDER_ROOTS", "/srv/docs,/data/shared")
    settings = Settings()
