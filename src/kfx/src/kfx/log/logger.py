"""Logging configuration for Ketos using structlog."""
...
class SizedLogBuffer:
    ...
    def __init__(
        ...
        The buffer can be overwritten by an env variable KETOS_LOG_RETRIEVER_BUFFER_SIZE
...
def add_otel_trace_context(_logger: Any, _method_name: str, event_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    OpenTelemetry is optional in kfx, so the import is resolved once at module
...
def _apply_logger_level_overrides() -> None:
    """Apply ``KETOS_LOG_LEVELS`` env var: ``name=LEVEL,name=LEVEL,...``.
    ...
    for pair in raw.split(","):
        ...
        if "=" not in entry:
            warnings.warn(
                f"KETOS_LOG_LEVELS: ignoring {entry!r} (expected 'name=LEVEL')",
                stacklevel=2,
            )
            continue
        ...
        if not name:
            warnings.warn(
                f"KETOS_LOG_LEVELS: ignoring {entry!r} (empty logger name)",
                stacklevel=2,
            )
            continue
        ...
        if numeric is None:
            warnings.warn(
                f"KETOS_LOG_LEVELS: ignoring {entry!r} (unknown level {level_str!r}, "
...
def _forward_loguru_message(message) -> None:
    """Forward Loguru messages through Ketos's configured structlog pipeline."""
...
def setup_loguru_logger(log_level: str, *, enqueue: bool = False) -> None:
    """Route Loguru's default logger through Ketos logging."""
...
def setup_log_file(log_file: Path, *, max_bytes: int, formatter: logging.Formatter | None = None) -> None:
    """Set up Ketos's rotating file handler.
...
def configure(
    *,
    ...
    # rebuilds from scratch. Env-only toggles (e.g. KETOS_PRETTY_LOGS) are not
    ...
    # Add callsite information only when KETOS_DEV is set
    if DEV:
        processors.append(
