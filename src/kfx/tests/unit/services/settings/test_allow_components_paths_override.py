...
def _clear_env(monkeypatch):
    for var in (
        "KETOS_ALLOW_CUSTOM_COMPONENTS",
        "KETOS_ALLOW_COMPONENTS_PATHS_OVERRIDE",
        "KETOS_COMPONENTS_PATH",
        "KETOS_COMPONENTS_INDEX_PATH",
    ):
...
def test_env_paths_bypass_when_override_true(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        monkeypatch.setenv("KETOS_ALLOW_COMPONENTS_PATHS_OVERRIDE", "true")
        monkeypatch.setenv("KETOS_COMPONENTS_PATH", tmp)
        monkeypatch.setenv("KETOS_COMPONENTS_INDEX_PATH", "/nonexistent/index.json")
...
def test_index_path_preserved_when_env_var_unset(monkeypatch):
    """The index path is only stripped when it came from KETOS_COMPONENTS_INDEX_PATH.
...
def test_multi_path_env_var_stripped_when_override_false(monkeypatch):
    """Comma-separated KETOS_COMPONENTS_PATH entries are each stripped when override is off."""
    _clear_env(monkeypatch)
    with tempfile.TemporaryDirectory() as tmp1, tempfile.TemporaryDirectory() as tmp2:
        monkeypatch.setenv("KETOS_ALLOW_CUSTOM_COMPONENTS", "false")
        monkeypatch.setenv("KETOS_ALLOW_COMPONENTS_PATHS_OVERRIDE", "false")
        monkeypatch.setenv("KETOS_COMPONENTS_PATH", f"{tmp1},{tmp2}")
