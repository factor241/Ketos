"""Scaffolder for ``kfx extension init``.

Writes the *basic* single-Bundle template at the requested directory:

    <target>/
        extension.json     -- v1 manifest with ``$schema`` pointer
...
from kfx.extension.manifest import _EXTENSION_ID_RE, EXTENSION_SCHEMA_URL
...
from kfx.extension.manifest import BUNDLE_NAME_RE as _BUNDLE_NAME_RE
...
class InitOptions:
    ...
    display_name: str
    """Human-readable display name shown in Ketos's palette."""
...
class InitResult:
    ...
    Mirrors the loader's :class:`~kfx.extension.LoadResult` shape so the
...
def _manifest_payload(options: InitOptions) -> dict:
    ...
    return {
        ...
        "description": (
            f"Auto-generated by `kfx extension init`. Replace this description "
...
Replace ``build`` with your real logic.  See the Ketos Component docs
at https://docs.ketos.test/components for input/output options.
"""
...
def _test_source(options: InitOptions) -> str:
    ...
    a constructed instance.  It does NOT depend on Ketos's full graph
...
A Ketos Extension generated by `kfx extension init`.

## Layout

```
{options.target.name}/
├── extension.json                 # v1 manifest
├── README.md
├── components/
│   └── {options.bundle_name}/
│       ├── __init__.py
...
> **Prerequisite**: `ketos` (which brings `kfx`) must be installed in
> your active environment before running any of the commands below or
> executing the generated `pytest` suite.  Install it with
> `pip install ketos` or your package manager of choice.

```bash
# Validate the manifest + bundle without executing imports
kfx extension validate .

# Run the bundled smoke tests (requires ketos / kfx)
pytest

# Launch a Ketos dev server with this extension loaded
kfx extension dev .
```

After `kfx extension dev`, edit any file under `components/` and click
**Reload** in the Ketos palette to pick up the change.
...
def _pyproject_payload(options: InitOptions) -> str:
    ...
    ``pip install -e .`` after ``kfx extension init``; without this file
    that step fails with ``Neither 'setup.py' nor 'pyproject.toml' found``.

    The ``[tool.ketos.extension]`` entry mirrors the ``extension.json``
    ...
    """
    distribution_name = f"kfx-{options.extension_id}".replace("_", "-")
...
version = "0.1.0"
description = "{options.display_name} -- a Ketos Extension."
