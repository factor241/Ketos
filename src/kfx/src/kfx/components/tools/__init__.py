...
_dynamic_imports = {
    ...
    # so existing flows / imports referencing kfx.components.tools keep working.
...
def __getattr__(attr_name: str) -> Any:
    ...
    # moved to kfx.components.files_and_knowledge.
    if attr_name in {"filesystem", "_filesystem_isolation", "_filesystem_namespace"}:
        from importlib import import_module

        result = import_module(f"kfx.components.files_and_knowledge.{attr_name}")
