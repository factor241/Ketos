...
KFX now supports a pluggable service architecture that allows you to customize and extend service implementations without modifying core code.
...
KFX also supports **adapter registries** -- collections of swappable implementations that share the same protocol.
...
Each adapter type exposes a typed accessor in `kfx.services.deps`:

```python
from kfx.services.deps import get_deployment_adapter
...
```python
from kfx.services import register_adapter
...
`kfx.toml` (preferred when both files exist):
...
Entry-point groups follow the naming convention `kfx.<adapter_type>.adapters` (e.g. `kfx.deployment.adapters`).

```toml
[project.entry-points."kfx.deployment.adapters"]
local = "my_package.deployment:LocalAdapter"
remote = "my_package.deployment:RemoteAdapter"
```
...
Entry-point groups and config section paths are derived from the `AdapterType` value automatically (e.g. `kfx.deployment.adapters` and `[deployment.adapters]`).
...
When adapters are part of core KFX, prefer namespacing them under `services/adapters`:
...
- **KFX owns shared slot primitives and slot taxonomy**:
  - `kfx.services.adapters.payload` defines shared primitives (`PayloadSlot`, `ProviderPayloadSchemas`)
  - each adapter domain in kfx defines canonical slot names once (for example deployment slots in
    `kfx.services.adapters.deployment.payloads`)
- **Adapters (KFX side) own adapter-facing payload models**:
  - adapters populate their `*PayloadSchemas` registry with adapter-side models
- **API hosts (for example Ketos) own API-facing payload models**:
...
- KFX defines deployment slot taxonomy and adapter registry (`DeploymentPayloadFields` / `DeploymentPayloadSchemas`)
- Ketos defines deployment API registry (`DeploymentApiPayloads`) and mapper behavior
...
Create an `kfx.toml` in your project root:
...
Then run:
```bash
kfx serve my_flow.json
```

Services will be automatically discovered from:
1. The current working directory (looking for `kfx.toml` or `pyproject.toml`)
...
```python
from kfx.services import register_service
from kfx.services.base import Service
from kfx.services.schema import ServiceType
...
```toml
[project.entry-points."kfx.services"]
database_service = "my_plugin.services:MyDatabaseService"
storage_service = "my_plugin.services:MyStorageService"
```

When your package is installed, KFX will automatically discover these services.

## Configuration Details

### Config File Locations

KFX searches for configuration in this order:

1. `./kfx.toml` (project-specific config)
2. `./pyproject.toml` under `[tool.kfx.services]` (Python project integration)
...
3. Ensure config file is in the correct location:
   - If settings service exists: `{settings.config_dir}/kfx.toml`
   - Otherwise: `./kfx.toml` or `./pyproject.toml` in current directory
4. Enable debug logging: `export KETOS_LOG_LEVEL=DEBUG`

### Import Errors

**Error:** `ModuleNotFoundError: No module named 'ketos'`

**Solutions:**
1. Ensure ketos is installed: `pip install ketos`
2. Check if module path in config is correct
3. Verify the package is importable: `python -c "import ketos.services.database"`
...
See:
- `kfx.toml.example` - Example configuration file showing Ketos service registration
- `src/kfx/services/` - Minimal built-in service implementations (auth, telemetry, tracing, variable, storage, etc.)
  - Auth: `kfx.services.auth.base` (BaseAuthService) and `kfx.services.auth.service` (AuthService). Use `get_auth_service()` from `kfx.services.deps`. Override with `auth_service = "ketos.services.auth.service:AuthService"` in config for full JWT/API key auth.
- `src/backend/base/ketos/services/` - Full-featured Ketos services
