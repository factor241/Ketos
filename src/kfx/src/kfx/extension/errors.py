...
DOCS_BASE = "https://docs.ketos.test/extensions/errors"
    ...
    def __post_init__(self) -> None:
        if self.code not in ERROR_CODES:
            msg = (
                ...
                f"Add it to kfx.extension.errors.ERROR_CODES and add a "
...
_BRANCH_TEMPLATES: dict[str, str] = {
    "manifest-not-found": ("No extension.json or [tool.ketos.extension] entry found in {location}."),
    ...
    "multi-bundle-unsupported": (
        "Manifest declares more than one bundle entry; v1 supports exactly one bundle per extension."
    ),
    "duplicate-bundle-name": (
    ...
    "version-constraint-unsatisfied": (
        "Manifest at {location} declares kfx.compat={content!r}, which does not "
        "include this kfx package's BUNDLE_API_VERSION; refusing to load."
    ),
    ...
    "duplicate-inline-bundle": (
        "Inline bundle name {content!r} appears in multiple KETOS_COMPONENTS_PATH entries; "
    ...
    "extension-reload-disabled": (
        ...
        "Set KETOS_ENABLE_EXTENSION_RELOAD=true to enable it on a local-development install (Mode A)."
    ),
    "extension-events-keyspace-forbidden": (
