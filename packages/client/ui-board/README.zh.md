...
- **裁剪使用固定的世界边距** —— 窗口超出可见画布 480 世界单位后停止绘制，与其尺寸无关；该边距覆盖它的手柄与展开的聊天面板，隐藏期间窗口保持自身状态。
