"""kfx validate -- structural and semantic validation of Ketos flow JSON.
...
Validation levels (each level implies all levels below it):
    ...
    Level 1 - structural
        ...
        (nodes built with a different Ketos version than the one installed).

    Level 2 - components
        Every node's ``data.type`` references a component type that exists in
        the kfx component registry.
...
``kfx.cli.validation`` subpackage so that all existing imports continue
to work unchanged.
"""
...
# This keeps ``from kfx.cli.validate import ...`` working for all consumers
# (the CLI entry point in __main__.py, tests, etc.).
from kfx.cli.validation import (  # noqa: F401
    ValidationIssue,
    ValidationResult,
...
from kfx.cli.validation.core import (  # noqa: F401
    _LEVEL_COMPONENTS,
    _LEVEL_EDGE_TYPES,
