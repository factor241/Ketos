...
2. Config file (kfx.toml / pyproject.toml)
...
from kfx.services.config_discovery import (
    get_nested_section,
...
from kfx.utils.concurrency import KeyedMemoryLockManager

if TYPE_CHECKING:
    from kfx.services.base import Service
    from kfx.services.factory import ServiceFactory
...
class ServiceManager:
    ...
    async def teardown(self) -> None:
        ...
        try:
            from kfx.services.adapters.registry import teardown_all_adapter_registries
    ...
    def _discover_from_entry_points(self) -> None:
        ...
        try:
            eps = entry_points(group="kfx.services")
        except Exception:  # noqa: BLE001
            logger.exception(
                "Failed to enumerate entry points for group='kfx.services'. "
    ...
    def _discover_from_config(self, config_dir: Path) -> None:
        """Discover services from config files (kfx.toml / pyproject.toml)."""
        source = get_preferred_config_source(
            config_dir,
            kfx_root_path=("services",),
            pyproject_root_path=("tool", "kfx", "services"),
        )
        if source is None:
            return
    ...
    def _load_services_from_config(self, config_path: Path, root_path: tuple[str, ...]) -> None:
        ...
        if config_path.name == "kfx.toml" or services:
    ...
    def _register_service_from_path(self, service_key: str, service_path: str) -> None:
        ...
        Args:
            ...
            service_path: Import path (e.g., "ketos.services.database.service:DatabaseService")
        """
        try:
