"""kfx pull -- fetch flows from a remote Ketos instance to local files.

Downloads flows from a live Ketos instance, normalizes them for version
...
Pull all flows from staging to ``flows/``::
    ...
    kfx pull --env staging

Pull all flows in a named project::

    kfx pull --env staging --project "My RAG Pipeline"

Pull a single flow by UUID::

    kfx pull --env staging --flow-id <uuid>

Pull to a custom directory::

    kfx pull --env production --output-dir ./local-flows/
"""
