"""Generate requirements.txt from a Ketos flow JSON.

Analyzes a flow's component code and configuration to determine the minimal
set of PyPI packages needed to run that flow on a standalone KFX runner.

Uses ``importlib.metadata`` to dynamically resolve import names to PyPI
distribution names and to compute the transitive dependency tree of ``kfx``,
...
* **System-level dependencies** — Native libraries required by Python packages
  ...
  ``requirements.txt``.
* **``--kfx-package`` and transitive filtering** — The ``kfx_package`` parameter
  controls only the output name (e.g. ``kfx-nightly``).  The "already provided
  by kfx" filter always resolves against the ``kfx`` distribution installed in
...
# Import names that are internal to the kfx/ketos runtime and should
# never appear as separate requirements.
_INTERNAL_IMPORT_NAMES: frozenset[str] = frozenset({"kfx", "ketos", "ketos_base"})
...
@lru_cache(maxsize=1)
def _get_kfx_transitive_dists() -> frozenset[str]:
    """Compute the full transitive closure of distributions provided by kfx.
...
@lru_cache(maxsize=1)
def _get_kfx_provided_imports() -> frozenset[str]:
    """Build the set of import names transitively provided by kfx.
...
def _resolve_provider_packages(provider_name: str) -> set[str]:
    ...
    Parent classes (e.g. ``LCModelComponent``) are all part of kfx, so any
    imports they introduce are already in kfx's transitive dependency tree
    ...
    kfx_provided = _get_kfx_provided_imports()
...
def _resolve_embedding_provider_packages(provider_name: str) -> set[str]:
    ...
    the ``LanguageModelComponent``: its code field only imports from ``kfx``
    ...
    # Use install_hint if provided (handles internal module paths like kfx.base.models.*)
    if install_hint:
        return {install_hint}
        ...
        if source and isinstance(source, str):
            ...
            for imp in imports:
                ...
                if imp in STDLIB_MODULES:
                    continue
                # Skip kfx / ketos internal imports - kfx provides these
                ...
                # even if the import itself is provided by kfx, because the
                # extras may not be.
                if imp in MODULE_EXTRA_DEPS:
                    for extra in MODULE_EXTRA_DEPS[imp]:
                        packages.add(extra)

                # Skip imports already provided by kfx
                if imp in kfx_provided:
                    continue
...
def generate_requirements_from_flow(
    ...
    *,
    kfx_package: str = "kfx",
    include_kfx: bool = True,
    pin_versions: bool = True,
) -> list[str]:
    """Generate a requirements list from a Ketos flow JSON.

    Args:
        flow: Parsed Ketos flow JSON (dict).
        kfx_package: Name of the KFX package to include (e.g. ``"kfx"`` or
            ``"kfx-nightly"``).
        include_kfx: Whether to include the KFX package itself.
...
def generate_requirements_txt(
    ...
    *,
    ...
    """Generate requirements.txt content from a Ketos flow JSON.

    Args:
        flow: Parsed Ketos flow JSON (dict).
        kfx_package: Name of the KFX package to include.
        include_kfx: Whether to include the KFX package itself.
    ...
    reqs = generate_requirements_from_flow(
        flow,
        kfx_package=kfx_package,
        include_kfx=include_kfx,
        pin_versions=pin_versions,
    )
    lines = [
        "# Auto-generated requirements for Ketos flow",
...
def generate_requirements_from_file(
    ...
    *,
    ...
    Args:
        flow_path: Path to a Ketos flow JSON file.
        kfx_package: Name of the KFX package to include.
        include_kfx: Whether to include the KFX package itself.
        pin_versions: If True, pin each package to the currently installed
            version.
