...
Validate a named environment::
    ...
    kfx login --env staging

Validate inline credentials::

    kfx login --target https://ketos.example.com --api-key sk-abc123
...
def _api_key_env_name(env_name: str, environments_file: str | None) -> str | None:
    ...
    try:
        from kfx.config.environments import _find_config_file, _load_config
        ...
        from kfx.config.environments import _load_raw
...
def _probe_connection(client: Any, sdk: Any) -> tuple[bool, str, int]:
    ...
    try:
    ...
    except sdk.KetosAuthError:
        return False, "auth", 0
    except sdk.KetosConnectionError as exc:
        ...
        # local Ketos version exactly.  Treat this as a successful probe.
    ...
    *,
    ...
    if env_cfg.api_key is None:
        ...
        if key_env_name:
            ...
            warning_parts.append("  Add [bold]api_key_env: KETOS_<ENV>_API_KEY[/bold] to your config,")
    ...
    if not ok:
        if msg == "auth":
            ...
            if key_env_name:
                err_console.print("  1. Open Ketos → Settings → API Keys → Create a new key")
    ...
    console.print(
        Panel.fit(
            ...
            title="[bold blue]kfx login[/bold blue]",
            border_style="green",
        )
    )
    ...
    if env_cfg.name in ("__inline__", "__env__") and env_cfg.api_key:
        ...
        console.print(f"[dim]      api_key_env: KETOS_{env_display.upper()}_API_KEY[/dim]")
