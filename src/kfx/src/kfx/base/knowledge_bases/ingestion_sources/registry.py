...
Thin wrapper over :class:`kfx.services.adapters.registry.AdapterRegistry`
...
* **Entry point** under the ``kfx.ingestion_source.adapters`` group::

      [project.entry-points."kfx.ingestion_source.adapters"]
      box = "ketos_box.source:BoxSource"

* **Config file** — ``kfx.toml`` (preferred) or ``pyproject.toml``::
...
from kfx.base.knowledge_bases.ingestion_sources.base import KBIngestionSource, SourceType
...
def _ensure_discovered() -> None:
    ...
    Mirrors the pattern in :func:`kfx.services.deps.get_deployment_adapter`:
    ...
    with _discovery_lock:
        ...
        # kfx is used as a plain library (e.g. from tests) — the
