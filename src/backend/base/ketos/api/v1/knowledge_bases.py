...
from kfx.base.knowledge_bases.backends import BackendType, create_backend
...
from kfx.base.vectorstores.chroma_security import chroma_client_create_collection_kwargs
from kfx.log import logger
from pydantic import BaseModel, Field

from ketos.api.error_codes import ApiErrorCode, coded_http_error
from ketos.api.utils import CurrentActiveUser, ingestion_run_service, knowledge_base_service
from ketos.api.utils.kb_helpers import KBAnalysisHelper, KBIngestionHelper, KBStorageHelper
from ketos.api.utils.kb_metadata import parse_per_file_metadata, parse_user_metadata
from ketos.api.v1.schemas import TaskResponse
from ketos.schema.knowledge_base import (
    BulkDeleteRequest,
    ChunkInfo,
...
from ketos.services.database.models.knowledge_base.model import KnowledgeBaseRecord
from ketos.services.deps import get_job_service, get_settings_service, get_task_service
from ketos.services.jobs import DuplicateJobError
from ketos.services.jobs.service import JobService
from ketos.services.memory_base.kb_path_helpers import validate_kb_path
from ketos.services.task.service import TaskService
from ketos.utils.kb_constants import (
...
async def _guard_kb_action(
    *,
    ...
    ``KETOS_AUTHZ_ENABLED=true``, the helper falls back to scanning every
    ...
    """
    from ketos.services.authorization.utils import (
        _auth_context,
        _coerce_action,
        _resolve_authz_domain,
    )
    from ketos.services.database.models.user.crud import get_user_by_id
    from ketos.services.deps import get_authorization_service, session_scope
...
def _validate_kb_path_containment(kb_user_path: Path, kb_path: Path, kb_name: str, username: str) -> None:
    ...
    :func:`ketos.services.memory_base.kb_path_helpers.validate_kb_path`
...
async def _delete_remote_backend_collection(
    *,
    ...
    delete the KB from Ketos's UI at all. Before this, the backend
...
async def ingest_folder_to_knowledge_base(
    ...
    try:
        ...
        from ketos.api.utils.kb_metadata import (
