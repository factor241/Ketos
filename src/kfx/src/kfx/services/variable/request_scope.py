"""Request-scoped variables for kfx serve (and other in-process runners).
...
lookup table in a ContextVar so :class:`~kfx.services.variable.service.VariableService`
can resolve the same names as the ``kfx run`` subprocess path.
...
def activate_request_variables(variables: dict[str, str] | None) -> contextvars.Token[dict[str, str] | None]:
    ...
    ``KETOS_REQUEST_VARIABLES`` env var. An empty ``{}`` is an *active* empty
