...
def _resolve(env_name: str, default: Path, *, create: bool = False) -> Path:
...
def ketos_config_dir(*, create: bool = False) -> Path:
    return _resolve("KETOS_CONFIG_DIR", _platform_roots()[0], create=create)


def ketos_data_dir(*, create: bool = False) -> Path:
    return _resolve("KETOS_DATA_DIR", _platform_roots()[1], create=create)


def ketos_cache_dir(*, create: bool = False) -> Path:
    return _resolve("KETOS_CACHE_DIR", _platform_roots()[2], create=create)


def ketos_temp_dir(*, create: bool = False) -> Path:
    return _resolve("KETOS_TEMP_DIR", _platform_roots()[3], create=create)
