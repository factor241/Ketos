...
- Window frames are registered once per `WindowKind` (`agent` and `clone` → `AgentCard`, which adds the chats-panel and fullscreen features, `connectors`/`settings`/`dashboard`/`tasks` → `WindowFrame` itself); the instance travels in the owner share, so two windows of one type render from one registration.
- `board.window.body` is keyed by `WindowBodyKind` and ships `conversation` (the session lane and composer of an agent window) and `clone` (the clone card editor); every other body kind renders an empty region. Switching a window's `bodyKind` through the store's `setWindowBodyKind` swaps the rendered occupant.
...
Trade-offs accepted: frames for kinds whose body has no occupant yet (`connectors`, `dashboard`, `settings`, `tasks`) render an empty body region, because the stages that own those panes have not landed; and each keyed seat declares its share twice — once as the common `owner` and once in the mapped `keyProps` table — because the catalog's lexical scan reads owner props from `owner` while the closed key domain requires the keyed table.
