...
request-scoped variables (injected via kfx serve ``global_vars``) must still
...
def variable_service_registered():
    ...
    Standalone kfx has no VariableService factory, so ``get_variable_service()``
...
# ---------------------------------------------------------------------------
# get_api_key_for_provider — env gating (no VariableService, == standalone kfx)
# ---------------------------------------------------------------------------
...
def test_literal_key_returned_as_is_regardless_of_flag():
    ...
    literal = "REDACTED_OPENAI_API_KEY"
...
def test_env_if_allowed_gates_connection_config(monkeypatch):
    ...
    from kfx.base.models.unified_models.instantiation import _env_if_allowed
