...
Manifest source forms (both supported):
    ...
    2. ``[tool.ketos.extension]`` in ``pyproject.toml``.
...
if sys.version_info >= (3, 11):
    ...
    import tomli as tomllib  # 3.10 fallback (kfx already depends on tomli)
...
"""The integer version of the BUNDLE_API.md contract that this kfx package
implements.  Manifests declare the contract versions they support via
``kfx.compat`` (a list of stringified integers); a manifest that does not
...
class LocaleBundle(BaseModel):
    ...
    start with ``components.``.  Ketos composes the global namespace from
...
class ExtensionManifest(BaseModel):
    ...
    name: StrictStr = Field(
        ...
        min_length=1,
        max_length=200,
        description="Human-readable display name shown in Ketos.",
    )
...
def _read_pyproject_extension(path: Path) -> dict[str, Any] | None:
    """Read ``[tool.ketos.extension]`` from a pyproject.toml.
    ...
    if not isinstance(tool, dict):
        return None
    ketos = tool.get("ketos")
    if not isinstance(ketos, dict):
        return None
    section = ketos.get("extension")
    if section is None:
        return None
    if not isinstance(section, dict):
        msg = "[tool.ketos.extension] must be a TOML table"
        raise TypeError(msg)
    return section
...
def load_manifest(root: Path | str) -> ManifestSource:
    ...
    msg = (
        ...
        f"'extension.json' or a [tool.ketos.extension] section in 'pyproject.toml'."
    )
    raise FileNotFoundError(msg)
