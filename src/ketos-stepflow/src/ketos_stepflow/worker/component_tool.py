"""Component for creating tool wrappers from Ketos components."""
...
async def component_tool_executor(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Create a tool wrapper from Ketos component code and inputs.

    This component takes a Ketos component's code/JSON and static inputs,
    ...
    try:
        ...
        for field_name, field_data in template.items():
            if isinstance(field_data, dict) and field_data.get("tool_mode", False):
                ...
                prop: dict[str, Any] = {
                    "type": _map_ketos_type_to_json_schema(field_type),
...
def _map_ketos_type_to_json_schema(ketos_type: str) -> str:
    """Map Ketos field types to JSON Schema types.

    Args:
        ketos_type: Ketos field type (e.g., "str", "int", "bool")
    ...
    return type_mapping.get(ketos_type, "string")
