"""Input handler that coerces Ketos Message objects to plain strings."""
...
class StringCoercionInputHandler(InputHandler):
    """Coerce Ketos Message objects to their ``.text`` for string fields.

    Matches template fields with ``type == "str"`` whose runtime value is a
    Ketos ``Message`` instance. Extracts the ``text`` attribute so the
    component receives a plain string as expected.

    This is needed because kfx components (1.6.4+) are stricter about type
