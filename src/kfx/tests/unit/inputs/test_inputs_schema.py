...
import pytest
from kfx.inputs.inputs import BoolInput, DictInput, FloatInput, InputTypes, IntInput, MessageTextInput, NestedDictInput
...
from kfx.template import Input, Output
from kfx.template.field.base import UNDEFINED
from kfx.type_extraction.type_extraction import post_process_type
...
def test_schema_to_ketos_inputs():
    ...
    # Convert schema to Ketos inputs
    inputs = schema_to_ketos_inputs(TestSchema)
...
def test_schema_to_ketos_inputs_sets_input_types_for_connectable_handles():
    """Non-string schema fields must carry input_types so the frontend renders connection handles.

    Regression test for https://git.ketos.test/ketos/ketos/issues/9424:
    MCP Tool components did not expose input ports for number, bool, or dict/JSON fields
    because schema_to_ketos_inputs left input_types unset, and the frontend's
    computeDisplayHandle hides handles for KETOS_SUPPORTED_TYPES with no input_types.
    """
    ...
    inputs = {inp.name: inp for inp in schema_to_ketos_inputs(ConnectableSchema)}
...
def test_schema_to_ketos_inputs_sets_data_input_types_for_nested_dict():
    ...
    inputs = {inp.name: inp for inp in schema_to_ketos_inputs(model)}

    assert isinstance(inputs["config"], NestedDictInput)
    assert inputs["config"].input_types == ["JSON"]


def test_schema_to_ketos_inputs_preserves_optional_defaults_and_nullable_objects():
    schema = {
        "type": "object",
        "properties": {
            "task": {"type": "string"},
    ...
    inputs = {input_.name: input_ for input_ in schema_to_ketos_inputs(model)}
...
def test_schema_to_ketos_inputs_invalid_type():
    ...
    with pytest.raises(TypeError, match="Unsupported field type:"):
        schema_to_ketos_inputs(InvalidSchema)
