...
@pytest.fixture
def ketos_cache_dir(tmp_path):
    """Create a temporary ketos cache directory."""
    cache_dir = tmp_path / "ketos"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(ketos_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = ketos_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("ketos"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Ketos")
    assert message.text == "Hello, Ketos!"

    # The base Message class in kfx doesn't support prompt serialization
    # This functionality is only available in the enhanced message class
    pytest.skip("Prompt serialization not supported in kfx base Message class")
...
def test_message_with_multiple_images(sample_image, ketos_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("ketos")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
