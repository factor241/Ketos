...
from ketos.api.error_codes import ApiErrorCode, coded_http_error
from ketos.api.utils import CurrentActiveUser, DbSession, DbSessionReadOnly
from ketos.api.v1.mappers.deployments import get_deployment_mapper
from ketos.api.v1.mappers.deployments.helpers import (
...
from ketos.services.adapters.deployment.context import deployment_provider_scope
from ketos.services.authorization import DeploymentAction, ensure_deployment_permission, filter_visible_resources
from ketos.services.authorization.utils import _resolve_authz_domain
from ketos.services.database.models.deployment.crud import (
...
from ketos.services.database.models.deployment_provider_account.crud import (
...
from ketos.services.deps import get_authorization_service, get_telemetry_service
from ketos.services.telemetry.schema import DeploymentPayload
...
DeploymentProviderAccountIdQuery = Annotated[
    UUID,
    Query(description="Ketos DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentProviderAccountIdPath = Annotated[
    UUID,
    Path(description="Ketos DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentIdPath = Annotated[
    UUID,
    Path(description="Ketos DB deployment UUID (`deployment.id`)."),
]
...
DeploymentIdQuery = Annotated[
    UUID,
    Query(description="Ketos DB deployment UUID (`deployment.id`)."),
]
IncludeProviderDeleteQuery = Annotated[
    ...
    Query(
        description=(
            ...
            "When false, only the local Ketos DB row is removed; "
...
async def _delete_local_deployment_row_with_commit_retry(
    *,
    ...
    stale Ketos row that still blocks later reads or provider-account
    deletion.
    """
    try:
    ...
    if existing_resource_key is not None:
        ...
        if existing_deployment is not None:
            raise coded_http_error(
                ...
                detail=f"The agent '{existing_resource_key}' is already managed by Ketos. "
...
async def list_deployments(
    ...
    *,
    load_from_provider: Annotated[
        bool,
        Query(
            description=("When true, list deployments directly from the provider (bypassing Ketos deployment rows).")
        ),
    ] = False,
    flow_version_ids: Annotated[
        ...
        Query(
            description=(
                "Optional Ketos flow version ids (pass as repeated query params, "
...
async def update_snapshot(
    ...
    *,
    ...
    ``provider_snapshot_id``.  Only Ketos-tracked snapshots (those with
    a ``flow_version_deployment_attachment`` row) can be updated.
    """
    from ketos.services.database.models.deployment.crud import get_deployment as get_deployment_row
    from ketos.services.database.models.flow_version.crud import get_flow_version_entry
