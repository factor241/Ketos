...
from kfx.cli.script_loader import find_graph_variable, load_graph_from_script
from kfx.cli.serve_app import FlowAlreadyRegisteredError, FlowMeta, FlowRegistry, create_multi_serve_app
from kfx.load import load_flow_from_json
from kfx.utils.flow_envelope import merge_flow_envelope, split_flow_envelope
...
def _gate_flow_for_serve(
    ...
    *,
    ...
    graph (the same gate ``kfx run`` uses), then re-attaches the envelope so the result is
    ...
    """
    from kfx.upgrade.cli_gate import UpgradeFlowError, apply_upgrade_gate
...
def serve_command(
    ...
    flow_dir: Path | None = typer.Option(
        ...
        help=(
            ...
            "Use /tmp/kfx-flows for single-pod sharing or a PVC mount for cross-pod. "
    ...
    *,
    ...
    try:
        api_key = get_api_key()
        verbose_print("KETOS_API_KEY is configured")
    except ValueError as e:
        typer.echo(str(e), err=True)
        typer.echo("Set the KETOS_API_KEY environment variable before serving.", err=True)
        raise typer.Exit(1) from e
    ...
    os.environ["KETOS_PRETTY_LOGS"] = "false"
    configure(log_level=log_level)
    ...
    if workers > 1 and flow_dir is not None and script_paths:
        # With --flow-dir, workers skip KFX_SERVE_STARTUP_PATHS and rely on warm_from_store().
        ...
        # file directly from KFX_SERVE_STARTUP_PATHS, which works fine.
    ...
    try:
        ...
        console.print(
            Panel.fit(
                f"[bold green]KFX Server Ready[/bold green]\n\n"
                ...
                title="[bold blue]KFX Server[/bold blue]",
                border_style="blue",
            )
        )
        console.print()
        ...
        try:
            if workers > 1:
                ...
                try:
                    uvicorn.run(
                        "kfx.cli.serve_app:create_serve_app",
                        host=host,
                        port=port,
                        workers=workers,
                        log_level=log_level,
                    ...
                    # any KFX_SERVE_* var the operator intentionally exported before launch.
...
async def _load_graph_and_meta(
    ...
    *,
    ...
    graph.prepare()
    if check_variables:
        from kfx.cli.validation import validate_global_variables_for_env
