...
import httpx
import pytest
from ketos.utils.version import (
...
class TestGetVersionInfo:
    ...
    @patch("ketos.utils.version.metadata")
    def test_get_version_info_ketos_package(self, mock_metadata):
        """Test getting version info for ketos package."""
        ...
        assert result["package"] == "Ketos"

    @patch("ketos.utils.version.metadata")
    def test_get_version_info_ketos_base_package(self, mock_metadata):
        """Test getting version info for ketos-base package."""
        ...
        assert result["package"] == "Ketos Base Nightly"
    ...
    def test_get_version_info_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["ketos", "ketos-base"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "ketos-nightly":
                return "1.0.0.dev456"
            raise mock_metadata.PackageNotFoundError
        ...
        assert result["main_version"] == "1.0.0"
        assert result["package"] == "Ketos Nightly"
    ...
    def test_get_version_info_base_nightly_package(self, mock_metadata):
        ...
        def mock_version(pkg_name):
            if pkg_name in ["ketos", "ketos-base", "ketos-nightly"]:
                raise mock_metadata.PackageNotFoundError
            if pkg_name == "ketos-base-nightly":
                return "1.0.0.a1"
            raise mock_metadata.PackageNotFoundError
    ...
    def test_get_version_info_canonical_dev_is_nightly(self, mock_metadata):
        """A canonical ``ketos`` install at a ``.dev`` version is labeled a nightly.

        The nightly now publishes as ``ketos==X.Y.Z.devN`` (the canonical name matches first),
...
class TestFetchLatestVersion:
    ...
    @patch("ketos.utils.version.httpx")
...
class TestGetVersionInfoFunction:
    ...
    @patch("ketos.utils.version.VERSION_INFO")
    def test_get_version_info_returns_version_info(self, mock_version_info):
        """Test that get_version_info returns VERSION_INFO."""
        mock_version_info = {"version": "1.0.0", "main_version": "1.0.0", "package": "Ketos"}

        with patch("ketos.utils.version.VERSION_INFO", mock_version_info):
