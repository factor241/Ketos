...
from ketos.schema.schema import InputType, OutputType, OutputValue
from ketos.serialization.serialization import get_max_items_length, get_max_text_length, serialize
from ketos.services.database.models.api_key.model import ApiKeyRead
from ketos.services.database.models.base import orjson_dumps
from ketos.services.database.models.flow.model import FlowCreate, FlowRead
from ketos.services.database.models.user.model import UserRead
from ketos.services.tracing.schema import Log
from ketos.utils.i18n import DEFAULT_LOCALE, SUPPORTED_LOCALES
...
class SimplifiedAPIRequest(BaseModel):
    ...
    user_id: str | None = Field(
        ...
        description=(
            ...
            "API key owner remains the effective Ketos user."
        ),
    )
...
class BaseConfigResponse(BaseModel):
    ...
    # but ``kfx extension dev`` and ``--env-file KETOS_ENABLE_EXTENSION_RELOAD=true``
    # opt in after the build is frozen, so the UI consults this field too.
    enable_extension_reload: bool
    # Mirrors ``KETOS_AUTHZ_ENABLED``. EE/custom frontends gate the Access
