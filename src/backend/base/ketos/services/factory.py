...
def import_all_services_into_a_dict():
    # Services are all in ketos.services.{service_name}.service
    ...
    for service_type in ServiceType:
        try:
            ...
            # Special handling for mcp_composer which is now in kfx module
            if service_name == "mcp_composer":
                module_name = f"kfx.services.{service_name}.service"
            else:
                module_name = f"ketos.services.{service_name}.service"
    ...
    # Import settings and auth bases from kfx (used in type hints but not ketos Service subclasses)
