#!/usr/bin/env python3
"""Example Ketos Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Ketos load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example Ketos Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 Ketos Load Testing Example Workflow")
    ...
    setup_script = script_dir / "ketos_setup_test.py"
    runner_script = script_dir / "ketos_run_load_test.py"
    ...
    try:
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-ketos",
                "--html",
                "ketos_load_test_report.html",
                "--csv",
                "ketos_load_test_results",
            ],
        ...
        print("   python setup_ketos_test.py --interactive")
        print()
        ...
        print("   - ketos_load_test_report.html (detailed HTML report)")
        print("   - ketos_load_test_results_*.csv (CSV data files)")
        print("   - ketos_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - ketos_load_test_error_summary_*.json (error analysis)")
        print("   - ketos_server_logs_during_test_*.log (Ketos server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Ketos UI")
        ...
        if args.auto:
            ...
            print("   - ketos_load_test_report.html")
            print("   - ketos_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "ketos_load_test_report.html",
                    "ketos_load_test_results_failures.csv",
                    "ketos_load_test_results_stats.csv",
                    "ketos_load_test_results_stats_history.csv",
                    "ketos_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("ketos_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("ketos_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("ketos_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Ketos load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open ketos_load_test_report.html in your browser for detailed analysis")
        print("   • Check ketos_load_test_results_*.csv for raw data")
        print("   • Review ketos_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze ketos_load_test_error_summary_*.json for error patterns")
        print("   • Examine ketos_server_logs_during_test_*.log for server-side issues")
