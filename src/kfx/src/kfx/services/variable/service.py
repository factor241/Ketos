"""Minimal variable service for kfx package with in-memory storage and environment fallback."""
...
class VariableService(Service):
    ...
    This is a lightweight implementation for KFX that maintains in-memory
    ...
    def _normalize_global_var_key(name: str) -> str:
        return f"x-ketos-global-var-{name.lower().replace('_', '-')}"

    @staticmethod
    def _get_request_variables() -> dict[str, str]:
        """Request-scoped variables from serve ContextVar or KETOS_REQUEST_VARIABLES env."""
        active = get_active_request_variables()
        if active is not None:
            return active

        # KETOS_REQUEST_VARIABLES is a process-wide env var, so reading it is an
        ...
        try:
            ...
            logger.debug("Invalid KETOS_REQUEST_VARIABLES JSON; skipping request-scoped lookup")
            return {}
        if not isinstance(parsed, dict):
            logger.debug("KETOS_REQUEST_VARIABLES must be a JSON object; skipping request-scoped lookup")
            return {}
    ...
    async def get_variable(self, name: str, **kwargs) -> str | None:  # noqa: ARG002
        ...
        Resolution order (first match wins):
          ...
          2. Request-scoped exact name (serve ContextVar or ``KETOS_REQUEST_VARIABLES``).
          3. Request-scoped ``x-ketos-global-var-*`` alias.
          4. Environment variable (exact name).
          5. Environment variable (``x-ketos-global-var-*`` alias).
        ...
        or the ``x-ketos-global-var-*`` form — always beats a same-named variable left
