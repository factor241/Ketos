...
def _unwrap_ketos_json_value(value: Any) -> Any:
    """Return the payload dict from Ketos JSON/Data values wired into MCP object parameters."""
...
def _try_convert_value(value: Any, expected_type: type, field_name: str, tool_name: str) -> Any:
    ...
    if expected_type in (dict, list):
        value = _unwrap_ketos_json_value(value)
...
async def get_flow_snake_case(flow_name: str, user_id: str, session, *, is_action: bool | None = None):
    try:
        ...
        msg = "Ketos Flow model is not available. This feature requires the full Ketos installation."
...
# TODO(ketos-ai/ketos#12541-followup): MCPSessionManager lives in this
...
class MCPStdioClient:
    ...
    async def _connect_to_server(self, command_str: str, env: dict[str, str] | None = None) -> list[StructuredTool]:
        ...
        # interpreter is ever interposed between Ketos and the server process.
