...
2. **Scans KFX files**: Finds all `.py` files in `src/kfx/src/kfx/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/ketos --cov-config=.coveragerc
```
...
- **Input**: `src/kfx/src/kfx/components/**/*.py` (legacy components)
