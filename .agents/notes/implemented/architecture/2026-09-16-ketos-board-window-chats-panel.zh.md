...
**几何是窗口矩形的函数。** `window/panel-geometry.ts` 用世界单位从 `BoardWindowState` 推导矩形：窗口态取窗口宽度的 26%（下限 220px、上限 320px），高度 92% 并垂直居中，再把自身宽度的 30% 掖到外框左缘之下，使可见条带约为窗口面积的正分之一；全屏态则贴着看板面板左缘、占满高度。由于面板位于被变换的画布表面内部，拖动、缩放、平移与缩放画布都无需面板写任何代码。
