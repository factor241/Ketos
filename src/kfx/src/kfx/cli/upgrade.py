"""Implementation of `kfx upgrade <flow.json>`."""
...
from kfx.upgrade.checker import CompatibilityReport, build_registry_lookup, check_flow_compatibility
...
def load_registry_from_index() -> dict[str, Any]:
    ...
    if not blob or "entries" not in blob:
        typer.echo(
            ...
            "cannot be checked. Try reinstalling kfx (e.g. `pip install --force-reinstall kfx`).",
            err=True,
        )
        raise typer.Exit(1)
...
def upgrade_command(
    ...
    *,
    ...
    # Exported Ketos flows may have an outer envelope:
