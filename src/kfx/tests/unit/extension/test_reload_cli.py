"""Tests for the reload CLI: HTTP client + ``kfx extension reload``."""

from __future__ import annotations

import pytest
from kfx.cli._extension_reload_client import (
    DEFAULT_TARGET,
    ReloadHttpResponse,
    reload_via_http,
    resolve_api_key,
...
def test_resolve_target_prefers_explicit(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("KETOS_HOST", "http://from-env")
    assert resolve_target("http://explicit/") == "http://explicit"


def test_resolve_target_uses_ketos_host(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("KETOS_HOST", "http://from-env/")
    monkeypatch.delenv("KETOS_SERVER_URL", raising=False)
...
def patched_httpx(monkeypatch: pytest.MonkeyPatch) -> dict[str, _StubClient | None]:
    ...
    def _factory_for(response: _StubResponse) -> None:
        ...
        monkeypatch.setattr(
            "kfx.cli._extension_reload_client.httpx.Client",
...
def test_reload_via_http_success(patched_httpx: dict, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert captured["url"] == "http://server/api/v1/extensions/kfx-pilot/bundles/pilot/reload"
