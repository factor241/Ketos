...
Plugins register via the ``ketos.plugins`` entry-point group. They receive
a wrapper so they cannot overwrite or shadow existing Ketos routes.
"""
...
def _get_route_keys(app: FastAPI) -> set[tuple[str, str]]:
    ...
    routes cannot overwrite or shadow existing Ketos routes.
    """
...
def load_plugin_routes(app: FastAPI) -> None:
    ...
    Plugins register themselves via the ``ketos.plugins`` entry-point group.
