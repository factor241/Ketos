"""Tests for ``kfx.extension.dev_registry``.
...
def isolated_state_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    ``KETOS_DEV_EXTENSIONS_DIR`` to a tmp_path subdirectory.
    """
...
def test_register_is_idempotent_and_refreshes_timestamp(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import kfx.extension.dev_registry as dr
...
def test_dev_extension_component_paths_forwards_all_warnings(
    ...
    from kfx.extension import dev_registry as dr
...
def test_state_dir_env_var_takes_precedence(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("KETOS_DEV_EXTENSIONS_DIR", str(override))
    assert state_file_path().parent == override
    # The override is honored even when KETOS_CONFIG_DIR is also set.
    monkeypatch.setenv("KETOS_CONFIG_DIR", str(tmp_path / "ignore-me"))
    assert state_file_path().parent == override


def test_config_dir_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("KETOS_DEV_EXTENSIONS_DIR", raising=False)
    cfg = tmp_path / "cfg"
    monkeypatch.setenv("KETOS_CONFIG_DIR", str(cfg))
