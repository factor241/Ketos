...
def test_build_request_variables_parses_ketos_request_variables():
    merged = {"access_token": "from-json", "USER_ID": "u1"}
    global_vars = {
        "KETOS_REQUEST_VARIABLES": json.dumps(merged),
        "access_token": "from-raw-override",
        "x-ketos-global-var-access-token": "alias-value",
    }
    ...
    assert flat["x-ketos-global-var-access-token"] == "alias-value"


def test_build_request_variables_keeps_raw_keys_when_blob_is_malformed():
    """A malformed KETOS_REQUEST_VARIABLES blob is skipped; raw keys still merge."""
    global_vars = {
        "KETOS_REQUEST_VARIABLES": "{not valid json",
...
def test_build_request_variables_ignores_non_dict_blob():
    ...
    global_vars = {
        "KETOS_REQUEST_VARIABLES": json.dumps(["a", "b"]),
        "access_token": "raw-tok",
    }
    flat = build_request_variables_from_global_vars(global_vars)
    assert flat == {"access_token": "raw-tok"}
...
def test_build_request_variables_coerces_structured_blob_values():
    ...
    global_vars = {
        "KETOS_REQUEST_VARIABLES": json.dumps(
...
def test_normalize_parsed_variables_coercion_rules():
    ...
    from kfx.services.variable.request_scope import normalize_parsed_variables
    ...
    token = activate_request_variables(
        {
            ...
            "x-ketos-global-var-wxo-github-access-token": "tok-123",
        }
    )
    try:
