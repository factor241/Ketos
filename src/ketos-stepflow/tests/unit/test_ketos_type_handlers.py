"""Unit tests for KetosType input/output handlers and DataFrame output handler.

Tests serialization (output) and deserialization (input) of Ketos
...
from ketos_stepflow.worker.handlers import (
    DataFrameOutputHandler,
    KetosTypeInputHandler,
    KetosTypeOutputHandler,
)

# ---------------------------------------------------------------------------
# KetosTypeOutputHandler — matches()
# ---------------------------------------------------------------------------


class TestKetosTypeOutputHandlerMatches:
    def setup_method(self):
        self.handler = KetosTypeOutputHandler()
...
# ---------------------------------------------------------------------------
# KetosTypeOutputHandler — process()
# ---------------------------------------------------------------------------


class TestKetosTypeOutputHandlerProcess:
    def setup_method(self):
        self.handler = KetosTypeOutputHandler()
    ...
    async def test_serialize_message(self):
        ...
        assert result["__ketos_type__"] == "Message"
    ...
    async def test_serialize_data(self):
        ...
        assert result["__ketos_type__"] == "Data"
...
class TestDataFrameOutputHandlerMatches:
    ...
    def test_matches_kfx_dataframe(self):
...
class TestDataFrameOutputHandlerProcess:
    ...
    async def test_serialize_dataframe(self):
        ...
        assert result["__ketos_type__"] == "DataFrame"
...
class TestKetosTypeInputHandlerMatches:
    def setup_method(self):
        self.handler = KetosTypeInputHandler()

    def test_matches_dict_with_marker(self):
        value = {"__ketos_type__": "Message", "text": "hi"}
        assert self.handler.matches(template_field={}, value=value) is True

    def test_matches_list_with_marked_items(self):
        value = [
            {"__ketos_type__": "Data", "text": "row1"},
            {"__ketos_type__": "Data", "text": "row2"},
        ]
        assert self.handler.matches(template_field={}, value=value) is True
...
# ---------------------------------------------------------------------------
# KetosTypeInputHandler — prepare() (Message)
# ---------------------------------------------------------------------------


class TestKetosTypeInputHandlerMessage:
    def setup_method(self):
        self.handler = KetosTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_message(self):
        serialized = {"__ketos_type__": "Message", "text": "hello"}
    ...
    async def test_deserialize_message_empty_text(self):
        serialized = {"__ketos_type__": "Message", "text": ""}
        fields = {"msg": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# KetosTypeInputHandler — prepare() (Data)
# ---------------------------------------------------------------------------


class TestKetosTypeInputHandlerData:
    def setup_method(self):
        self.handler = KetosTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_data(self):
        serialized = {"__ketos_type__": "Data", "text": "some data"}
...
# ---------------------------------------------------------------------------
# KetosTypeInputHandler — prepare() (DataFrame)
# ---------------------------------------------------------------------------


class TestKetosTypeInputHandlerDataFrame:
    def setup_method(self):
        self.handler = KetosTypeInputHandler()
...
# ---------------------------------------------------------------------------
# KetosTypeInputHandler — prepare() (list handling)
# ---------------------------------------------------------------------------


class TestKetosTypeInputHandlerLists:
    def setup_method(self):
        self.handler = KetosTypeInputHandler()
    ...
    async def test_deserialize_list_of_messages(self):
        value = [
            {"__ketos_type__": "Message", "text": "msg1"},
            {"__ketos_type__": "Message", "text": "msg2"},
        ]
        fields = {"msgs": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_list_of_data(self):
        value = [
            {"__ketos_type__": "Data", "text": "d1"},
            {"__ketos_type__": "Data", "text": "d2"},
        ]
        fields = {"items": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_multiple_fields(self):
        ...
        fields = {
            "msg": ({"__ketos_type__": "Message", "text": "hello"}, {}),
            "data": ({"__ketos_type__": "Data", "text": "world"}, {}),
        }
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# KetosTypeInputHandler — unknown / edge cases
# ---------------------------------------------------------------------------


class TestKetosTypeInputHandlerEdgeCases:
    def setup_method(self):
        self.handler = KetosTypeInputHandler()

    @pytest.mark.asyncio
    async def test_unknown_ketos_type_returns_dict(self):
        """Unknown __ketos_type__ values should return the raw dict."""
        serialized = {"__ketos_type__": "UnknownType", "data": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)

        assert isinstance(result["field"], dict)
        assert result["field"]["__ketos_type__"] == "UnknownType"

    @pytest.mark.asyncio
    async def test_empty_ketos_type_returns_dict(self):
        """Empty __ketos_type__ value should return the raw dict."""
        serialized = {"__ketos_type__": "", "text": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
class TestKetosTypeRoundTrip:
    def setup_method(self):
        self.output_handler = KetosTypeOutputHandler()
        self.input_handler = KetosTypeInputHandler()
...
class TestDataFrameRoundTrip:
    ...
    deserialize with KetosTypeInputHandler."""
    ...
    async def test_dataframe_round_trip(self):
        ...
        serialized = await self.output_handler.process(original)
        assert serialized["__ketos_type__"] == "DataFrame"
...
class TestPandasDataFrameRoundTrip:
    ...
    DataFrames instead of kfx DataFrames, depending on the Ketos
