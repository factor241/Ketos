...
from kfx.inputs.validators import CoalesceBool
    ...
    def validate_value(cls, v: Any, info):
        ...
        if not isinstance(v, list):
            msg = (
                ...
                "- A Data object (Ketos's internal data structure)\n"
            )
        ...
        for i, item in enumerate(v):
            if not isinstance(item, dict | Data):
                msg = (
                    ...
                    "- A Data object (Ketos's internal data structure for passing data between components)\n"
...
class JSONInput(HandleInput, InputTraceMixin, ListableInputMixin, ToolModeMixin):
    ...
    This is the new standard input for Ketos data structures.
...
class ModelInput(BaseInputMixin, ModelInputMixin, ListableInputMixin, InputTraceMixin, ToolModeMixin):
    ...
    def normalize_value(cls, v):
        ...
        if isinstance(v, str) or (isinstance(v, list) and all(isinstance(item, str) for item in v)):
            ...
            try:
                from kfx.base.models.unified_models import normalize_model_names_to_dicts
...
class MessageInput(StrInput, InputTraceMixin):
    ...
    def _validate_value(v: Any, info):
        ...
        if isinstance(v, Message):
            # If it's from a different module (e.g., ketos.schema.Message),
...
class MessageTextInput(StrInput, MetadataTraceMixin, InputTraceMixin, ToolModeMixin):
    """Represents a text input component for the Ketos system.

    This component is used to handle text inputs in the Ketos system.
...
class BoolInput(BaseInputMixin, ListableInputMixin, MetadataTraceMixin, ToolModeMixin):
    ...
    def coerce_message_or_data(cls, v: Any):
        ...
        # CoalesceBool runs. See https://git.ketos.test/ketos/ketos/issues/9424
        if isinstance(v, Message):
            return v.text
...
class DictInput(BaseInputMixin, ListableInputMixin, InputTraceMixin, ToolModeMixin):
    ...
    def validate_value(cls, v: Any, info):
        ...
        # See https://git.ketos.test/ketos/ketos/issues/9424
        if isinstance(v, dict):
            return v
        if isinstance(v, Message):
            v = v.text
...
from kfx.template.field.base import Input  # noqa: E402
