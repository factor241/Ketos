...
from kfx.graph.flow_builder.flow import empty_flow
...
def _load_registry_user_aware() -> dict[str, dict]:
    ...
    Tries the ketos-side overlay (which reads the current_user_id
    ContextVar and walks ``<sandbox>/.components/*.py`` for that user).
    Falls back to the bare base registry when:
        - the ketos package isn't installed alongside kfx (e.g., the
          MCP server is running standalone),
        - no user is bound to the context.

    Keeps the kfx package free of a hard dependency on the ketos
