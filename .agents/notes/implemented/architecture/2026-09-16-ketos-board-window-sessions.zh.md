...
**Composer 由看板拥有且完整。** `ComposerBar` 在窗口内重建了参考聊天栏：agent 预设芯片（仅在会话为空时可切换；工作目录已移入窗口的聊天面板）、基于宿主命令目录的 `+` 操作菜单加上看板自有的文件与图片条目、带参数提示与描述的 `/` 命令弹层、来自 `remote.fileReferences.list` 与 `remote.sessionReferenceResolver.candidates` 的 `@` 提及行、权限芯片（完全访问走 `RiskConfirmation`）、基于 `ctx.modelDirectories` 且带推理等级子菜单的模型芯片、计划芯片、来自 `contextPressure` 投影的上下文圆环、目标/待办/队列条，以及带排队或引导的发送/停止。Markdown 通过共享的 `MarkdownText` 渲染；车道把 `legacy.nodes` 折叠为用户／assistant 正文、单行工具行，以及会话的运行中调用与失败状态（[阶段 6 备注](../feature/2026-09-18-ketos-chat-window-states-titles.zh.md)），标题栏的全屏开关就地放大窗口（[note](2026-09-16-ketos-board-window-fullscreen.zh.md)），不再导航离开。模拟文案及其栖身字段（`BoardWindowState` 上的 `status`、`statusText`、`contextUsed`、`sessionId`）被删除，而不是留着不用。
...
**Composer 只保留预设芯片。** 工作目录已从 composer 移入窗口的聊天面板：面板中的文件夹动作会把选中的目录注册为新聊天的项目文件夹；上下文行仍以单行承载 agent 预设。
...
- **复用 `ui-attachment` 的组件或对话附件面。** 否决：`ComposerAttachments` 与草稿注册表是包内部实现（公开的 `IConversation` 面不含草稿动词），因此看板在组件状态中保存图片附件并以行内提示片段发送；阶段 6 备注随后通过 `ctx.fileUpload` 暂存非图片文件，并把芯片保留在 composer 状态中。
...
- **复用 `ui-commands` 的菜单或其客户端贡献。** 否决：第二个注册者无法在共享触发源上占用 `/`，也无法重复注册宿主命令，因此看板读取 `remote.commands.list()` 并自持菜单文案，命令行通过带参数与草稿附件的 `remote.commands.execute` 执行（[阶段 6 备注](../feature/2026-09-18-ketos-chat-window-states-titles.zh.md)）。
...
agent 窗口成为真正的聊天，并拥有参考 composer 的整套控件：它拥有会话，能流式接收回答、发送、引导、停止，切换权限与模型，运行斜杠命令，附带图片，并可扩展为整个看板面板。车道刻意比主转录更薄——没有确认或提问 UI（按计划改为导航）、工具结果折叠为名称加失败标记，编辑器式提及芯片暂缓（提及插入纯文本），composer 自持 Enter 排队 / Cmd 引导策略；阶段 6 备注补充了完整的车道状态清单、文件暂存与带参数的命令执行。测试中的会话替身必须预先添加，因为 `TestSessions.add()` 通过 `act` 稳定状态，而从窗口的挂载 effect 调用会嵌套 act 作用域；生产环境中的 `create()` 是一次远程往返。
