...
from kfx.run._defaults import apply_run_defaults, resolve_fallback_to_env_vars
...
_KETOS_NAMESPACE_UUID = uuid.UUID("3c091057-e799-4e32-8ebc-27bc31e1108c")
...
async def execute_graph_with_capture(graph, input_value: str | None, session_id: str | None = None):
    ...
    Args:
        ...
        session_id: Optional session ID. ``None`` auto-generates one so that
            ...
            shell/env-var typos (see ``kfx.run._defaults.validate_provided_id``).
...
def flow_id_from_path(file_path: Path, root_dir: Path) -> str:
    ...
    return str(uuid.uuid5(_KETOS_NAMESPACE_UUID, relative))
...
def load_sdk(command_name: str) -> Any:
    """Lazily import ``ketos_sdk`` to keep CLI startup fast.
    ...
    try:
        ...
        msg = f"ketos-sdk is required for kfx {command_name}. Install it with: pip install ketos-sdk"
