"""Tests for manifest-first precedence over ``ketos.plugins``.
...
from kfx.extension.loader._plugins import canonicalize_distribution
...
def test_finds_manifest_shipping_distribution(tmp_path: Path) -> None:
    dist = make_installed_extension(tmp_path, "kfx-pilot")
    roots = installed_extension_roots(distributions=[dist])
    assert "kfx-pilot" in roots
    assert (roots["kfx-pilot"] / "extension.json").is_file()


def test_canonicalizes_distribution_name(tmp_path: Path) -> None:
    """PEP-503: ``Kfx_Pilot`` and ``kfx-pilot`` are the same distribution."""
    dist = make_installed_extension(tmp_path, "kfx-pilot")
    # Override the metadata Name to test the canonicalize path.
    dist._name = "KFX_Pilot"
    roots = installed_extension_roots(distributions=[dist])
    assert list(roots) == ["kfx-pilot"]
...
def test_finds_pyproject_only_distribution(tmp_path: Path) -> None:
    """A distribution shipping pyproject.toml with [tool.ketos.extension] is found."""
    dist = make_installed_pyproject_extension(tmp_path, "kfx-pyproject")
    roots = installed_extension_roots(distributions=[dist])
    assert "kfx-pyproject" in roots
    assert (roots["kfx-pyproject"] / "pyproject.toml").is_file()


def test_pyproject_without_extension_section_is_ignored(tmp_path: Path) -> None:
    """A pyproject.toml without [tool.ketos.extension] is NOT treated as a manifest.
...
def test_malformed_pyproject_section_surfaces_manifest_invalid(tmp_path: Path) -> None:
    """A pyproject with [tool.ketos.extension] missing required fields surfaces manifest-invalid.
    ...
    """
    from kfx.extension import load_installed_extensions

    dist = make_installed_pyproject_malformed_extension(tmp_path, "kfx-bad-pyproject")
    ...
    assert "kfx-bad-pyproject" in owners
    ...
    assert result.distribution == "kfx-bad-pyproject"
...
def test_malformed_pyproject_section_suppresses_legacy_component_entry_point(tmp_path: Path) -> None:
    ...
    its legacy ``ketos.plugins`` component entry-points must be
...
def test_extension_json_wins_over_pyproject(tmp_path: Path) -> None:
    ...
    (pkg_dir / "pyproject.toml").write_text(
        '[project]\nname = "kfx-both"\n[tool.ketos.extension]\nid = "kfx-both"\n',
        encoding="utf-8",
    )
    dist = FakeDist(
        name="kfx-both",
        root=tmp_path,
        files=[Path("kfx-both") / "extension.json", Path("kfx-both") / "pyproject.toml"],
    )
    roots = installed_extension_roots(distributions=[dist])
    # Both forms point at the same root directory, but the manifest path
    # used for resolution should be the JSON file.
    assert "kfx-both" in roots
    assert (roots["kfx-both"] / "extension.json").is_file()
...
def test_pyproject_only_extension_loads_at_official_slot(tmp_path: Path) -> None:
    ...
    Without this path the AC's "extension.json or [tool.ketos.extension]"
    ...
    bundle_dir = tmp_path / "kfx-pyproject" / "components"
    ...
    assert result.distribution == "kfx-pyproject"
    assert result.extension_id == "kfx-pyproject"
    # Bundle name comes from the pyproject manifest section.
    assert result.bundle == "kfx_pyproject"
...
def test_pyproject_only_distribution_suppresses_legacy_component_entry_point(tmp_path: Path) -> None:
    ...
    A legacy ``ketos.plugins`` component entry-point on a pyproject-form
    ...
    owners = manifest_owning_distributions(distributions=[dist])
    assert "kfx-pyproject" in owners
    kept, skipped = filter_component_entry_points([component_ep], skip=owners)
...
def test_owning_distributions_is_set_of_canonical_names(tmp_path: Path) -> None:
    ...
    assert owners == frozenset({"kfx-pilot"})
...
def test_skips_manifest_shipping_distribution(tmp_path: Path) -> None:
    ...
    """
    manifest_dist = make_installed_extension(tmp_path, "kfx-pilot")
...
def test_keeps_entry_points_without_distribution() -> None:
    ...
    kept, skipped = filter_plugin_entry_points([ep], skip=frozenset({"kfx-pilot"}))
    assert kept == [ep]
    assert skipped == []
...
def test_uses_real_distributions_by_default(tmp_path: Path) -> None:
    ...
    Ketos itself does not ship an extension manifest today, but if a
...
def test_canonicalize_distribution_matches_pep503() -> None:
    assert canonicalize_distribution("Kfx-Pilot") == "kfx-pilot"
    assert canonicalize_distribution("kfx_pilot") == "kfx-pilot"
    assert canonicalize_distribution("KFX...PILOT") == "kfx-pilot"
...
def test_component_filter_keeps_components_on_non_manifest_distribution(tmp_path: Path) -> None:
    ...
    The legacy ``ketos.plugins`` path is still the registration channel
