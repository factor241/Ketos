...
**The ring keeps its place in every state.** A window whose session exists renders the ring even before the first report: the arc is empty, the label is the localized `—`, and the tooltip is the empty-state text. A reading renders `% · used / window` with compact K/M units from the board dictionary, and the arc and the percent carry the warning or critical tint. The empty, warning, and critical states were verified in a live browser through the real projection path, with the stub provider reporting the token counts (0% → `normal`, 85% → `warning`, 96% → `critical`, unprompted windows → `empty`).
...
**Every asynchronous settle re-checks ownership of its window record.** A shared `owns(record, windowId, sessionId)` guard covers creation, chat creation, fork, and the model-directory load: a window closed or rebound while a call is in flight neither publishes its result nor recreates the record it left. The model-directory path in particular used to recreate a released record when its catalog load rejected after the window closed; a spec opens a window, releases it, rejects the load, and asserts the bridge holds no record and the dead channel stays frozen.
...
**The ring yields on narrow columns, verified in the live browser.** A dedicated probe (`.playwright-mcp/stage-14-multi-window-perf/probe-ring-narrow.mjs`) sweeps viewport widths with a docked chats panel and measures the card's content box — the box the container query reads: the ring is visible at 416px and hidden at 396px, the display decision matches the 405px rule at all nine sweep points, and a canvas zoom that changes the rendered size leaves the layout width and the decision untouched. The probe runs against a fresh home, since a restored layout or zoom would change the starting geometry.
...
Verification: `tests/context-ring.client.spec.ts` covers the formula against `contextOccupancy`, the 80/95 ladder, compact units (including `999 999` reading as `1M`), and the reading; `tests/conversation-body.client.spec.tsx` covers the four rendered states and that a reading update does not move the transcript lane; `tests/leaks.client.spec.tsx` covers the 30 mixed cycles, the release of every per-window subscription (`chat`, `session`, six projections, model directory), the late-load ownership guard, and outstanding timers; `tests/stress-sessions.client.spec.tsx` covers ten windows and five parallel streams with failure isolation and no crosstalk; `tests/window-title.client.spec.tsx` covers the chats-panel rename path, the refused rename, the blank-title fallback, and ten distinct dock and header titles with per-window statuses. `pnpm run test:gui`, `DSH_SNAPSHOT=replay pnpm run test:web`, `pnpm run typecheck`, `pnpm run lint`, `pnpm run duplication`, `pnpm run hygiene`, and `pnpm run doc-sync` are green; the live audit verdicts are `ok: true`.
