"""Core run functionality for executing Ketos graphs."""
...
from kfx.run._defaults import apply_run_defaults, resolve_fallback_to_env_vars, validate_provided_id
...
async def run_flow(
    ...
    *,
    ...
    """Execute a Ketos graph script or JSON flow and return the result.

    This function analyzes and executes either a Python script containing a Ketos graph,
    ...
    # --- upgrade compatibility gate (shared with `kfx serve`) ---
    if upgrade_flow and flow_dict is not None:
        ...
        # `kfx upgrade` uses). component_cache.all_types_dict is empty here because it is
    ...
    try:
        ...
        if flow_dict is not None:
            ...
            from kfx.load import aload_flow_from_json
        ...
        # don't fail. See kfx.run._defaults.apply_run_defaults for the full rationale.
    ...
    try:
        ...
        # See kfx.run._defaults.resolve_fallback_to_env_vars for why this flag is
        # plumbed through (mirrors ketos's API path so load_from_db variables
