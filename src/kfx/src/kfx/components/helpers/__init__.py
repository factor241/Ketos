...
if TYPE_CHECKING:
    from kfx.components.helpers.calculator_core import CalculatorComponent
    from kfx.components.helpers.create_list import CreateListComponent
    from kfx.components.helpers.current_date import CurrentDateComponent
    from kfx.components.helpers.id_generator import IDGeneratorComponent
    from kfx.components.helpers.memory import MemoryComponent
    from kfx.components.helpers.output_parser import OutputParserComponent
    from kfx.components.helpers.store_message import MessageStoreComponent
...
# This allows imports like: import kfx.components.helpers.current_date
_redirected_submodules = {
    "kfx.components.helpers.current_date": "kfx.components.utilities.current_date",
    "kfx.components.helpers.calculator_core": "kfx.components.utilities.calculator_core",
    "kfx.components.helpers.id_generator": "kfx.components.utilities.id_generator",
    "kfx.components.helpers.memory": "kfx.components.models_and_agents.memory",
}
...
def __getattr__(attr_name: str) -> Any:
    ...
    # e.g., kfx.components.helpers.id_generator -> kfx.components.utilities.id_generator
    if attr_name == "id_generator":
        from importlib import import_module

        result = import_module("kfx.components.utilities.id_generator")
        globals()[attr_name] = result
        return result
    if attr_name == "calculator_core":
        from importlib import import_module

        result = import_module("kfx.components.utilities.calculator_core")
        globals()[attr_name] = result
        return result
    if attr_name == "current_date":
        from importlib import import_module

        result = import_module("kfx.components.utilities.current_date")
        globals()[attr_name] = result
        return result
    if attr_name == "memory":
        from importlib import import_module

        result = import_module("kfx.components.models_and_agents.memory")
        globals()[attr_name] = result
        return result
    ...
    if attr_name in ("CurrentDateComponent", "CalculatorComponent", "IDGeneratorComponent"):
        from kfx.components import utilities
    ...
    if attr_name == "MemoryComponent":
        from kfx.components import models_and_agents
