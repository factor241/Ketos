"""Memory management for kfx with dynamic dispatch.

Routes memory operations to either the full ketos implementation (when
ketos is installed AND a real database service is registered) or the kfx
...
decision can't distinguish "ketos is importable" from "a real DB is wired",
and picking the ketos backend with a NoopDatabaseService yields silent
...
def _impl():
    if has_ketos_db_backend():
        from ketos import memory as impl
    else:
        from kfx.memory import stubs as impl
    return impl
