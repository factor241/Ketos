          ...
          {data && (
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 gap-2 rounded-md border border-border bg-card p-3 text-xs sm:grid-cols-2">
                <IdField
                  label={t("knowledge.runId")}
                  value={data.id}
                  testId="run-id"
                />
                <IdField
                  label={t("knowledge.jobId")}
                  value={data.job_id}
                  testId="job-id"
                />
              </div>
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="flex items-center gap-2 truncate">
                            <ForwardedIconComponent
                              name={iconName}
                              className={cn("h-3 w-3 shrink-0", toneClass)}
                            />
                        ...
                        {item.error_message && (
                          <div className="rounded-sm bg-error-red px-2 py-1 text-accent-red-foreground">
                            {t("knowledge.itemIngestionFailed")}
                          </div>
                        )}
                      </div>
                    );
...
interface IdFieldProps {
  ...
  testId: "run-id" | "job-id";
}

function IdField({ label, value, testId }: IdFieldProps) {
  return (
