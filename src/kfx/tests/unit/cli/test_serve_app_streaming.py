...
from kfx.cli.serve_app import FlowMeta, FlowRegistry, StreamRequest, create_multi_serve_app
...
def multi_serve_app(mock_graphs, mock_metas, monkeypatch):
    ...
    monkeypatch.setenv("KETOS_API_KEY", "test-api-key")
    monkeypatch.setattr(get_settings_service().settings, "allow_custom_components", True)

    with patch("kfx.cli.serve_app.execute_graph_with_capture") as mock_execute:
        ...
        from kfx.cli.serve_app import verify_api_key
...
def mock_api_key(monkeypatch):
    ...
    with patch("kfx.cli.serve_app.verify_api_key") as mock_verify:
...
class TestMultiServeStreaming:
    ...
    async def test_stream_basic_functionality(self, multi_serve_app, mock_api_key):
        ...
        with patch("kfx.cli.serve_app.run_flow_generator_for_serve") as mock_generator:
    ...
    async def test_regular_run_endpoint_still_works(self, multi_serve_app, mock_api_key):
        ...
        with patch("kfx.cli.serve_app.extract_result_data") as mock_extract:
    ...
    async def test_stream_blocks_custom_components_when_disabled(
        ...
        async with (
            ...
            with (
                ...
                patch(
                    "kfx.cli.serve_app.run_flow_generator_for_serve",
                    new=AsyncMock(),
                ) as mock_run_flow,
            ):
                response = await client.post(
    ...
    async def test_stream_setup_error_returns_valid_json(self, mock_graphs, mock_metas, monkeypatch):
        ...
        async with (
            ...
            with patch(
                "kfx.cli.serve_app.validate_flow_for_current_settings",
