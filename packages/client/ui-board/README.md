...
- **Culling hides windows with a fixed world margin** — a window stops painting once it is 480 world units beyond the visible canvas, whatever its size; the margin covers its handles and an open chats panel, and the window keeps its state while hidden.
