...
from ketos.schema.serialize import UUIDstr
from ketos.services.database.utils import validate_non_empty_string, validate_non_empty_string_preserve_value
...
class Deployment(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # ``DeploymentType`` is imported from KFX
    # (``kfx.services.adapters.deployment.schema``).  The DB-level
    # ``deployment_type_enum`` constraint is defined by a Ketos alembic
    # migration, **not** by KFX code.  This decoupling is intentional:
    #
    # KFX adds a new member  -> INSERT/UPDATE with the new value is rejected
    #                           by the DB until a Ketos migration adds it
    #                           to the enum type.  Ketos explicitly opts
    #                           in to new deployment types.
    #
    # KFX renames a value    -> Existing rows are unaffected (the DB stores
    ...
    #
    # KFX removes a member   -> Same as rename: stored rows retain the
    ...
    #   1. Add the member to ``DeploymentType`` in KFX (or confirm it exists).
    #   2. Create a Ketos alembic migration that runs:
    ...
    #   3. Coordinate with KFX — adapters and callers may still reference
    #      the old value.
    #
