...
import pytest
from kfx.services.settings.base import (
...
def test_multi_worker_forces_direct_event_delivery(monkeypatch):
    ...
    """
    monkeypatch.setenv("KETOS_WORKERS", "4")
    monkeypatch.setenv("KETOS_EVENT_DELIVERY", "streaming")
    settings = Settings()
...
def test_single_worker_keeps_explicit_event_delivery(monkeypatch):
    ...
    monkeypatch.setenv("KETOS_WORKERS", "1")
    monkeypatch.setenv("KETOS_EVENT_DELIVERY", "polling")
    settings = Settings()
...
@pytest.mark.parametrize(
    ...
    [
        ("KETOS_HOST", "0.0.0.0", "host", "0.0.0.0"),
        ("KETOS_PORT", "8080", "port", 8080),
        ("KETOS_WORKERS", "2", "workers", 2),
        ("KETOS_LOG_LEVEL", "info", "log_level", "info"),
        ("KETOS_CACHE_TYPE", "memory", "cache_type", "memory"),
        ("KETOS_STORAGE_TYPE", "s3", "storage_type", "s3"),
        ("KETOS_PROMETHEUS_ENABLED", "true", "prometheus_enabled", True),
        ("KETOS_PROMETHEUS_PORT", "9999", "prometheus_port", 9999),
        ("KETOS_MCP_SERVER_ENABLED", "false", "mcp_server_enabled", False),
        ("KETOS_SKIP_MCP_AUTO_INIT", "true", "skip_mcp_auto_init", True),
        ("KETOS_DO_NOT_TRACK", "true", "do_not_track", True),
        ("KETOS_DEV", "true", "dev", True),
        ("KETOS_BACKEND_ONLY", "true", "backend_only", True),
        ("KETOS_AUTO_SAVING", "false", "auto_saving", False),
        ("KETOS_FALLBACK_TO_ENV_VAR", "false", "fallback_to_env_var", False),
        ("KETOS_VARIABLE_STORE", "kubernetes", "variable_store", "kubernetes"),
    ],
)
def test_env_var_round_trip(monkeypatch, env_var, env_value, field, expected):
    """A sampling of KETOS_* env vars still populate the right fields."""
