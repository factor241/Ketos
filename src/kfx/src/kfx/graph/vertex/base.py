...
from kfx.graph.schema import INPUT_COMPONENTS, OUTPUT_COMPONENTS, InterfaceComponentTypes, ResultData
from kfx.graph.utils import UnbuiltObject, UnbuiltResult, emit_build_start_event, log_transaction
from kfx.graph.vertex.param_handler import ParameterHandler
from kfx.interface import initialize
from kfx.interface.listing import lazy_load_dict
from kfx.log.logger import logger
from kfx.schema.artifact import ArtifactType
from kfx.schema.data import Data
from kfx.schema.message import Message
from kfx.schema.properties import Usage
from kfx.schema.schema import INPUT_FIELD_NAME, OutputValue, build_output_logs
from kfx.utils.schemas import ChatOutputResponse
from kfx.utils.util import sync_to_async
...
if TYPE_CHECKING:
    ...
    from kfx.graph.vertex.schema import NodeData
...
class Vertex:
    ...
    async def build(
        ...
        **kwargs,
        ...
        from kfx.interface.components import ensure_component_loaded
    ...
    def before_callback_event(self, *args, **kwargs) -> StepStartedEvent:  # noqa: ARG002
        ...
        return StepStartedEvent(step_name=self.display_name, raw_event={"ketos": metrics})
    ...
    def after_callback_event(self, result, *args, **kwargs) -> StepFinishedEvent:  # noqa: ARG002
        ...
        if hasattr(self, "raw_event_metrics"):
            metrics = self.raw_event_metrics({"component_id": self.id})
        return StepFinishedEvent(step_name=self.display_name, raw_event={"ketos": metrics})
