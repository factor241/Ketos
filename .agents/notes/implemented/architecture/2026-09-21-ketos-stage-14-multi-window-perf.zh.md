...
**圆环在任何状态下都保留自己的位置。** 会话已存在但尚无首次报告的窗口同样渲染圆环：圆弧为空、文本为本地化的 `—`、提示为空态文案。有读数时按本地化模板渲染 `% · used / window`，单位取看板字典里的紧凑 K/M；警告与严重状态同时给圆弧和百分比上色。空态、警告与严重三态已在真实浏览器中用真实投影值验证（0% → `normal`，85% → `warning`，96% → `critical`，未提问的窗口 → `empty`）。
...
**圆环在窄列上让位，并已在真实浏览器中验证。** 专用探针（`.playwright-mcp/stage-14-multi-window-perf/probe-ring-narrow.mjs`）在小视口下进入全屏并展开停靠的聊天面板：488px 卡片上圆环宽 38px，380px 与 180px 时降为 `display: none` 且宽度为 0，composer 无溢出、控制台无错误。
...
Verification: `tests/context-ring.client.spec.ts` 覆盖与 `contextOccupancy` 的公式一致性、80/95 阶梯、紧凑单位与读数；`tests/conversation-body.client.spec.tsx` 覆盖四种渲染状态以及读数更新不会移动转录车道；`tests/leaks.client.spec.tsx` 覆盖 30 轮混合循环、订阅释放与残留定时器；`tests/stress-sessions.client.spec.tsx` 覆盖十窗口五并发流、故障隔离与无串扰；`tests/window-title.client.spec.tsx` 覆盖聊天面板重命名路径、被拒重命名、空标题回退，以及十个各不相同的 dock 与标题栏标题。`pnpm run test:gui`、`DSH_SNAPSHOT=replay pnpm run test:web`、`pnpm run typecheck`、`pnpm run lint`、`pnpm run duplication`、`pnpm run hygiene` 与 `pnpm run doc-sync` 均为绿色；真实浏览器审计的三个 verdict 均为 `ok: true`。
