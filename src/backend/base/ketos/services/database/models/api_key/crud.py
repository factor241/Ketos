...
from ketos.services.database.models.api_key.model import ApiKey, ApiKeyCreate, ApiKeyRead, UnmaskedApiKeyRead
...
async def check_key(session: AsyncSession, api_key: str) -> User | None:
    ...
    Validates API keys based on the KETOS_API_KEY_SOURCE setting:
    - 'db': Validates against database-stored API keys (default)
    - 'env': Validates against the KETOS_API_KEY environment variable,
...
async def _check_key_from_env(session: AsyncSession, api_key: str, settings_service) -> User | None:
    ...
    KETOS_API_KEY environment variable. If valid, returns the superuser for authorization.
    """
