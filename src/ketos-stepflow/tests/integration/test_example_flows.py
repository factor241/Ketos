...
Tests the full end-to-end path used by the ``stepflow-ketos run`` CLI:
KetosConverter -> StepflowClient -> orchestrator -> ketos worker.
...
def _find_poc_flow() -> Path:
    ...
    # here.parents[4] = ketos repo root (e.g. /Users/.../ketos)
    ketos_repo = here.parents[4]
    candidate = ketos_repo.parent / "stepflow" / "docs" / "static" / "files" / "2025-09-ketos-poc-flow.json"
    if candidate.exists():
        return candidate
...
def test_poc_flow_translation(converter, poc_flow_data):
    ...
    for step in flow.steps:
        assert step.component.startswith(("/ketos/", "/builtin/")), f"unexpected component prefix: {step.component}"
