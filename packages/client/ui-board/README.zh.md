...
通过侧栏面板列表中的 board 图标打开该面板；画布填满主面板并从空开始。拖动背景平移，滚轮缩放（0.2–2.0，以指针为中心），用 dock 的加号按钮在当前视图中心添加 agent 卡片。窗口交互遵循同一约定：拖动标题栏移动，拖动边缘手柄缩放单轴，拖动角手柄按比例缩放窗口（两轴同一系数，对角固定），按住 Shift 拖动可关闭 24 px 网格吸附；点击窗口将其提到 z 序最前，关闭按钮移除窗口。窗口绝不会小于创建时的尺寸（agent 窗口的默认聊天尺寸 552×648）——composer 的各行无法在更小的空间内排布——而宽度、高度或对角方向的放大不受限制。窗口整体比应用放大一档：标题、车道文本、芯片、控件与 composer 都读取声明在看板根节点上的一组度量（`--board-font-*` 以及控件、内边距与间距值），因此外框与其内容一同变大，而 dock、Omnibox、小地图与 portal 菜单保持应用的尺寸。标题栏的聊天按钮会从窗口左缘下方滑出一块紧凑面板：它列出项目（工作区，外加未分组一档），选中项目后再列出该项目的聊天及其相对时间与运行标记。选择某个聊天会让窗口指向它，面板上的加号会在当前项目里新建聊天。面板占据窗口左侧的一部分——在舒适宽度下约为其面积的正分之一，在最小窗口中保持 220px 的可读下限——并随窗口移动或缩放，Escape 或再次点击同一按钮会把它滑回外框下方。进入全屏后，面板不再是下层浮层，而是贴着看板面板左缘的普通侧栏，聊天在它右侧居中排布。在 dock 或小地图中选择窗口会聚焦它并把视图居中到其上。窗口首次渲染时会创建自己的会话（该会话出现在侧栏列表中，聚焦窗口会使其成为当前会话）。composer 与主聊天一致：`+` 打开操作菜单（图片附件、目标、计划、反馈、压缩、权限、模型、导出），`/` 按输入过滤斜杠命令并显示参数提示与描述，`@` 提示文件、目录与会话，芯片行可切换权限预设（完全访问走风险确认）以及模型与其推理等级，圆环显示上下文占用，麦克风按钮在引擎提供语音识别时把语音转写写入草稿；工作目录芯片与 agent 预设芯片同处一行，路径在中间省略，因此项目名保持完整，完整路径在提示气泡中。Enter 发送，Cmd/Ctrl+Enter 引导正在执行的回合，Shift+Enter 换行，圆形按钮停止；卡片会自行测量宽度，在窗口变窄时隐藏芯片文字或换行工具栏，因此发送控件在任何尺寸下都可达；`Load earlier turns` 会把更早的回合拉入车道，标题栏的全屏按钮让窗口铺满看板面板，按 Escape 或再点同一按钮即可返回；全屏期间其他窗口以及 dock、Omnibox、小地图都会让位，返回时恢复的是窗口保存的矩形。
...
- **聊天面板只负责列出聊天，不负责管理** —— 选择聊天会重新绑定窗口（从而像聚焦窗口一样让该会话成为当前会话），加号会在当前项目内新建一个；重命名、归档、分叉、搜索、分组与排序菜单以及工作区管理都留在应用自身的侧栏里，而且客户端根本没有删除会话的能力。
- **重新绑定后窗口保留自己的标题** —— 标题栏显示的是窗口打开时的名字，而不是聊天的标题；聊天标题由面板与 composer 的目录芯片承载。
