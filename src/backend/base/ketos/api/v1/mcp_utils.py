...
from kfx.base.mcp.constants import MAX_MCP_TOOL_NAME_LENGTH
from kfx.base.mcp.util import get_flow_snake_case, get_unique_name, sanitize_mcp_name
from kfx.log.logger import logger
from kfx.utils.flow_validation import CustomComponentValidationError
from kfx.utils.helpers import build_content_type_from_extension
from mcp import types
from sqlmodel import select

from ketos.api.v1.endpoints import simple_run_flow
from ketos.api.v1.schemas import SimplifiedAPIRequest
from ketos.helpers.flow import json_schema_from_flow
...
from ketos.services.deps import get_settings_service, get_storage_service, session_scope
...
# Carries per-request variables injected via HTTP headers (e.g., X-Ketos-Global-Var-*)
            ...
            if project_id is None:
                ...
                for user_file in user_files:
                    ...
                    if stored_filename.startswith(f"{MCP_SERVERS_FILE}_{current_user.id}"):
                        # reserved file name for ketos MCP server config file(s)
                        continue
...
async def handle_read_resource(uri: str, project_id: UUID | str | None = None) -> bytes:
    ...
    try:
        ...
        # not enough because the filesystem doesn't know about Ketos users, and
