"""FastAPI application factory for serving **multiple** KFX graphs at once.
...
:func:`kfx.cli.commands.verify_api_key`.
"""
...
from kfx.cli.runtime_variables import apply_global_vars_to_graph
from kfx.load import load_flow_from_json
from kfx.log.logger import logger
from kfx.utils.flow_validation import validate_flow_for_current_settings

if TYPE_CHECKING:
    from collections.abc import AsyncGenerator

    from kfx.cli.flow_store import FlowStore
...
class RunRequest(BaseModel):
    """Request model for executing a KFX flow."""
...
class StreamRequest(BaseModel):
    """Request model for streaming execution of a KFX flow."""
...
class FlowRegistry:
    ...
    by a :class:`~kfx.cli.flow_store.FlowStore`; the default ``NullFlowStore``
    keeps everything in memory only.

    - ``NullFlowStore`` (default): single-worker, no disk I/O.
    - ``FilesystemFlowStore("/tmp/kfx-flows")``: all uvicorn workers in the
      same pod share flows via ``/tmp``.
    - ``FilesystemFlowStore("/mnt/kfx-flows")``: same code, cross-pod if the
      path is a PVC mount.
    ...
    **Per-request stale check (FilesystemFlowStore only)**
    ...
    latency matters, prefer ``/tmp/kfx-flows`` (local tmpfs) for single-pod
...
class UploadFlowRequest(BaseModel):
    ...
    id: str | None = Field(default=None, description="Stable flow ID from Ketos export (must be a valid UUID)")
...
async def run_flow_generator_for_serve(
    ...
    try:
        ...
        # to integrate with the full KFX streaming pipeline from endpoints.py
...
def create_multi_serve_app(
    *,
    ...
    """Create a FastAPI app exposing KFX flows via a mutable registry.

    Routes dispatch to ``registry`` at request time, so flows added after
    startup (via ``POST /flows/upload/``) are immediately reachable.
    """
    app = FastAPI(
        title=f"KFX Multi-Flow Server ({len(registry)})",
        description=(
            "Hosts KFX graphs under the `/flows/{{id}}` prefix. "
    ...
    async def stream_flow(flow_id: str, request: StreamRequest) -> StreamingResponse:
        ...
        try:
            validate_flow_for_current_settings(graph)
            from kfx.events.event_manager import create_stream_tokens_event_manager
...
def create_serve_app() -> FastAPI:
    ...
    worker calls this factory, which reads ``KFX_SERVE_FLOW_DIR`` and
    ``KFX_SERVE_NO_ENV_FALLBACK`` from the environment, pre-warms its own
    ...
    ``uvicorn.run("kfx.cli.serve_app:create_serve_app", workers=N, ...)``.
    """
    import asyncio
    import os
    from pathlib import Path

    from kfx.cli.flow_store import FilesystemFlowStore, NullFlowStore
    ...
    if startup_paths and not flow_dir:
        ...
        from kfx.cli.commands import build_registry_from_directory, build_registry_from_paths
