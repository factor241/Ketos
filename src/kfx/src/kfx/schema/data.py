"""Lightweight JSON class for kfx package - contains only methods with no ketos dependencies.

This module provides the JSON class (formerly Data) as the base type for Ketos data structures.
...
from kfx.utils.constants import MESSAGE_SENDER_AI, MESSAGE_SENDER_USER
from kfx.utils.image import create_image_content_dict

if TYPE_CHECKING:
    from kfx.schema.dataframe import Table
...
class JSON(CrossModuleModel):
    ...
    This is the base type for Ketos data structures, replacing the legacy Data class.
    Data is maintained as an alias for backwards compatibility.
    ...
    def validate_data(cls, values):
        ...
        if not isinstance(values["data"], dict):
            msg = (
                ...
                " This will raise an error in version ketos==1.3.0."
            )
            logger.warning(msg)
    ...
    def to_lc_message(
        ...
        if sender == MESSAGE_SENDER_USER:
            if files:
                from kfx.schema.image import get_file_paths
    ...
    def filter_data(self, filter_str: str) -> JSON:
        ...
        """
        from kfx.template.utils import apply_json_filter

        return apply_json_filter(self.data, filter_str)

    def to_message(self) -> Message:
        from kfx.schema.message import Message  # Local import to avoid circular import
