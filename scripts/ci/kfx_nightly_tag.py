"""Generate the nightly tag for the canonical ``kfx`` package.

The nightly publishes ``kfx==<base>.devN`` to the canonical ``kfx`` PyPI project (not a separate
``kfx-nightly`` distribution), so the dev counter is computed against ``kfx``'s own release history.
``<base>`` is the in-development version from
``src/kfx/pyproject.toml`` (the latest ``release-*`` branch the nightly builds from) and ``N`` is
...
PYPI_KFX_URL = "https://pypi.org/pypi/kfx/json"


def _kfx_base_version() -> str:
    """Return the base_version (e.g. "1.11.0") from src/kfx/pyproject.toml."""
    kfx_pyproject_path = Path(__file__).parent.parent.parent / "src" / "kfx" / "pyproject.toml"
    pyproject_data = tomllib.loads(kfx_pyproject_path.read_text())
...
def create_kfx_tag() -> str:
    """Return the next ``kfx`` nightly tag (with a leading ``v``)."""
    base_version = _kfx_base_version()
    dev_numbers = _dev_numbers(PYPI_KFX_URL, base_version)
