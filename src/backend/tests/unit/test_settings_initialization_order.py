...
class TestSettingsInitializationOrder:
    ...
    def test_is_settings_service_initialized_returns_false_initially(self):
        ...
        from ketos.services.deps import is_settings_service_initialized
    ...
    def test_is_settings_service_initialized_returns_true_after_init(self):
        ...
        from ketos.services.deps import get_settings_service, is_settings_service_initialized
        from kfx.services.manager import get_service_manager
    ...
    def test_cli_check_pattern_success_case(self, tmp_path):
        ...
        if "KETOS_DATABASE_URL" in os.environ:
            del os.environ["KETOS_DATABASE_URL"]
...
class TestCLISubprocessIntegration:
    ...
    def test_cli_env_file_values_actually_used(self, tmp_path):
        ...
        env_file.write_text(
            f"""
KETOS_DATABASE_URL=sqlite:///{db_path}
KETOS_AUTO_SAVING=false
KETOS_AUTO_LOGIN=false
KETOS_SUPERUSER=ketos
KETOS_SUPERUSER_PASSWORD=test-superuser-password
KETOS_LOG_LEVEL=ERROR
        """.strip()
        )

        # Create a test script that starts ketos and checks if the database was created
...
# Start ketos run with --env-file in background
...
proc = subprocess.Popen(
    [
        sys.executable, "-m", "ketos", "run", "--env-file", str(env_file),
