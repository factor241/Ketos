...
- **Locale.** The ru language pack registers `ru` with fallback `en`, registers the shared `common`, `settings.locale`, and `board` ru dictionaries, and applies `setLocale('ru')` only when the durable `locale` settings snapshot resolves with no stored `preference` and the browser itself asks for a `ru`-tagged language, so the shipped `zh`/`en` fallback contract and an explicit en survive disposal and re-apply.
