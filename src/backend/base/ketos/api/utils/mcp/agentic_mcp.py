"""Utilities for auto-configuring the Ketos Agentic MCP server."""
...
from ketos.api.v2.mcp import get_server_list, update_server
from ketos.services.database.models.user.model import User
from ketos.services.deps import get_service, get_variable_service
...
async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the Ketos Agentic MCP server for all users.

    This function adds the ketos-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring Ketos Agentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "ketos-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "ketos.agentic.mcp"],
            "metadata": {
                "description": "Ketos Agentic MCP server providing tools for flow/component operations, "
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the Ketos Agentic MCP server from all users.

    This function removes the ketos-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing Ketos Agentic MCP server from all users...")
...
async def initialize_agentic_user_variables(user_id: UUID | str, session: AsyncSession) -> None:
    ...
    try:
        ...
        from kfx.services.settings.constants import AGENTIC_VARIABLES, DEFAULT_AGENTIC_VARIABLE_VALUE
