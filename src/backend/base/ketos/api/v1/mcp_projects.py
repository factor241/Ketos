...
from kfx.log import logger
from kfx.services.deps import get_settings_service, session_scope
from kfx.services.mcp_composer.service import (
...
from ketos.api.v1.auth_helpers import handle_auth_settings_update
from ketos.api.v1.mcp import ResponseNoOp
from ketos.api.v1.mcp_utils import (
...
from ketos.api.v1.schemas import (
    AuthSettings,
    ComposerUrlResponse,
    MCPInstallRequest,
    MCPProjectResponse,
...
from ketos.services.auth.mcp_encryption import decrypt_auth_settings, encrypt_auth_settings
from ketos.services.database.models import Flow, Folder
from ketos.services.database.models.api_key.crud import check_key, create_api_key
from ketos.services.database.models.api_key.model import ApiKeyCreate
...
async def verify_project_auth(
    ...
    # OAuth projects must present a valid API key at the Ketos transport endpoint: network-level
    ...
    # composer-to-Ketos hop should be authenticated explicitly once mcp-composer can forward
...
async def verify_project_auth_conditional(
    ...
    async with session_scope() as session:
        ...
        from ketos.services.auth.utils import get_current_user_mcp
    ...
    try:
        async with session_scope() as session:
            ...
            if auth_settings_updated and new_auth_type in {"apikey", "none"}:
                from ketos.api.v1.projects_mcp_helpers import reconcile_mcp_server_for_auth_update
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add Ketos API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a Ketos API key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    ketos_api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", ketos_api_key])
        ...
        # Remove stale entries that point to the same Ketos URLs (e.g. after the project is renamed)
...
class ProjectMCPServer:
    def __init__(self, project_id: UUID):
        ...
        self.server = Server(f"ketos-mcp-project-{project_id}")
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Ketos host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
                ...
                try:
                    ...
                    async with session.begin_nested():
                        ...
                        if should_reconcile_project_server:
                            from ketos.api.v1.projects_mcp_helpers import register_mcp_servers_for_project
...
async def get_or_start_mcp_composer(auth_config: dict, project_name: str, project_id: UUID) -> None:
    ...
    """
    from kfx.services.mcp_composer.service import MCPComposerConfigError
