...
from kfx.services.interfaces import DeploymentServiceProtocol
...
from ketos.services.database.utils import require_non_empty

if TYPE_CHECKING:
    from collections.abc import Sequence

    from ketos.api.utils import DbSession
...
# ---------------------------------------------------------------------------
# Route helpers (moved from ketos.api.v1.deployments)
# ---------------------------------------------------------------------------
...
def handle_adapter_errors(*, mapper: BaseDeploymentMapper | None = None):
    ...
    ``kfx.services.adapters.deployment.exceptions`` module.  Non-domain
...
async def resolve_adapter_mapper_from_provider_id(
    ...
    *,
    ...
    from ketos.api.v1.mappers.deployments.registry import get_deployment_mapper
...
async def get_deployment_row_or_404(
    *,
    ...
    # registered and ``KETOS_AUTHZ_ENABLED`` is on; otherwise it stays
