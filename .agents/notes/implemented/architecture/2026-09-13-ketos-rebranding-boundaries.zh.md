...
- **Локаль.** Языковой пакет ru регистрирует `ru` с фолбэком `en`, регистрирует ru-словари общих `common` и `settings.locale` и применяет `setLocale('ru')` только когда durable-снапшот настроек `locale` резолвится без сохранённого `preference` — явный выбор en переживает disposal и повторное применение.
