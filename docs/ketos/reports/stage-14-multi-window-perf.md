...
| Живой аудит 14.1, основной проход | `audit-verdict.json`: 21/21 проверок `ok: true`; панорамирование 60.0 FPS / 30.2 мс, зум 59.9 FPS / 32.6 мс, открытие окна 29.5–69.2 мс, 5/5 ответов, 0 длинных задач, 0 ошибок консоли и хоста |
| Живой аудит 14.1, проходы кольца A/B/C | `audit-verdict-A|B|C.json`: 22/22 проверок `ok: true` в каждом; `normal`/`warning`/`critical` на 0/85/96%, AX-имя `- img "85% · 850K / 1M"`; скриншоты `06-context-ring-<state>.png` |
