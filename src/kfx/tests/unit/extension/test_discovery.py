...
import pytest
from kfx.extension.discovery import (
    DEFAULT_SEED_DIR,
    SEED_DIR_ENV_VAR,
...
from kfx.extension.manifest import ManifestSource
...
def _write_pyproject_extension(root: Path, manifest: dict[str, object]) -> Path:
    """Drop a pyproject.toml that declares ``[tool.ketos.extension]``.
    ...
    bundle_name = manifest["bundles"][0]["name"]  # type: ignore[index]
    (root / bundle_name).mkdir(exist_ok=True)
    body = "[tool.ketos.extension]\n"
    body += f'id = "{manifest["id"]}"\n'
    body += f'version = "{manifest["version"]}"\n'
    body += f'name = "{manifest["name"]}"\n'
    body += "\n"
    body += "[tool.ketos.extension.kfx]\n"
    body += 'compat = ["1"]\n'
    body += "\n"
    body += "[[tool.ketos.extension.bundles]]\n"
...
def fake_installed_distributions(tmp_path: Path) -> list[_FakeDistribution]:
    ...
    for index, bundle in enumerate(_BUNDLE_NAMES):
        ext_root = tmp_path / f"site-packages/kfx_{bundle}"
        ext_id = f"kfx-{bundle}"
...
def seed_dir_with_three_bundles(tmp_path: Path) -> Path:
    ...
    seed.mkdir()
    for index, bundle in enumerate(_BUNDLE_NAMES):
        sub = seed / f"kfx_{bundle}"
        manifest = _manifest(extension_id=f"kfx-{bundle}", bundle_name=bundle, version=f"2.{index}.0")
...
def test_discover_installed_finds_three_distributions(
    ...
    assert ids == ["kfx-anthropic", "kfx-openai", "kfx-qdrant"]
...
def test_discover_installed_accepts_pyproject_form(
    ...
    assert by_id["kfx-anthropic"].manifest.kind == "pyproject.toml"
    assert by_id["kfx-openai"].manifest.kind == "extension.json"
...
def test_discover_installed_emits_typed_error_on_malformed_manifest(tmp_path: Path) -> None:
    ...
    """
    bad_root = tmp_path / "site-packages/kfx_broken"
    bad_root.mkdir(parents=True)
    (bad_root / "extension.json").write_text("{ this is not json", encoding="utf-8")
    bad = _FakeDistribution(name="kfx-broken", root=bad_root, manifest_relative="extension.json")

    good_root = tmp_path / "site-packages/kfx_openai"
    _write_extension_json(good_root, _manifest("kfx-openai", "openai"))
    good = _FakeDistribution(name="kfx-openai", root=good_root, manifest_relative="extension.json")

    extensions, errors = discover_installed_extensions(distributions=[bad, good])

    assert len(errors) == 1
    assert errors[0].code == "manifest-unreadable"
    assert "kfx-broken" in errors[0].content if errors[0].content else True
    assert len(extensions) == 1
    assert extensions[0].extension_id == "kfx-openai"
...
def test_discover_installed_emits_typed_error_on_schema_failure(tmp_path: Path) -> None:
    ...
    root = tmp_path / "site-packages/kfx_bad_schema"
    root.mkdir(parents=True)
    # Missing ``kfx`` field -- pydantic will reject this.
    (root / "extension.json").write_text(
        json.dumps({"id": "kfx-x", "version": "1.0.0", "name": "X", "bundles": []}),
        encoding="utf-8",
    )
    dist = _FakeDistribution(name="kfx-x", root=root, manifest_relative="extension.json")
...
def test_canonicalize_distribution_normalizes_per_pep503() -> None:
    assert canonicalize_distribution("KFX_OpenAI") == "kfx-openai"
    assert canonicalize_distribution("kfx.openai") == "kfx-openai"
    assert canonicalize_distribution("kfx--OpenAI") == "kfx-openai"
...
class _EditableDistribution(importlib_metadata.Distribution):
    ...
    listed in RECORD), and the ``ketos.extensions`` entry-point is
...
def test_discover_installed_falls_back_to_entry_point_for_editable_install(
    ...
    """Editable installs are discovered via the ``ketos.extensions`` entry-point.
    ...
    _make_editable_package(
        site,
        module_name="kfx_editable_bundle",
        manifest=_manifest("kfx-editable-bundle", "bundle"),
    )
    ...
    sys.modules.pop("kfx_editable_bundle", None)

    dist = _EditableDistribution(
        name="kfx-editable-bundle",
        module_name="kfx_editable_bundle",
    )
    ...
    assert len(extensions) == 1
    ext = extensions[0]
    assert ext.extension_id == "kfx-editable-bundle"
    assert ext.source_kind == "installed"
    assert ext.bundle_name == "bundle"
    assert ext.manifest.kind == "extension.json"
    assert ext.extension_root == site / "kfx_editable_bundle"
...
def test_discover_installed_entry_point_fallback_handles_missing_module(
    ...
    # No package on sys.path corresponds to ``kfx_nonexistent``.
    monkeypatch.syspath_prepend(str(tmp_path))
    import sys

    sys.modules.pop("kfx_nonexistent_bundle", None)

    dist = _EditableDistribution(
        name="kfx-nonexistent-bundle",
        module_name="kfx_nonexistent_bundle",
    )
...
def test_discover_installed_entry_point_fallback_skipped_when_files_have_manifest(
    ...
    """
    site = tmp_path / "site-packages/kfx_wheelish"
    _write_extension_json(site, _manifest("kfx-wheelish", "wheelish"))
...
def test_discover_seed_supports_pathsep_multiple_roots(tmp_path: Path) -> None:
    """``$KETOS_SEED_DIR`` accepts multiple roots joined by ``os.pathsep``."""
    root_a = tmp_path / "a"
    root_b = tmp_path / "b"
    root_a.mkdir()
    root_b.mkdir()
    _write_extension_json(root_a / "kfx_openai", _manifest("kfx-openai", "openai"))
    _write_extension_json(root_b / "kfx_anthropic", _manifest("kfx-anthropic", "anthropic"))

    composite = f"{root_a}{os.pathsep}{root_b}"
    extensions, errors = discover_seed_extensions(seed_dir_env=composite)

    assert errors == []
    assert {ext.extension_id for ext in extensions} == {"kfx-openai", "kfx-anthropic"}
...
def test_discover_seed_skips_directories_without_manifest(tmp_path: Path) -> None:
    ...
    _write_extension_json(seed / "kfx_openai", _manifest("kfx-openai", "openai"))

    extensions, errors = discover_seed_extensions(seed_dir_env=str(seed))

    assert errors == []
    assert [ext.extension_id for ext in extensions] == ["kfx-openai"]
...
def test_discover_seed_skips_dot_directories(tmp_path: Path) -> None:
    ...
    hidden = seed / ".cache"
    hidden.mkdir()
    _write_extension_json(hidden, _manifest("kfx-hidden", "hidden"))
    _write_extension_json(seed / "kfx_visible", _manifest("kfx-visible", "visible"))

    extensions, _ = discover_seed_extensions(seed_dir_env=str(seed))
    ids = {ext.extension_id for ext in extensions}
    assert ids == {"kfx-visible"}
...
def test_default_seed_dir_constant_is_opt_ketos_bundles() -> None:
    """Pinning the documented default so docs and code agree."""
    assert Path("/opt/ketos/bundles") == DEFAULT_SEED_DIR
...
def test_discover_all_merges_installed_and_seed(
    ...
    seed = tmp_path / "seed"
    _write_extension_json(seed / "kfx_seed_only", _manifest("kfx-seed-only", "seed_only"))
    ...
    assert any(ext.extension_id == "kfx-seed-only" for ext in extensions)
...
def test_discover_all_emits_seed_bundle_shadowed_when_ids_collide(
    ...
    """
    # ``fake_installed_distributions`` ships ``kfx-openai``; the seed dir
    # below carries the same id from a different on-disk source.
    seed = tmp_path / "seed"
    shadow_root = seed / "kfx_openai_shadow"
    _write_extension_json(shadow_root, _manifest("kfx-openai", "openai", version="9.9.9"))
    fresh_root = seed / "kfx_only_seed"
    _write_extension_json(fresh_root, _manifest("kfx-only-seed", "only_seed"))
    ...
    assert extension_ids.count("kfx-openai") == 1
    assert any(ext.extension_id == "kfx-openai" and ext.source_kind == "installed" for ext in extensions)
    assert any(ext.extension_id == "kfx-only-seed" for ext in extensions)

    shadow_errors = [err for err in errors if err.code == "seed-bundle-shadowed"]
    assert len(shadow_errors) == 1
    [err] = shadow_errors
    assert err.content == "kfx-openai"
