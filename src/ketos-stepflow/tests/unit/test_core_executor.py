...
class TestCoreExecutorWithRealComponent:
    """Integration tests with real Ketos components.

    Note: Some Ketos components have complex dependencies (database, etc.)
    ...
    async def test_import_prompt_component(self, executor, mock_context):
        ...
        module = importlib.import_module("kfx.components.models_and_agents.prompt")
    ...
    async def test_import_chat_input_component(self, executor, mock_context):
        ...
        module = importlib.import_module("kfx.components.input_output.chat")
