...
Override scope:
    ...
    * Live-fetched providers (Ollama, IBM WatsonX, OpenRouter) are *not*
      ...
      :mod:`kfx.base.models.model_utils` runs **after** the catalog is
...
Cache layout:
    ...
    * Disk snapshot at ``<ketos-cache>/models_dev/snapshot.json`` for
      offline cold starts.
...
# Translation: models.dev provider slug -> Ketos display name. Only
# providers Ketos already knows how to instantiate (have an entry in
...
def _translate_model_entry(
    ...
    *,
    ...
    """Convert one models.dev model entry to Ketos's ``ModelMetadata`` shape.
