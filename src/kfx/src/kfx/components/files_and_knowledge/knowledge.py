...
from ketos.services.auth.utils import decrypt_api_key, encrypt_api_key
from langchain_chroma import Chroma

from kfx.base.knowledge_bases.backends import BackendType, BaseVectorStoreBackend, create_backend
...
from kfx.base.knowledge_bases.ingestion_sources.flow_component import FlowComponentSource
from kfx.base.knowledge_bases.knowledge_base_utils import get_knowledge_bases
from kfx.base.models.unified_models import get_embedding_model_options, get_embeddings
from kfx.base.vectorstores.chroma_security import chroma_langchain_collection_kwargs
from kfx.components.files_and_knowledge._kb_paths import (
    get_knowledge_bases_root_path as _get_knowledge_bases_root_path,
)
from kfx.components.files_and_knowledge._kb_paths import (
    load_kb_metadata,
)
from kfx.components.processing.converter import convert_to_dataframe
...
from kfx.services.deps import (
    get_settings_service,
    session_scope,
)
from kfx.utils.component_utils import set_current_fields, set_field_display
from kfx.utils.validate_cloud import raise_error_if_astra_cloud_disable_component
...
# Ketos's TabInput palette — see ``MemoryComponent`` for the same convention.
MODE_INGEST = "Ingest"
...
class KnowledgeComponent(Component):
    """One component for both writing into and reading from a Ketos knowledge base.
    ...
    description = "Ingest into or retrieve from a Ketos knowledge base."
    icon = "database"
    name = "Knowledge"
            ...
            default_factory=lambda: {
                "data": {
                    "node": {
                        ...
                        "description": "Create new knowledge in Ketos.",
                        ...
                        "template": {
                            ...
                            "02_embedding_model": ModelInput(
                                ...
                                info=(
                                    ...
                                    "Ketos uses the configured credentials for that model provider."
                                ),
                                required=True,
    ...
    async def update_build_config(
        ...
        if field_name == "knowledge_base":
            # Lazy import keeps kfx importable without ketos installed.
            from ketos.services.database.models.user.crud import get_user_by_id
    ...
    def _update_metadata_metrics(self, kb_path: Path, chroma: Chroma) -> None:
        ...
        from ketos.api.utils.kb_helpers import KBAnalysisHelper, KBStorageHelper
    ...
    def _extract_source_types_from_input(cls, input_value: Any) -> set[str]:
        ...
        # Message / Data / JSON all expose a ``data`` dict via the kfx schema.
    ...
    async def _create_knowledge_base_record(
        ...
        *,
        ...
        """Persist the component-created KB in the DB when Ketos is available."""
        try:
    ...
    async def _kb_path(self) -> Path | None:
        ...
        # Lazy import to keep ``kfx`` importable standalone — ketos's
    ...
    async def _begin_ingestion_run(
        ...
        try:
            from ketos.api.utils import ingestion_run_service, knowledge_base_service
    ...
    async def _record_kb_status(
        ...
        *,
        ...
        try:
            from ketos.api.utils import knowledge_base_service
            from ketos.services.database.models.knowledge_base.model import KnowledgeBaseStatus
        except ImportError:
            return
        try:
    ...
    async def retrieve_data(self) -> DataFrame:
        ...
        # Lazy import: ketos's user/DB models aren't part of kfx's
        # standalone install, so ``kfx run <starter>.json`` can't resolve
