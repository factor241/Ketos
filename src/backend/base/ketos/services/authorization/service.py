"""Ketos authorization service (OSS pass-through; plugins enforce RBAC)."""
...
class KetosAuthorizationService(BaseAuthorizationService):
    ...
    def __init__(self, settings_service: SettingsService) -> None:
        ...
        self.set_ready()
        logger.debug("Ketos authorization service initialized")
        ...
        # plugin registered via ``kfx.toml``, ``enforce()`` returns True for
        ...
        if authz_enabled and not self.SUPPORTS_CROSS_USER_FETCH:
            logger.warning(
                "KETOS_AUTHZ_ENABLED=true but the OSS pass-through authorization service is "
                ...
                "Register an authorization plugin via kfx.toml or set KETOS_AUTHZ_ENABLED=false "
