"""Tests for kfx.processing.process.apply_tweaks.

These mirror the ketos-side regression tests in
``src/backend/tests/unit/test_process.py`` so the two copies of ``apply_tweaks``
(``ketos.processing.process`` used by the API and ``kfx.processing.process``)
...
from kfx.processing.process import apply_tweaks
from kfx.utils.flow_validation import CODE_EXECUTION_COMPONENT_TYPES, CODE_EXECUTION_FIELD_NAMES
...
def test_apply_tweaks_blocks_code_named_field():
    ...
    with patch("kfx.processing.process.logger") as mock_logger:
...
# kfx/utils/flow_validation.py). "code" is the conventional exec input that
...
def test_every_code_execution_type_has_registered_code_fields():
    ...
    for component_type, code_fields in _EXPECTED_CODE_FIELDS_BY_TYPE.items():
        ...
        assert not missing, (
            ...
            f"guard would not block. Add them to CODE_EXECUTION_FIELD_NAMES in kfx/utils/flow_validation.py."
        )
