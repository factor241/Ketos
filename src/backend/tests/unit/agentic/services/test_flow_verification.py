...
import pytest
from ketos.agentic.services.flow_probe_input import PROBE_INPUT_TEXT
from ketos.agentic.services.flow_verification import (
...
class TestVerifyBuiltFlow:
    ...
    async def test_should_not_leak_a_secret_in_the_caveat(self):
        async def run_fn(_flow):
            return {"error": "Auth failed using api key REDACTED_OPENAI_API_KEY"}
