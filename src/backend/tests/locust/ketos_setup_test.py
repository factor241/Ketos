#!/usr/bin/env python3
"""Ketos Load Test Setup CLI

This script sets up a complete Ketos test environment by:
1. Starting Ketos (optional)
...
Usage:
    python setup_ketos_test.py --help
    python setup_ketos_test.py --interactive
    python setup_ketos_test.py --flow "Basic Prompting"
    python setup_ketos_test.py --list-flows
"""
...
async def authenticate_setup_client(client) -> tuple[str, str, str]:
    ...
    configured_username = os.environ.get("KETOS_SUPERUSER", "")
    password = os.environ.get("KETOS_SUPERUSER_PASSWORD", "")

    if configured_username and not password:
        raise RuntimeError("Set KETOS_SUPERUSER_PASSWORD when KETOS_SUPERUSER is set, or unset both.")

    if password:
        username = configured_username or "ketos"
    ...
    if auto_login_response.status_code != 200:
        raise RuntimeError(
            "Authentication failed. Set KETOS_SUPERUSER_PASSWORD and optionally KETOS_SUPERUSER, "
            "or enable KETOS_AUTO_LOGIN."
        )
...
async def get_starter_projects_from_api(host: str, access_token: str) -> list[dict]:
    """Get starter projects from Ketos API."""
    import httpx
...
async def list_available_flows(host: str, access_token: str) -> list[tuple[str, str, str]]:
    """List all available starter project flows from Ketos API.
...
async def setup_ketos_environment(host: str, flow_name: str | None = None, interactive: bool = False) -> dict:
    """Set up complete Ketos environment with real starter project flows."""
    try:
        import httpx
    except ImportError:
    ...
    async with httpx.AsyncClient(base_url=host, timeout=60.0) as client:
        ...
        try:
            ...
            # Remove the id to let Ketos generate a new one
...
def print_setup_results(setup_state: dict):
    ...
    print(f"export KETOS_HOST='{setup_state['host']}'")
    ...
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-ketos --headless --users 25 --duration 120"
    )
    print()

    print("# Generate HTML report:")
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-ketos --headless --users 50 --duration 180 --html report.html"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up Ketos load test environment with real starter project flows",
...
Examples:
  ...
  # Use specific flow
  python setup_ketos_test.py --flow "Memory Chatbot"

  # List available flows
  python setup_ketos_test.py --list-flows

  # Setup with custom host
  python setup_ketos_test.py --host http://localhost:8000 --interactive

  # Save credentials to file
  python setup_ketos_test.py --interactive --save-credentials test_creds.json
        """,
    )
    ...
    parser.add_argument(
        ...
        help="Ketos host URL (default: http://localhost:7860, use https:// for remote instances)",
    )
        ...
        async def list_flows_only():
            ...
            async with httpx.AsyncClient(base_url=args.host, timeout=30.0) as client:
                ...
                try:
                    health_response = await client.get("/health")
                    if health_response.status_code != 200:
                        raise Exception(f"Ketos not available at {args.host}")
                except Exception as e:
                    print(f"❌ Cannot connect to Ketos at {args.host}: {e}")
                    sys.exit(1)
                ...
                try:
                    ...
                    print(f"❌ Authentication failed: {e}")
                    print(
                        "Set KETOS_SUPERUSER_PASSWORD and optionally KETOS_SUPERUSER, or enable KETOS_AUTO_LOGIN."
                    )
                    sys.exit(1)
    ...
    try:
        ...
        setup_state = asyncio.run(
            setup_ketos_environment(host=args.host, flow_name=args.flow, interactive=args.interactive)
        )
