...
Each is decoupled from ``ketos`` via late imports (``kfx`` ships without
...
class RunFlow(Component):
    ...
    Decoupled from ``ketos`` via a late import (same pattern as
    ``_load_registry_user_aware``): ``kfx`` may run without the backend.
    """
    ...
    async def run_flow(self) -> Data:
        ...
        try:
            ...
            from ketos.agentic.services.flow_preparation import inject_model_into_flow
...
class GenerateComponent(Component):
    """Generate, validate and register a NEW custom Ketos component.
    ...
    imports ``ketos`` (same decoupling as ``RunFlow``): ``kfx`` may run
    without the backend.
    """

    display_name = "Generate Component"
    description = (
        "Create a brand-new custom Ketos component from a natural-language description. "
    ...
    async def generate_component(self) -> Data:
        ...
        try:
            from ketos.agentic.services.assistant_service import execute_flow_with_validation
        ...
        try:
            from ketos.agentic.services.agent_run_context import current_agent_run_model
            from ketos.agentic.services.user_components_context import current_user_id
        ...
        try:
            from ketos.agentic.services.component_events import emit_component_generation_failed
