"""kfx init -- scaffold a new Flow DevOps project.
...
Examples::
    ...
    kfx init my-rag-project
    kfx init .                   # scaffold into the current directory
    kfx init my-project --no-github-actions
"""
...
Run against a local instance (started with ``kfx serve``):

    pytest tests/ --ketos-url http://localhost:8000

Run against a named environment (staging, production, etc.):

    pytest tests/ --ketos-env staging -m integration
...
def test_flow_output_quality(flow_runner):
    ...
    result = flow_runner("my-flow-endpoint", "What is Ketos?")
...
def init_command(
    ...
    *,
    ...
    if example:
        from kfx.cli.create import create_command as _create
    ...
    if gitignore.exists():
        ...
        if "ketos-environments.toml" not in existing_content:
    ...
    if example:
        ...
        console.print("  4. [bold]kfx serve flows/hello-world.json[/bold]     (run it locally)")
        console.print("  5. [bold]kfx push --dir flows/ --env local[/bold]    (deploy to Ketos)")
    else:
        console.print("  3. [bold]kfx create my-flow --template hello-world[/bold]")
        console.print("  4. [bold]kfx push --dir flows/ --env local[/bold]")
    console.print(f"  {'6' if example else '5'}. [bold]pytest tests/ --ketos-env local[/bold]")
    console.print()
