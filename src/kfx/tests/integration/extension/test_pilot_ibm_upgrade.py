...
that ship in ``kfx-ibm``: ``DB2VectorStoreComponent``,
...
the three IBM components from the pre-extraction Ketos:
    ...
    3. The kfx-ibm distribution is importable AND ships the manifest in
...
def test_short_import_path_flow_upgrades(migration_table, class_name, _legacy_module, expected_target) -> None:
    ...
    flow = _saved_flow(_saved_flow_node(f"ibm-short-{class_name}", f"kfx.components.ibm.{class_name}"))
...
@pytest.mark.integration
def test_kfx_ibm_distribution_is_importable() -> None:
    ...
    (kfx's own venv does not list kfx-ibm as a dep); the ketos
    ...
    try:
        ...
        pytest.skip("kfx-ibm not installed in this test environment")
...
@pytest.mark.integration
def test_kfx_ibm_ships_manifest() -> None:
    ...
    """
    try:
        dist = importlib_metadata.distribution("kfx-ibm")
    ...
    if _is_editable_install(dist):
        ...
        package_dir = Path(kfx_ibm.__file__).parent
        manifest_path = package_dir / "extension.json"
        assert manifest_path.is_file(), (
            f"kfx-ibm source tree at {package_dir} does not ship "
        ...
        assert manifests, (
            "kfx-ibm distribution does not ship extension.json in its "
