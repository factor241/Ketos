"""HTTP client helper for ``kfx extension reload``.
...
in :mod:`kfx.cli._extension_commands` delegates to :func:`reload_via_http`
...
def reload_via_http(
    *,
    ...
    """POST a reload request to the Ketos extension dev server.
                ...
                "errors": [
                    {
                        ...
                        "message": f"Could not reach Ketos server at {url}: {exc}",
                        "location": url,
                        "hint": (
                            "Start the dev server (e.g. `kfx run`) and pass --target / "
                            "set KETOS_HOST if it is not on http://localhost:7860."
                        ),
                    }
                ],
            },
            ...
            "errors": [
                {
                    ...
                    "location": url,
                    "hint": "Confirm the URL points at a Ketos server with the v1 API enabled.",
                }
            ],
        }
