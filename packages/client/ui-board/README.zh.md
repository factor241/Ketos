...
- **窗口车道只渲染正文与工具名** —— assistant 文本以 Markdown 呈现且流式文本可见，工具调用只有在结果写入日志后才出现并折叠为一行；仍在运行的调用完全不显示。完整的工具卡片（终端、diff、读取、搜索、网页）随拥有它们的阶段到来。
...
- **带参数的命令从菜单选中时只填入草稿** —— goal、plan、feedback 与 permission 会把 `/<name> ` 插入草稿并作为普通提示发出，命令不会执行；只有不带参数的命令会在选中时直接执行。客户端的 `/model` 与 `/file` 命令不在该列表中：看板列出的是宿主命令注册表，它不包含这两条命令。
