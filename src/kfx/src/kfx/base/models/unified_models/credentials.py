...
from kfx.services.deps import get_variable_service, session_scope
from kfx.services.variable.request_scope import is_env_fallback_disabled
...
def get_api_key_for_provider(user_id: UUID | str | None, provider: str, api_key: Any = None) -> str | None:
    ...
    # key). Env is the fallback for the no-user (kfx run) / no-DB-value case.
    ...
    # Fall through to os.environ regardless so kfx run (no user_id) can still pick
...
def _env_value_for(var_key: str) -> str | None:
    """Read a provider key from the environment, accepting a KETOS_ alias.

    Provider keys are conventionally bare (``GOOGLE_API_KEY``), but some .env
    templates prefix everything with ``KETOS_`` (matching how Ketos reads
    its own settings). Accept ``KETOS_<VAR>`` as a fallback so e.g.
    ``KETOS_GOOGLE_API_KEY`` enables Gemini exactly like ``GOOGLE_API_KEY``.
    ...
    if value and value.strip():
        return value
    prefixed = os.environ.get(f"KETOS_{var_key}")
