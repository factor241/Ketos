...
from kfx.base.tools.constants import (
    TOOL_OUTPUT_DISPLAY_NAME,
    TOOL_OUTPUT_NAME,
    TOOLS_METADATA_INFO,
    TOOLS_METADATA_INPUT_NAME,
)
from kfx.custom.tree_visitor import RequiredInputsVisitor
from kfx.exceptions.component import StreamingError
from kfx.field_typing import Tool  # noqa: TC001

# Lazy import to avoid circular dependency
# from kfx.graph.state.model import create_state_model
# Lazy import to avoid circular dependency
# from kfx.graph.utils import has_chat_output
from kfx.helpers.custom import format_type
from kfx.memory import astore_message, aupdate_messages, delete_message
from kfx.schema.artifact import get_artifact_type, post_process_raw
from kfx.schema.data import Data
from kfx.schema.log import Log
from kfx.schema.message import ErrorMessage, Message
from kfx.schema.properties import Source, Usage
from kfx.schema.token_usage import accumulate_usage, extract_usage_from_chunk
from kfx.serialization.serialization import serialize
from kfx.template.field.base import UNDEFINED, Input, Output
from kfx.template.frontend_node.custom_components import ComponentFrontendNode
from kfx.utils.async_helpers import run_until_complete
from kfx.utils.secrets import is_secret_value, unwrap_secret_value
from kfx.utils.util import find_closest_match
...
if TYPE_CHECKING:
    ...
    from kfx.inputs.inputs import InputTypes
    from kfx.schema.dataframe import DataFrame
    from kfx.schema.log import LoggableType
...
class Component(CustomComponent):
    ...
    def _should_track_input(self, input_obj: InputTypes) -> bool:
        ...
        from kfx.inputs.input_mixin import SENSITIVE_FIELD_TYPES
    ...
    async def _build_tools_metadata_input(self):
        try:
            from kfx.inputs.inputs import ToolsInput
        except ImportError as e:
            msg = "Failed to import ToolsInput from kfx.inputs.inputs"
    ...
    def is_connected_to_chat_input(self) -> bool:
        # Lazy import to avoid circular dependency
        from kfx.graph.utils import has_chat_input
    ...
    async def send_message(self, message: Message, id_: str | None = None, *, skip_db_update: bool = False):
        ...
        This is the central method for sending messages in Ketos. It handles:
